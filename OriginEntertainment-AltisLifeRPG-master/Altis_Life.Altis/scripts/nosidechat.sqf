...
[] spawn {
	...
	sound_peak = {[60,15] call fnc_usec_pitchWhine;for "_i" from 1 to 15 do {playSound format ["%1",_this select 0];};};
	warning_string = {1 cutText [format ["%1",_this select 0],"PLAIN DOWN"];2 cutText [format ["%1",_this select 0],"PLAIN"];};
	warning = {_randomnr = [2,-1] call BIS_fnc_selectRandom;(vehicle player) SetVelocity [_randomnr * random (4) * cos getdir (vehicle player), _randomnr * random (4) * cos getdir (vehicle player), random (4)];};
			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == localize "str_channel_side") then 
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						...
						[] spawn warning;
						["NO VOICE ON SIDE"] spawn warning_string;
					};
				};
				...
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						...
						[] spawn warning;
						["NO VOICE ON SIDE! This is your LAST warning!"] spawn warning_string;
					};
				};
				if (disconnect_me >= 3) then {
					if (isNil "warn_last") then {
						...
						playMusic ["PitchWhine",0];
						[] spawn warning;
						["We warned you..."] spawn warning_string;
						1 fademusic 10;
						1 fadesound 10;
