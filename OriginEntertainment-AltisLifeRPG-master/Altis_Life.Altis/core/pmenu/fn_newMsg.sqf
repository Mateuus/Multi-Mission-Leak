...
switch(_type) do
{
	...
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["The W.A.P.D is currently unavailable."];};
	...
	case 8:
	{
		if(({side _x == east} count playableUnits) == 0) exitWith {hint format["The E.A.P.D is currently unavailable."];};
