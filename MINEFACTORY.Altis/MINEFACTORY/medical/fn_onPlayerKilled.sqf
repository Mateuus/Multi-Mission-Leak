...
if ((_unit GVAR ["isCalling", false]) && !isNull life_currentPhonePartner) then { [6, []] call phoneCalls_fnc_partial; };
life_currentPhonePartner = ObjNull;
_unit SVAR ["incomingCall", false];
_unit SVAR ["isCalling", false, true];
...
_unit spawn {
	...
	_Timer ctrlSetText  "Du kannst Respawnen";
};
...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {CASH > 0}) then {
		[format[ "%1 wurde als Bankräuber identifiziert!Dead",[CASH] call life_fnc_numberText]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
		CASH = 0;
	};
};
