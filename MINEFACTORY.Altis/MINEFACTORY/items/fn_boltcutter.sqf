...
if((typeOf _building) == "Land_Research_house_V1_F" && (nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) GVAR ["locked",true]) exitWith {hint  "You must open the outside doors before opening it!"};
...
if((_building GVAR [format["bis_disabled_Door_%1",_door],0]) == 0) exitWith {hint  "Die Tür ist bereits offen!"};
...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	[[1,2],"!!!!! JEMAND BRICHT IN DIE ZENTRALBANK EIN !!!!!!",true,[]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
} else {
	[0,"%1 wurde beim Einbruch in ein Haus gesehen.",true,[profileName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
};

life_action_inUse = true;
//Setup the progress bar
disableSerialization;
_title =  "Schloss wird aufgebrochen";
