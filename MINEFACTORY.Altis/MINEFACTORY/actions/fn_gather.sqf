...
if (_requiredItem != "") then
{
	...
	if (_valItem < 1) exitWith
	{
		...
		{
			case "pickaxe": {[ "Eine Spitzhacke wird benötigt."] spawn life_fnc_nachricht;};  
		};
...
if(_diff == 0) exitWith {
	[ "Dein Inventar ist voll."] spawn life_fnc_nachricht;
	life_action_inUse = false;
};
...
if(([true,_resource,_diff] call life_fnc_handleInv)) then
{
	...
	titleText[format[ "Du hast %2 %1(s) gesammelt",( _itemName),_diff],"PLAIN"];
};
