...
[0,"%1 dein %2 wird von der Polizei beschlagnahmt.",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
life_action_inUse = true;

_upp =  "Fahrzeug wird beschlagnahmt";
...
if(player distance _vehicle > 10) exitWith {hint  "Fahrzeug wird beschlagnahmtCancelled"; life_action_inUse = false;};
...
if(playerSide == west) then {
	...
	if(playerSide == west) then {
			[0,"%1 hat %2's %3 beschlagnahmt",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
			if(_vehicle in life_vehicles) then {
			SUB(BANK,_price); hint format[ "Du hast %1 $ bezahlt, weil du dein eigenen %2 beschlagnahmt hast.",_type,_price];
			} else {
			ADD(BANK,_price); hint format[ "Du hast einen %1 beschlagnahmt\n\nDu hast $%2 für das Aufräumen der Straßen bekommen!",_type,_price];
			};
			if(life_atmbank < 0) then {life_atmbank = 0};
	};
} else {
	hint  "Fahrzeug wird beschlagnahmtCancelled";
};
