...

{
	...
	if (CHVD_allowNoGrass) then {
		_textLow = if (isLocalized "Leicht") then { "Leicht"} else {"Low"};
		_ctrl lbAdd _textLow;
	};
	_textStandard = if (isLocalized "standard") then { "standard"} else {"Standard"};
	_ctrl lbAdd _textStandard;
	_textHigh = if (isLocalized "Hoch") then { "Hoch"} else {"High"};
	_ctrl lbAdd _textHigh;
	_textVeryHigh = if (isLocalized "Sehr hoch") then { "Sehr hoch"} else {"Very High"};
