	...
	} else {
{
	...
	_time = round (_time / 3);
	if (_status == 0)then {
	_myListbox lbAdd format ["%5 Stück von %3 für $%2 von %1, endet in %4 Stunden",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText,[_amount] call life_fnc_numberText];
...
hint "Erfolgreich alle Anzeigen gelden";