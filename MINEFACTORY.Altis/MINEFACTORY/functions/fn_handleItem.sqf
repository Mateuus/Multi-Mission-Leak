						...
						if(_ispack) then {
							...
							if(_ongun) then {
								...
								if(_override) then {
									...
									if(_slotTaken) then {
										_action = [ "Möchtest du den Aufsatz zu deiner Waffe oder deinem Inventar hinzufügen? Wenn du es zur Waffe hinzufügst, geht der vorhandene Aufsatz verloren!", "Aufsatz Platz belegt!", "Waffe", "Inventar"] call BIS_fnc_guiMessage;
										if(_action) then {
											switch(_type) do {
												case 1: {player addPrimaryWeaponItem _item;};
												case 2: {player addSecondaryWeaponItem _item;};
