...
if(playerSide == west) then {
	...
	_Btn4 ctrlSetText  "Aus Fahrzeug ziehen";
	...
	if(_curTarget isKindOf "Ship") then {
		_Btn6 ctrlSetText  "Boot schieben";
		...
		if((typeOf (_curTarget) in _dlcVehicles) && !(288520 in getDLCs 1)) then {
			if(_curTarget isKindOf "Air") then {
				_Btn6 ctrlSetText  "In den Helikopter einsteigen";
			} else {
				_Btn6 ctrlSetText  "Ins Kart setzen";
			};
	...
	if(_curTarget isKindOf "Ship") then {
		_Btn2 ctrlSetText  "Boot schieben";
		...
		if((typeOf (_curTarget) in _dlcVehicles) && !(288520 in getDLCs 1)) then {
			if(_curTarget isKindOf "Air") then {
				_Btn2 ctrlSetText  "In den Helikopter einsteigen";
			} else {
				_Btn2 ctrlSetText  "Ins Kart setzen";
			};
	...
	if(typeOf _curTarget == "O_Truck_03_device_F") then {
		_Btn3 ctrlSetText  "Vom Gerät abbauen";
		...
		if(typeOf (_curTarget) in ["C_Van_01_fuel_F","I_Truck_02_fuel_F","B_Truck_01_fuel_F"] && _curTarget in life_vehicles) then {
			if(!isNil {_curTarget getVariable "fuelTankWork"}) then {
				_Btn3 ctrlSetText  "Vorgang abgebrochen";
