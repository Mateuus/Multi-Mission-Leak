...
_mode = SEL(_this,0);
_exit = false;
if((lbCurSel 2302) == -1) exitWith {hint  "Du hast kein Fahrzeug ausgewählt!";closeDialog 0;};
...
_licensesName = "";
{
	if(!(EQUAL(_x,"")) && {!(LICENSE_VALUE(_x,_shopSide))}) then {
		ADD(_licensesName, M_CONFIG(getText,"Licenses",_x,"displayName") + "<br/>");
		_exit = true;
	};
} foreach _licenses;

if(_exit) exitWith {hint parseText format[( "Du besitzt die benötigte Lizenz nicht!")+ "<br/><br/>%1",_licensesName];closeDialog 0;};

if(_basePrice < 0) exitWith {closeDialog 0;}; //Bad price entry
if(CASH < _basePrice) exitWith {hint format[ "Du hast nicht genug Geld, um das Fahrzeug zu kaufen.\n\nFehlender Betrag: $%1",[_basePrice - CASH] call life_fnc_numberText];closeDialog 0;};
...
if(EQUAL(_spawnPoint,"")) exitWith {hint  "Ein Fahrzeug blockiert gerade den Spawnpunkt";closeDialog 0;};
SUB(CASH,_basePrice);
hint format[ "Du kaufst eine(n) %1 für $%2",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_basePrice] call life_fnc_numberText];
