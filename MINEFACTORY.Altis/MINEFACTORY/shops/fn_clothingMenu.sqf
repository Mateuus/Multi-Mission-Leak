...
if(player != vehicle player) exitWith {titleText[ "Du führst bereits eine Aktion aus. Bitte warte, bis diese beendet ist.InVehicle","PLAIN"];};
_exit = false;
...
if(!(EQUAL(_license,""))) then {
	...
	if(!(LICENSE_VALUE(_license,_flag))) exitWith {hint  "Du besitzt die benötigte Lizenz nicht!"; _exit = true;};
};
if(_exit) exitWith {};
...
(findDisplay 3100) displaySetEventHandler ["KeyDown","if((_this select 1) == 1) then {closeDialog 0;}"]; //Fix Custom Skin after ESC

sliderSetRange [3107, 0, 360];

//Cop / Civ Pre Check
if((SEL(_this,3) in ["bruce","dive","reb","kart"] && playerSide != civilian)) exitWith {hint  "Du musst ein Zivilist sein, um diesen Shop nutzen zu können!"; closeDialog 0;};
if((SEL(_this,3) == "reb" && !license_civ_rebel)) exitWith {hint  "Du besitzt keine Rebellen Lizenz!"; closeDialog 0;};
if((SEL(_this,3) in ["cop"] && playerSide != west)) exitWith {hint  "Du musst ein Polizist sein, um diesen Shop nutzen zu können!"; closeDialog 0;};
if((SEL(_this,3) in ["dive"] && !license_civ_dive)) exitWith { hint  "Du benötigst eine Taucherlizenz, um diesen Tauchershop nutzen zu können!"; closeDialog 0;};
...
if(isClass(missionConfigFile >> "Licenses" >> life_clothing_store)) then {
	...
	if(!(LICENSE_VALUE(life_clothing_store,_flag))) exitWith {
		hint format[ "Du benötigst eine %1, um diesen Shop nutzen zu können!", _displayName];
		closeDialog 0;
	};
};
