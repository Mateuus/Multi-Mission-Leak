...
if(count _invs > 0) then {
	{
		...
		_inv = _inv + format["%1 %2<br/>",SEL(_x,1),( _displayName)];
	...
	[0,"%1 hatte Schmuggelware im Wert von $%2 bei sich.",true,[(_civ GVAR ["realname",name _civ]),[_illegal] call life_fnc_numberText]] remoteExecCall ["life_fnc_broadcast",west];
} else {
	_inv =  "Keine illegalen Gegenstände";
};

if(!alive _civ || player distance _civ > 5) exitWith {hint format[ "Konnte %1 nicht durchsuchen",_civ GVAR ["realname",name _civ]]};
//hint format["%1",_this];
...
if(_robber) then {
	[0,"%1 wurde als Bankräuber identifiziert!",true,[(_civ GVAR ["realname",name _civ])]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
};
