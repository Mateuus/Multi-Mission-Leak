...
[] spawn {
	...
	while {true} do {
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player GVAR ["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText[ "Du wurdest automatisch freigelassen, da die maximale Verhaftungszeit überschritten wurde. Wenn die RP Situation noch Läuft lasse sich wieder fesseln e.t.c.!","PLAIN"];
		};
	};
};

titleText[format[ "Du wurdest von %1 festgenommen",_cop GVAR ["realname",name _cop]],"PLAIN"];
