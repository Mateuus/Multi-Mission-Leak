...
class CfgRemoteExec {
    class Functions {
		...
		F(life_fnc_news_open,CLIENT)
		F(life_fnc_setLocker,CLIENT)
		F(life_fnc_spielernotiz,CLIENT)
		...
		F(DB_fnc_news,SERVER)
		F(DB_fnc_news_new,SERVER)
		F(TON_fnc_getUpdate,SERVER)
		F(DB_fnc_fahrschulen,SERVER)
		F(TON_fnc_robShops,SERVER)
		F(TON_fnc_shopState,SERVER)
		F(slocker_fnc_lockerReturnData,SERVER)
		F(slocker_fnc_lockerUpdateInventory,SERVER)
		F(slocker_fnc_lockerInsertPlayer,SERVER)
		F(TON_fnc_vAH_update,SERVER)
		F(TON_fnc_update,SERVER)
		F(TON_fnc_insertbericht,SERVER)
		...
		F(life_fnc_recieveMessage,ANYONE)
		F(TON_fnc_pcplayer_query,ANYONE)
		F(life_fnc_spielerinfo,ANYONE)
		F(life_fnc_initdatabase,ANYONE)
		F(life_fnc_globalSoundClient,ANYONE)
        F(phoneCallsServer_fnc_startCall,ANYONE)
        F(phoneCallsServer_fnc_stopCall,ANYONE)
