...
KRON_validateName = {
	private ["_name","_whitelist","_allowed"];
	_whitelist = toArray "abcdefghijklmnopqrstuvwxyzåäö_ABCDEFGHIJKLMNOPQRSTUVWXYZÅÄÖ [](){}|.ø1234567890?!#%&$-*";
	_whitelist = _whitelist + [229,228,246,197,196,214]; //åäöÅÄÖ
...
KRON_startsWith = { //Om _string börjar med _start returnar den true
	//Funkar ej, använd life_fnc_rizooueqiujrzomxvqjrvxtfracppjrckttqkukvwwticyrsdkwhimpaeao
};
...
/*
	...
	_allowed = true;
	{
		if(_x not in _whitelist) exitWith {
			_allowed = false;
		};
	} foreach (toArray _name);
	hint (str _allowed);
...
KRON_AddToString = {
	private["_str","_toAdd","_index","_out"];
	_str=_this select 0;
	_toAdd = _this select 1;
	_index = _this select 2;
	
	_out = [];
	{
		if(_forEachIndex == _index) then {
			_out = _out + (toArray _toAdd);
		};
		_out pushBack _x;
