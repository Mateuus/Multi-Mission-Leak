class DeathScreen
{
	...
	class Controls
	{
		...
		class RespawnBtn : Life_RscButtonMenu
		{
			...
			h = (1 / 25);
			text = "Respawn";
			onButtonClick = "closeDialog 0; life_respawned = true; [] call life_fnc_ahlahrugiqknetuumakyonsqyothaelchcsczkdkarivhpuegwjdsgr;";
		...
		class MedicBtn : Life_RscButtonMenu
		{
			...
			w = (9 / 40);
			h = (1 / 25);
			onButtonClick = "[] call life_fnc_hshxheayodwnwvxactodikaqxwqahdvsflthsilxoleiuhxbrvvpbnshcru;";
			text = "Request Medic";
			colorBackground[] = {"(profilenamespace getvariable ['GUI_BCG_RGB_R',0.3843])", "(profilenamespace getvariable ['GUI_BCG_RGB_G',0.7019])", "(profilenamespace getvariable ['GUI_BCG_RGB_B',0.8862])", 0.7};
			class Attributes 
			{
		...
		class DoNotCombatlog : Life_RscText
		{
			idc = 7308;
			colorBackground[] = {0,0,0,0};
			text = "Du är död, du får inte stänga av spelet förrens du har respawnat!";
			x = 0.342656 * safezoneW + safezoneX;
			y = 0.0776 * safezoneH + safezoneY + (5 / 250);
			w = 0.9;
			h = (1 / 25);
		};
	};
