...
*/ 
if(!isNull life_hc_inUse) exitWith {_this remoteExec ["DB_fnc_queryRequest", life_hc_inUse]}; 
...
if(typeName _queryResult == "STRING") exitWith { 
 [] remoteExec ["XAS_fnc_insertPlayerInfo",_ownerID,false]; 
}; 
...
switch (_side) do { 
 ...
 case civilian: { 
  ...
  _houseData = _uid spawn TEX_fnc_fetchPlayerHouses; 
  waitUntil {scriptDone _houseData}; 
  _queryResult pushBack (missionNamespace getVariable[format["houses_%1",_uid],[]]); 
  _gangData = _uid spawn TEX_fnc_queryPlayerGang; 
...
_queryResult remoteExec ["XAS_fnc_requestReceived",_ownerID,false];