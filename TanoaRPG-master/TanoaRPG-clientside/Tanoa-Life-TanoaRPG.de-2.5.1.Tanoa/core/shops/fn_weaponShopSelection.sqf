...
{
	_iS = [_item,__GETC__(life_weapon_shop_array)] call TEX_fnc_index;
	if(_iS == -1) then 
	{
		_price = 0;
	}
	...
	_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#8cff9b'>$%1</t></t>",[(_price)] call tanoarpg_fnc_numberText];
	...
	else
{
	...
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#ff0000'>$%1</t><br/>You lack: <t color='#8cff9b'>$%2</t></t>",[(_price)] call tanoarpg_fnc_numberText,[(_price - life_cash)] call tanoarpg_fnc_numberText];
	}
		else
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#8cff9b'>$%1</t></t>",[(_price)] call tanoarpg_fnc_numberText];
