...
if(life_clothing_lastItem == _data) exitWith {
	life_cMenu_lock = false;
	[player, objNull, 1, "[ANTI-DOPPEL-SYSTEM]", format["Clothing Dupe: %1", _data], "[VERSUCHTES DOPPEL]"] remoteExec ["TEX_FNC_LOGIT", 2];
};

[_data,true,nil,nil,nil,nil,nil,true] call tanoarpg_fnc_handleItem;
life_cMenu_lock = false;
life_clothing_lastItem = _data;
_price ctrlSetStructuredText parseText format [(localize "STR_GNOTF_Price")+ " <t color='#8cff9b'>$%1</t>",[(_control lbValue _selection)] call tanoarpg_fnc_numberText];
...
_total ctrlSetStructuredText parseText format [(localize "STR_Shop_Total")+ " <t color='#8cff9b'>$%1</t>",[_totalPrice] call tanoarpg_fnc_numberText];