...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","soldner"] && playerSide != civilian) exitWith {hint "Du bist kein Zivilist!"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Du benötigst ein Rebellentraining!"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "Du bist kein Polizist, daher kannst du diesen Shop nicht nutzen!"; closeDialog 0;};
...
if((_this select 3) in ["dive"] && !license_civ_dive) exitWith {hint "Du brauchst eine Taucherlizenz um diesen Shop benutzen zu können!"; closeDialog 0;};
if((_this select 3) in ["adac"] && playerSide != independent) exitWith {hint "ADAC"; closeDialog 0;};
if((_this select 3) in ["imperium"] && playerSide != east) exitWith {hint "Du bist kein Imperialist!"; closeDialog 0;};


life_clothing_store = (_this select 3);

//License Check?
_var = [life_clothing_store,0] call tanoarpg_fnc_licenseType;
if(_var select 0 != "") then
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {hint format["You need a %1 to buy from this shop!",[_var select 0] call tanoarpg_fnc_varToStr]; closeDialog 0;};
};
...
if(isNil "life_clothesPurchased") exitWith
{
	...
	if(backpack player != "") then
	{
			...
			else
		{
			...
			{
				{
					[_x,true,true] call tanoarpg_fnc_handleItem;
	...
	{
		{[_x,true,false,false,true] call tanoarpg_fnc_handleItem;} foreach life_oldUniformItems;
	};
	...
	if(vest player != "") then
	{
			...
			else
		{
			...
			{
				{[_x,true,false,false,true] call tanoarpg_fnc_handleItem;} foreach life_oldVestItems;
			};
		};
	};
};
...
if((life_clothing_purchase select 4) == -1) then
{
	if(life_oldBackpack != backpack player) then
	{
		if(life_oldBackpack == "") then {removeBackpack player;} else
		{
			removeBackpack player;
			player addBackpack life_oldBackpack;
			{[_x,true,true] call tanoarpg_fnc_handleItem;} foreach life_oldBackpackItems;
		};
	};
};
