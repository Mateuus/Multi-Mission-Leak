...
{
	{
		_inv = _inv + format["%1 %2<br/>",_x select 1,[([_x select 0,0] call tanoarpg_fnc_varHandle)] call tanoarpg_fnc_varToStr];
		_index = [_x select 0,__GETC__(sell_array)] call TEX_fnc_index;
		if(_index != -1) then
		{
	...
	{
		[getPlayerUID _civ,_civ getVariable["realname",name _civ],"482"] remoteExec ["tanoarpg_fnc_wantedAdd", 2, false];
	};
	
	[getPlayerUID _civ,_civ getVariable["realname",name _civ],"481"] remoteExec ["tanoarpg_fnc_wantedAdd", 2, false];
	[0,"STR_Cop_Contraband",true,[(_civ getVariable["realname",name _civ]),[_illegal] call tanoarpg_fnc_numberText]] remoteExec ["tanoarpg_fnc_broadcast", west, false];
}
	else
{
...
if(_robber) then
{
	[0,"STR_Cop_Robber",true,[(_civ getVariable["realname",name _civ])]] remoteExec ["tanoarpg_fnc_broadcast", -2, false];
};