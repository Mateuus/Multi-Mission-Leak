...
//if(isNull _vehicle OR !(_vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "House_F"  OR (typeOf _vehicle in life_container))) exitWith {}; //Either a null or invalid vehicle type.
if(isNull _vehicle OR !(("Land_Wreck_Traw_F" == typeOf _vehicle) OR ("Land_Wreck_Traw2_F" == typeOf _vehicle) OR _vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "House_F" OR (typeOf _vehicle in life_container))) exitWith {}; //Either a null or invalid vehicle type.
...
if(_vehicle isKindOf "House_F") then {
	private["_mWeight"];
	_mWeight = 0;
	{_mWeight = _mWeight + ([(typeOf _x)] call tanoarpg_fnc_vehicleWeightCfg);} foreach (_vehicle getVariable["containers",[]]);
	_veh_data = [_mWeight,(_vehicle getVariable["Trunk",[[],0]]) select 1];
} else {
	_veh_data = [_vehicle] call tanoarpg_fnc_vehicleWeight;
};
...
[_vehicle] call tanoarpg_fnc_vehInventory;
...
_vehicle spawn
{
	...
	if(_this isKindOf "House_F") then {
		[(_this getVariable["Trunk",[[],0]]), (_this getVariable["house_id",-1]), (getPosATL _this)] remoteExec ["TEX_fnc_updateHouseTrunk", 2, false];
	};
};