...
if(_veh isKindOf "House_F") then {
	private["_mWeight"];
	_mWeight = 0;
	{_mWeight = _mWeight + ([(typeOf _x)] call tanoarpg_fnc_vehicleWeightCfg);} foreach (_veh getVariable["containers",[]]);
	_veh_data = [_mWeight,(_veh getVariable["Trunk",[[],0]]) select 1];
} else {
	_veh_data = [_veh] call tanoarpg_fnc_vehicleWeight;
};
...
//Player Inventory Items
{
		...
		else
	{
		if(life_cash > 0) then
		{
			_pInv lbAdd format["$%1",[life_cash] call tanoarpg_fnc_numberText];
...
//Trunk Inventory Items
{
	...
	{
		_var = [_x select 0,0] call tanoarpg_fnc_varHandle;
		...
		else
	{
		...
		if(_val > 0) then
		{
			_tInv lbAdd format["$%1",[_val] call tanoarpg_fnc_numberText];
