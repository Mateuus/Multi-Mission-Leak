...
_num = ctrlText 3505;
if(!([_num] call TEX_fnc_isnumber)) exitWith {hint localize "STR_MISC_WrongNumFormat";};
_num = parseNumber(_num);
if(_num < 1) exitWith {hint localize "STR_MISC_Under1";};

_index = [_ctrl,((life_trunk_vehicle getVariable "Trunk") select 0)] call TEX_fnc_index;
...
_num = [_ctrl,_num,life_carryWeight,life_maxWeight] call tanoarpg_fnc_calWeightDiff;
if(_num == 0) exitWith {hint localize "STR_NOTF_InvFull"};
_weight = ([_ctrl] call tanoarpg_fnc_itemWeight) * _num;
if(_ctrl == "money") then
{
	if(_num == _value) then
	{
...
{
	if([true,_ctrl,_num] call tanoarpg_fnc_handleInv) then
	{
		if(_num == _value) then
		{
			_data set[_index,-1];
