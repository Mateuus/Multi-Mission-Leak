...
if(count _itemArray == 0) exitWith
{
    switch(playerSide) do {
        case west: {
            [] call tanoarpg_fnc_copLoadout;
        };
        
        case civilian: {
            [] call tanoarpg_fnc_civLoadout;
        };
		...
		case east: {
            [] call tanoarpg_fnc_imperiumLoadout;
        };
    };
};
...
if(_goggles != "") then {_handle = [_goggles,true,false,false,false] spawn tanoarpg_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_headgear != "") then {_handle = [_headgear,true,false,false,false] spawn tanoarpg_fnc_handleItem; waitUntil {scriptDone _handle};};
...
life_maxWeight = 100;
{
    _item = [_x,1] call tanoarpg_fnc_varHandle;
    [true,_item,1] call tanoarpg_fnc_handleInv;
} foreach (_yItems);
life_maxWeight = 24;

//Primary & Secondary (Handgun) should be added last as magazines do not automatically load into the gun.
if(_prim != "") then {_handle = [_prim,true,false,false,false] spawn tanoarpg_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_seco != "") then {_handle = [_seco,true,false,false,false] spawn tanoarpg_fnc_handleItem; waitUntil {scriptDone _handle};};
...
if(playerSide == independent && {uniform player == "U_Rangemaster"}) then {
	[player,0,"skins\human\medic\medic_uniform.paa"] remoteExec ["tanoarpg_fnc_setTexture", -2, false];
};