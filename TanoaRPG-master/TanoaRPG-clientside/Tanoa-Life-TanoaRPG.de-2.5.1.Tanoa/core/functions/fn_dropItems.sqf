	...
	switch(_item) do
	{
		...
		case "life_cash":
		{
			if(life_cash > 0) then
			{
				...
				_obj setPos _pos;
				[player, player, _value, "DROP", life_atmcash, life_cash] remoteExec ["TEX_fnc_handleDBLog", 2];
				[_obj] remoteExec ["tanoarpg_fnc_simDisable", -2,true];
				missionNamespace setVariable[_x,0];
			};
		};
