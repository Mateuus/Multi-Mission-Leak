/*
	...
	Parameter(s):
	_this select 0: STRING - Packet variable name (always "tanoarpg_fnc_MP_packet")
	_this select 1: ARRAY - Packet value (sent by tanoarpg_fnc_MP function; see it's description for more details)
...
if(!(["tanoarpg_fnc_",_functionName] call BIS_fnc_inString) && {!(["SPY_fnc_",_functionName] call BIS_fnc_inString)} && {!(["DB_fnc_",_functionName] call BIS_fnc_inString)} && {!(["TEX_fnc_",_functionName] call BIS_fnc_inString)} &&
{!(toLower(_functionName) in ["bis_fnc_execvm","bis_fnc_effectkilledairdestruction","bis_fnc_effectkilledairdestructionstage2"])} && {!(["XAS_fnc_",_functionName] call BIS_fnc_inString)}) exitWith {false};
	...
	if(toLower(_functionName) == "spy_fnc_cookiejar") exitWith {
		...
		if(_reportUID != _callerUID) exitWith {
			if(isServer && _mode == 0) then {
				...
				[[_callerName,"False reporting to SpyGlass (cheater)"],"SPY_fnc_notifyAdmins",true,false] spawn tanoarpg_fnc_MP;
			};
			_exitScope = true;
		};
	};
...
if (ismultiplayer && _mode == 0) then {
	if (isserver) then {
		if (typename _target == typename []) then {
			...
			//--- Multi execution
			{
				[_varName,[_mode,_params,_functionName,_x,_isPersistent,_isCall,_callerName,_callerUID]] call tanoarpg_fnc_MPexec;
			} foreach _target;
		} else {
			...
			tanoarpg_fnc_MP_packet = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];

			//--- Send to clients
			if (_ownerID < 0) then {
				//--- Everyone
				publicvariable "tanoarpg_fnc_MP_packet";
			} else {
				if (_ownerID != _serverID) then {
					//--- Client
					_ownerID publicvariableclient "tanoarpg_fnc_MP_packet";
				};
			};

			//--- Server execution (for all or server only)
			if (_ownerID < 0 || _ownerID == _serverID) then {
				["tanoarpg_fnc_MP_packet",tanoarpg_fnc_MP_packet] spawn tanoarpg_fnc_MPexec;
			};
