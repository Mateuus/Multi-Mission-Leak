...
with missionnamespace do {
	...
	if(isServer && isDedicated) then {
		tanoarpg_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];
	} else {
		tanoarpg_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,profileName,getPlayerUID player];
	};
	publicvariableserver "tanoarpg_fnc_MP_packet";
