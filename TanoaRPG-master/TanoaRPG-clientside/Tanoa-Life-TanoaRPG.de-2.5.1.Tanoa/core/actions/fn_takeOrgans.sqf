...
if(!([false,"scalpel",1] call tanoarpg_fnc_handleInv)) exitWith {"Du besitzt kein Skalpel"};
...
if(!([true,"kidney",1] call tanoarpg_fnc_handleInv)) exitWith {hint "You don't have room to store any organs!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
...
[player] remoteExec ["tanoarpg_fnc_hasOrgan", _unit, false];