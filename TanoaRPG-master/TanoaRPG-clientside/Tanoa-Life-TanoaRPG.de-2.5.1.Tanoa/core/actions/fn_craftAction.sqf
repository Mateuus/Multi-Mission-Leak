...
_config = [_itemFilter] call tanoarpg_fnc_craftCfg;
{
	...
	if(_item == _x select 0)then
	{
		...
		for [{_i=0},{_i<_invSize-1},{_i=_i+2}] do {
			...
			_str = [_matsNeed select _i] call tanoarpg_fnc_varToStr;
...
if(_itemFilter == "item") then {
	_weight = ([_item] call tanoarpg_fnc_itemWeight);
};
...
if(_itemFilter == "item") then{
	_itemName = [_newItem] call tanoarpg_fnc_varToStr;
} else {
	_itemInfo = [_newItem] call tanoarpg_fnc_fetchCfgDetails;
...
for [{_i=0},{_i<_invSize-1},{_i=_i+2}] do {
	...
	_handledItem = [_oldItem select _i,1] call tanoarpg_fnc_varHandle;
	if(!([false,_handledItem,_oldItem select _i+1] call tanoarpg_fnc_handleInv)) exitWith {_removeItemSuccess = false;};
};
...
if(_itemFilter == "item") then{
	_handledItem = [_newItem,1] call tanoarpg_fnc_varHandle;
	[true,_handledItem,1] call tanoarpg_fnc_handleInv;
};
...
if(_itemFilter == "weapon") then{
	...
	if(player canAdd _newItem) then{
		...
		if(currentWeapon player == "") then{
			...
			for [{_i=0},{_i<_invSize-1},{_i=_i+2}] do {
				_handledItem = [_oldItem select _i,1] call tanoarpg_fnc_varHandle;
				[true,_handledItem,_oldItem select _i+1] call tanoarpg_fnc_handleInv;
			};
			life_is_processing = false;
		};
	};
