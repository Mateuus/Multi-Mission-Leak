...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	[0,"STR_NOTF_BeingImpounded_1",true,[(_vehicleData select 0) select 1,_vehicleName]] remoteExec ["tanoarpg_fnc_broadcast", -2,false];
	life_action_inUse = true;
	
	_upp = localize "STR_NOTF_Impounding_1";
	...
	if(player distance _vehicle > 10) exitWith {hint localize "STR_NOTF_ImpoundingCancelled_1"; life_action_inUse = false;};
	...
	if((count crew _vehicle) == 0) then
	{
		...
		[_vehicle,true,player,true] remoteExec ["TEX_fnc_vehicleStore", 2, false];
		waitUntil {!life_impound_inuse};
		hint format[localize "STR_NOTF_Impounded_1",_type,_price];
		[0,"STR_NOTF_HasImpounded_1",true,[profileName,(_vehicleData select 0) select 1,_vehicleName]] remoteExec ["tanoarpg_fnc_broadcast", -2, false];
		life_atmcash = life_atmcash + _price;
	}
		else
	{
		hint localize "STR_NOTF_ImpoundingCancelled_1";
	};
};
