...
/*
	...
	Blah blah.
*/
private["_group","_hideout","_action","_cpRate","_cP","_progressBar","_title","_titleText","_ui","_flagTexture","_themarkername","_gangname3"];
...
[[0,1],"STR_GNOTF_CaptureSuccess",true,[name player,(group player) getVariable "gang_name"]] remoteExec ["tanoarpg_fnc_broadcast", -2, false];
_hideout setVariable["inCapture",false,true];
_hideout setVariable["gangOwner",grpPlayer,true];

// CREATE MARKER AT MAP BY Pictureclass

_themarkername = str(getPos _hideout);
_gangname3 = formatText["Eingenommen von: %1",(group player) getVariable "gang_name"];
if (getMarkerColor _themarkername == "") then
{
	gang_owner_marker = createMarker [_themarkername, position player];
	_themarkername setMarkerShape "ICON";
	_themarkername setMarkerType "hd_warning";
	_themarkername setMarkerColor "ColorBlue";
	_themarkername setMarkerText str(_gangname3);
	gang_owner_marker = "";
}
else
{
