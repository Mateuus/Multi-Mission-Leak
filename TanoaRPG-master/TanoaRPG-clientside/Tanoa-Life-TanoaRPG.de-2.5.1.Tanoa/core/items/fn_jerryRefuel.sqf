...
if(!([false,"fuelF",1] call tanoarpg_fnc_handleInv)) exitWith {};
...
switch (true) do
{
	case (_vehicle isKindOF "LandVehicle"):
	{
		...
		{
			[_vehicle,(fuel _vehicle)+0.5] remoteExec ["tanoarpg_fnc_setFuel", _vehicle, false];
		}
			else
		{
	...
	case (_vehicle isKindOf "Air"):
	{
		if(!local _vehicle) then
		{
			[_vehicle,(fuel _vehicle)+0.2] remoteExec ["tanoarpg_fnc_setFuel", _vehicle, false];
		}
			else
		{
	...
	case (_vehicle isKindOf "Ship"):
	{
		if(!local _vehicle) then
		{
			[_vehicle,(fuel _vehicle)+0.35] remoteExec ["tanoarpg_fnc_setFuel", _vehicle, false];
		}
			else
		{
...
[true,"fuelE",1] call tanoarpg_fnc_handleInv;