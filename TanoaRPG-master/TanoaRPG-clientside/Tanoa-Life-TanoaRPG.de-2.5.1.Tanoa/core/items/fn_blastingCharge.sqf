...
if(((fed_bank getVariable ["Rsb", ObjNull]) getVariable "bis_disabled_Door_1") == 1) then
{
	_glitch = 0;
	...
	if(_glitch == 0) then
	{
		...
		[2,-1,"Ich hab mich in die Bank GEGLITCHED und stehe gefesselt im Polizei HQ Lijnhaven und warte auf meine Bestrafung !!!"] call tanoarpg_fnc_newMsg;
...
if(!([false,"blastingcharge",1] call tanoarpg_fnc_handleInv)) exitWith {}; //Error?

_vault setVariable["chargeplaced",true,true];
[0,"STR_ISTR_Blast_Placed"] remoteExec ["tanoarpg_fnc_broadcast", west, false];
[player, objNull, 1, "[BANK]", "Hat das C4 platziert"] remoteExec ["TEX_FNC_LOGIT", 2];
hint localize "STR_ISTR_Blast_KeepOff";
_handle = [] spawn tanoarpg_fnc_demoChargeTimer;
[] remoteExec ["tanoarpg_fnc_demoChargeTimer", west, false];
