...
if(!([_amount] call TEX_fnc_isnumber)) exitWith {hint "You didn't enter an actual number format.";ctrlShow[2001,true];};
...
hint format["You gave $%1 to %2",[(parseNumber(_amount))] call tanoarpg_fnc_numberText,_unit getVariable["realname",name _unit]];
life_cash = life_cash - (parseNumber(_amount));
[0] call XAS_fnc_updatePartial;
[_unit,_amount,player] remoteExec ["tanoarpg_fnc_receiveMoney", _unit, false];
[player, _unit, _amount, "BAR", life_atmcash, life_cash] remoteExec ["TEX_fnc_handleDBLog", 2];
