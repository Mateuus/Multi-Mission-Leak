...
_action = [
	...
	[(_houseCfg select 0)] call tanoarpg_fnc_numberText,
...
if(_action) then {
	...
	[1] call XAS_fnc_updatePartial;	//NEU GEGEN COMBAT LOG BEIM HAUSKAUF DAS GELD BLEIBT !!!!
	[_uid,(str (getPosATL _house)),player] remoteExec ["TEX_fnc_addHouse", 2, false];
	...
	[player, player, (_houseCfg select 0), "HOUSEBOUGHT", life_atmcash, life_cash] remoteExec ["TEX_fnc_handleDBLog", 2];
};