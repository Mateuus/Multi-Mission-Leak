...
if(!([_uid] call tanoarpg_fnc_isUIDActive)) exitWith {hint localize "STR_House_Raid_OwnerOff"};
...
_value = 0;
{
	...
	_index = [_var,life_illegal_items] call TEX_fnc_index;
	if(_index != -1) then {
		_vIndex = [_var,__GETC__(sell_array)] call TEX_fnc_index;
		if(_vIndex != -1) then {
			_houseInvData set[_forEachIndex,-1];
			_houseInvData = _houseInvData - [-1];
			_houseInvVal = _houseInvVal - (([_var] call tanoarpg_fnc_itemWeight) * _val);
...
if(_value > 0) then {
	[0,"STR_House_Raid_Successful",true,[[_value] call tanoarpg_fnc_numberText]] remoteExec ["tanoarpg_fnc_broadcast", -2, false];
	life_atmcash = life_atmcash + _value;
	[player, player, _value, "RAIDHOUSE", life_atmcash, life_cash] remoteExec ["TEX_fnc_handleDBLog", 2];
	_house setVariable["Trunk",[_houseInvData,_houseInvVal],true];
	[(_house getVariable["Trunk",[[],0]]), (_house getVariable["house_id",-1]), (getPosATL _house)] remoteExec ["TEX_fnc_updateHouseTrunk", 2, false];
} else {
