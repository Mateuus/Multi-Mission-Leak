...
_action = [
	format[(localize "STR_GNOTF_MaxMemberMSG")+ "<br/><br/>" +(localize "STR_GNOTF_CurrentMax")+ "<br/>" +(localize "STR_GNOTF_UpgradeMax")+ "<br/>" +(localize "STR_GNOTF_Price")+ " <t color='#8cff9b'>$%3</t>",_maxMembers,_slotUpgrade,[_upgradePrice] call tanoarpg_fnc_numberText],
...
if(_action) then {
	if(life_atmcash < _upgradePrice) exitWith {
		hint parseText format[
			...
			[life_atmcash] call tanoarpg_fnc_numberText,
			[_upgradePrice - life_atmcash] call tanoarpg_fnc_numberText
		];
	};
	__SUB__(life_atmcash,_upgradePrice);
	grpPlayer setVariable["gang_maxMembers",_slotUpgrade,true];
	hint parseText format[localize "STR_GNOTF_UpgradeSuccess",_maxMembers,_slotUpgrade,[_upgradePrice] call tanoarpg_fnc_numberText];
	[2,grpPlayer] remoteExec ["TEX_fnc_updateGang", 2, false];
} else {
