...
if(!isNull (findDisplay 3494)) then {
    ...
    lbClear _list;
    life_last_vehicles = [];
    _objects = nearestObjects [player, ["Air","Car","Ship"], 200];
    {
        if(vehicle player != _x) then {
            _color = [(typeOf _x),(_x getVariable "Life_VEH_color")] call tanoarpg_fnc_vehicleColorStr;
            _text = format["(%1)",_color];
            _list lbAdd format ["%1 - %2 (%3)", getText(configFile >> "cfgVehicles" >> typeOf _x >> "DisplayName"), _text, round(player distance _x)];
            life_last_vehicles pushBack _x;
        };
    } foreach _objects;
};