...
_text = "<t size='1.5' color='#00FF00'>TanoaRPG Boerse</t><br/>";
...

{
	...
	_row = [_x] call tanoarpg_fnc_marketGetPriceRow;
	
	_price = _row select 1;
	_globalchange = _row select 2;
	_lastchange = _row select 3;
	
	_resname = [_x, 0] call tanoarpg_fnc_varHandle;
	_resname = [_resname] call tanoarpg_fnc_varToStr;
	
	_text = format["%1<br/><t size='1.2'>%2</t><br/><t size='1.3' color='#FFFF00'>$%3</t><br/>", _text,_resname, [_price] call tanoarpg_fnc_numberText];
