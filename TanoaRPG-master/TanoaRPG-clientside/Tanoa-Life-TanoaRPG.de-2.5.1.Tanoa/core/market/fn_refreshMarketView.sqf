...
if(_selectedindex == -1) then
{
	...
	{
		_index = lbAdd [1500, [ [_x select 0, 0] call tanoarpg_fnc_varHandle ] call tanoarpg_fnc_varToStr ];
...
else
{
	...
	
	{
		...
		{
			ctrlSetText [1001, format["$%1", [_x select 1] call tanoarpg_fnc_numberText ] ];
			
			//Trend Global
			ctrlSetText [1004, format["$%1", [_x select 2] call tanoarpg_fnc_numberText ] ];
			...
			//Trend local
			ctrlSetText [1005, format["$%1", [_x select 3] call tanoarpg_fnc_numberText ] ];
