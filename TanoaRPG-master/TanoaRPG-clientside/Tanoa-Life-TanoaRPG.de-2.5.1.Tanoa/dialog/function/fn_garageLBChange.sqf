...
_vehicleColor = [_className,_dataArr select 1] call tanoarpg_fnc_vehicleColorStr;
_vehicleInfo = [_className] call tanoarpg_fnc_fetchVehInfo;
_trunkSpace = [_className] call tanoarpg_fnc_vehicleWeightCfg;

_retrievePrice = [_className,__GETC__(life_garage_prices)] call TEX_fnc_index;
_sellPrice = [_className,__GETC__(life_garage_sell)] call TEX_fnc_index;
...
(getControl(2800,2803)) ctrlSetStructuredText parseText format[
	...
	",
[_retrievePrice] call tanoarpg_fnc_numberText,
[_sellPrice] call tanoarpg_fnc_numberText,
