...
class Life_craft {
	...
	onLoad = "[] spawn tanoarpg_fnc_craft";
	...
	class controls {
		...
		class craftList : life_RscListBox
		{
			idc = 669;
			sizeEx = 0.030;
			onLBSelChanged = "[] spawn tanoarpg_fnc_craft_update";
			x = 0.105;
			y = 0.31;
			w = 0.275; h = 0.44;
		};
		...
		class FilterList : Life_RscCombo
		{
			idc = 673;
			colorBackground[] = {0,0,0,0.7};
			onLBSelChanged  = "[] call tanoarpg_fnc_craft_updateFilter";
			x = 0.69;
			y = 0.32;
			w = (6.25 / 30);
			h = (1 / 25);
		...
		class ButtonCraft : Life_RscButtonMenu {
			...
			colorBackground[] = {"(profilenamespace getvariable ['GUI_BCG_RGB_R',0.3843])", "(profilenamespace getvariable ['GUI_BCG_RGB_G',0.7019])", "(profilenamespace getvariable ['GUI_BCG_RGB_B',0.8862])", 0.5};
			onButtonClick = "if(!life_is_processing) then {[] spawn tanoarpg_fnc_craftAction};";
			x = 0.69;
			y = 0.26;
			w = (6.25 / 40);
			h = (1 / 25);
