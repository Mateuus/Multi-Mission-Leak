...
if (safezone) exitWith {
	hint "Du kannst in der Safzone keine Fahrzeuge aufbrechen!";
};
...
if (_locked == 2) then {
if(_curTarget GVAR ["alarm",false]) then 
	{
		_dist = 400;
		_inRad = [];
		{if (player distance _x < _dist) then {_inRad pushBack _x};}forEach playableUnits;
		[_curTarget,"carAlarm",_dist] remoteExec ["life_fnc_playSound",_inRad];
	};
};};
...
if(!_isVehicle) then {
	...
	if(_dice < 30) then {
		...
		//[_curTarget,"carAlarm",50] remoteExec ["life_fnc_playSound",RCLIENT];
		if(playerSide != independent) then  {[_curTarget,"carAlarm"] call life_fnc_globalSound;};
