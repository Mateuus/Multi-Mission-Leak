...
_pic = "textures\IOL\logoiol.paa";
[
    '<img align=''right'' size=''3'' shadow=''1'' image='+(str(_pic))+' />',
    safeZoneX+1.4,//safeZoneX+0.027,
...
{
   _handle = [] execVM "scripts\IgiLoad\IgiLoadInit.sqf";
};
...
_toDel = [];
_uid = steamid; 
{if ((SEL(_x,5) == _uid) && (SEL(_x,7) == 2)) then {_total = _total + (SEL(_x,4));_toDel pushBack (SEL(_x,0))};}forEach all_ah_items;
if (_total > 0) then 
{
	{
