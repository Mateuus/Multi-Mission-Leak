...
/* Database Related Settings */
    ...
    save_vehicle_items[] = { "pickaxe","fuelEmpty","fuelFull", "spikeStrip", "lockpick", "defuseKit","storageSmall","storageBig","redgull","coffee","waterBottle","apple","peach","banana","tbacon","donuts","rabbit","salema","ornate","mackerel","tuna","mullet","catshark","turtle_soup","hen","rooster","sheep","goat","defibrillator","toolkit" };
...
/* System Settings */
	...
	enable_stamina = false; //Set to false to disable the ARMA 3 Stamina system.
	enable_setcustomaimcoef = true; //Set to 0.15 custom value
    total_maxWeight = 24; //Static variable for the maximum weight allowed without having a backpack
    respawn_timer = 37; //How many seconds a player should wait, before being able to respawn. Minimum 5 seconds.

    /* Channel 7 News Station Configurations */
    news_broadcast_cost = 250000; //Cost for a player to send a news station broadcast.
    news_broadcast_cooldown = 7; //Time in minutes that is required between news station broadcasts. (Default = 20 minutes)
    ...
    fuel_cost = 90; //Cost of fuel per liter at fuel stations (if not defined for the vehicle already).
    ...
    restricted_uniforms[] = { "U_Rangemaster", "U_O_SpecopsUniform_ocamo", "U_B_GEN_Commander_F", "U_B_GEN_Soldier_F", "U_O_T_Soldier_F" };
    restricted_weapons[] = { "launch_O_Titan_F" };
    ...
    sendtoJail_locations[] = { "police_hq_1","swat_hq","ChekpointAlpha_1","armybase","police_hq_2","police_hq_3","police_hq_5","police_hq_4"  }; //Enter the variableName from the mission.sqm here to allow cops to send a person to jail at these locations.

    /* Medical System Configurations */
    revive_cops = false; //true to enable cops the ability to revive everyone or false for only medics/ems.
    revive_fee = 9000; //Revive fee that players have to pay and medics only EMS(independent) are rewarded with this amount.
    hospital_heal_fee = 5000; //Fee to heal at a hospital NPC

    /* Paycheck & Bank System Configurations */
    bank_cop = 10000000; //Amount of cash in bank for new cops
    bank_civ = 10000000; //Amount of cash in bank for new civillians
    bank_med = 10000000; //Amount of cash in bank for new medics

    paycheck_cop = 18000; //Payment for cops 18000
    paycheck_civ = 16000; //Payment for civillians 160000
    paycheck_med = 18000; //Payment for medics 18000
    ...
    seize_uniform[] = { "U_B_GEN_Commander_F", "U_O_SpecopsUniform_ocamo", "U_B_GEN_Soldier_F", "U_Rangemaster", "U_B_CTRG_Soldier_F", "U_O_V_Soldier_Viper_F", "U_O_T_Soldier_F" }; //Any specific uniforms you want to be seized from players
    seize_vest[] = { "V_TacVest_blk_POLICE", "V_TacVest_gen_F","V_PlateCarrierSpec_tna_F", "V_PlateCarrierSpec_tna_F" }; //Any specific vests you want to be seized from players
    seize_headgear[] = { "H_Cap_police","H_Helmet_Skate","H_MilCap_blue", "H_MilCap_gen_F", "H_Beret_gen_F", "H_Beret_Colonel", "H_HelmetB_TI_tna_F", "H_HelmetB_Enh_tna_F" }; //Any hats or helmets you want seized from players
    seize_minimum_rank = 0; //Required minimum CopLevel to be able to seize items from players

    /* Vehicle System Configurations */
    chopShop_vehicles[] = { "Car", "Air", "Ship", "Tank" }; //Vehicles that can be chopped. (Can add: "Ship" and possibly more -> look at the BI wiki...)
    ...
    vehicle_purchase_multiplier_COP = .9; //Cop Vehicle Buy Price = Config_Vehicle price * multiplier
    vehicle_purchase_multiplier_MEDIC = .8; //Medic Vehicle Buy Price = Config_Vehicle price * multiplier
    ...
    vehicle_rental_multiplier_COP = .9; //Cop Vehicle Rental Price = Config_Vehicle price * multiplier
    vehicle_rental_multiplier_MEDIC = .8; //Medic Vehicle Rental Price = Config_Vehicle price * multiplier
    ...
    vehicle_chopShop_multiplier = .6; //Chop Shop price for vehicles. TO AVOID EXPLOITS NEVER SET HIGHER THAN A PURCHASE/RENTAL multipler!   Payout = Config_vehicle Price * multiplier
   ...
   /* Impound Variables */
    ...
    impound_boat = 17500; //Price for impounding boats
    impound_air = 22500; //Price for impounding helicopters / planes
	impound_tank = 25000;//Price for impounding a tank
	
	
    /* Wanted System Settings *
    /* crimes[] = {String, Bounty, Code} */
    crimes[] = {
        {"STR_Crime_187V","25000","187V"},
        {"STR_Crime_187","25000","187"},
        {"STR_Crime_901","50000","901"},
        {"STR_Crime_215","200","215"},
        {"STR_Crime_213","1000","213"},
        {"STR_Crime_211","100","211"},
        {"STR_Crime_207","15000","207"},
