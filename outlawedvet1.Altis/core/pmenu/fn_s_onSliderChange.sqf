...
switch (_mode) do {
	...
	case 5:
    {
        life_settings_oviewDistanceFoot = round(_value);
        ctrlSetText[2952,format["%1",round(_value)]];
        [] call life_fnc_updateViewDistance;
        profileNamespace setVariable ["life_oviewDistanceFoot",round(_value)];
    };

    case 6:
    {
        life_settings_oviewDistanceCar = round(_value);
        ctrlSetText[2962,format["%1",round(_value)]];
        [] call life_fnc_updateViewDistance;
        profileNamespace setVariable ["life_oviewDistanceCar",round(_value)];
    };

    case 7:
    {
        life_settings_oviewDistanceAir = round(_value);
        ctrlSetText[2972,format["%1",round(_value)]];
        [] call life_fnc_updateViewDistance;
        profileNamespace setVariable ["life_oviewDistanceAir",round(_value)];
    };
