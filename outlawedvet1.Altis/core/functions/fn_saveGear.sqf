...
*/
private["_return","_uItems","_bItems","_vItems","_pItems","_hItems","_yItems","_uMags","_vMags","_bMags","_pMag","_hMag","_uni","_ves","_bag","_handled","_rMag","_savedVirtualItems"];
_return = [];
...
if (count (secondaryWeaponMagazine player) > 0 && alive player) then {
    ...
    if (!(_rMag isEqualTo "")) then {
        ...
        if (_ves) then {
            _vMags pushBack __rMag;
            _handled = true;
        };

        if (_uni && !_handled) then {
            _uMags pushBack _rMag;
            _handled = true;
        };

        if (_bag && !_handled) then {
            _bMags pushBack _rMag;
            _handled = true;
        };
    };
};
