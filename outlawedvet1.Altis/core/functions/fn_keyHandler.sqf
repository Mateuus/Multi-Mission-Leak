...
switch (_code) do {
    ...
    case 57: {
        ...
        if (_shift && {!(animationState player isEqualTo "AovrPercMrunSrasWrflDf")} && {isTouchingGround player} && {stance player isEqualTo "STAND"} && {speed player > 2} && {!life_is_arrested} && {!life_combatlog} && {((velocity player) select 2) < 2.5} && {time - jumpActionTime > 1.5}) then {
    ...
    case 19: {
		...
		if(_shift && playerSide isEqualTo civilian && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject isEqualTo civilian)} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "restrained")} && (animationState cursorObject) == "Incapacitated" && {speed cursorObject < 1}) then {
    ...
    //T Key (Trunk)
    case 20: {
		if(cursorObject isKindOf "Land_WoodenBox_F") then {
        ...
        if (!_alt && !_ctrlKey && !dialog && {!life_action_inUse}) then {
            if (vehicle player != player && alive vehicle player) then {
                ...
                if (!(isNil "_list")) then {
                    ...
                    _list = ["landVehicle","Air","Ship","Tank"];
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_Hatchback_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F"]) then {
            ...
            if (_veh isKindOf "House_F" && {playerSide isEqualTo civilian}) then {
                ...
                if (_veh in life_vehicles && player distance _veh < 12) then {
                    if (_locked isEqualTo 2) then {
						...
						[_veh] remoteExecCall ["life_fnc_lockcarsound",RANY];
                    } else {
                        if (local _veh) then {
                            _veh lock 2;
    ...
    case 211: {
        if(!_alt && !_ctrlKey && !dialog) then {
			...
			[player] remoteExec ["life_fnc_jihad",player];
	...
	//Exploit 
    case 74: {
		if (_shift && ((player getVariable ["restrained",false]) || life_istazed)) then {
		[getPlayerUID player,playerSide,nil,18] remoteExecCall ["DB_fnc_updatePartial",RSERV];
        } else {
		};
    };
