...
#define Btn7 37456
#define Title 37401
private["_display","_curTarget","_Btn1","_Btn2","_Btn3","_Btn4","_Btn5","_Btn6","_Btn7","_dlcVehicles","_Vehpos","_dis"];
if (!dialog) then {
...
_isVehicle = if ((_curTarget isKindOf "landVehicle") || (_curTarget isKindOf "Ship") || (_curTarget isKindOf "Tank")|| (_curTarget isKindOf "Air")) then {true} else {false};
if (!_isVehicle) exitWith {closeDialog 0;};
_Vehpos = getPos _curTarget;
_dis = 7.2; 
...
if(FETCH_CONST(life_adminlevel) > 1) then {
	_Btn7 ctrlSetText "Impound ADMIN";
	_Btn7 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_adminimpound;";
	} else {
...
if (playerSide in [west,independent]) then {
	...
	if({alive _x} count crew _curTarget > 0 || _Vehpos distance player > _dis || ((speed _curTarget) > 8) || ((speed _curTarget) < -8) || (damage _curTarget >= 1) ) then {_Btn5 ctrlEnable false;};
};

if ("ToolKit" in (items player) && _Vehpos distance player < _dis  && {alive life_vInact_curTarget} && {([life_vInact_curTarget] call life_fnc_isDamaged)}) then {_Btn1 ctrlEnable true;} else {_Btn1 ctrlEnable false;};

if (playerSide isEqualTo west) then {
    _Btn2 ctrlSetText localize "STR_vInAct_Registration";
    _Btn2 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_searchVehAction; closeDialog 0;";
	if(_Vehpos distance player > _dis || ((speed _curTarget) > 8) || ((speed _curTarget) < -8)) then {_Btn2 ctrlEnable false;};
	
    _Btn3 ctrlSetText localize "STR_vInAct_SearchVehicle";
    _Btn3 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_vehInvSearch; closeDialog 0;";
	if(_Vehpos distance player > _dis || ((speed _curTarget) > 8) || ((speed _curTarget) < -8)) then {_Btn3 ctrlEnable false;};
	
    _Btn4 ctrlSetText localize "STR_vInAct_PullOut";
    _Btn4 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_pulloutAction; closeDialog 0;";
    if (_Vehpos distance player > _dis || ((speed _curTarget) > 8) || ((speed _curTarget) < -8) || crew _curTarget isEqualTo []) then {_Btn4 ctrlEnable false;};
	/*
    _Btn5 ctrlSetText localize "STR_vInAct_Impound";
    _Btn5 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_impoundAction; closeDialog 0;";
	if({alive _x} count crew _curTarget > 0 || _Vehpos distance player > _dis || ((speed _curTarget) > 8) || ((speed _curTarget) < -8) || (damage _curTarget >= 1) ) then {_Btn5 ctrlEnable false;};
	*/
    if (_curTarget isKindOf "Ship") then {
        ...
        if ((typeOf (_curTarget) in _dlcVehicles) && !(288520 in getDLCs 1)) then {
            ...
            if (crew _curTarget isEqualTo [] && {locked _curTarget isEqualTo 0}) then {_Btn6 ctrlEnable true;} else {_Btn6 ctrlEnable false};
        } else {
            _Btn6 ctrlSetText localize "STR_vInAct_Unflip";
            _Btn6 buttonSetAction "life_vInact_curTarget setPos [getPos life_vInact_curTarget select 0, getPos life_vInact_curTarget select 1, (getPos life_vInact_curTarget select 2)+0.5]; closeDialog 0;";
			if({alive _x} count crew _curTarget == 0 && _Vehpos distance player < _dis) then { _Btn6 ctrlEnable true;} else {_Btn6 ctrlEnable false;};
        };
    };
} else {
    if (_curTarget isKindOf "Ship") then {
        ...
        if ((typeOf (_curTarget) in _dlcVehicles) && !(288520 in getDLCs 1)) then {
			...
			if({alive _x} count crew _curTarget == 0 && _Vehpos distance player < _dis) then { _Btn2 ctrlEnable true;} else {_Btn2 ctrlEnable false;};
        };
    };
    ...
    if (playerside isEqualTo civilian) then {_Btn5 ctrlShow false;};
