...
if (playerSide isEqualTo west && _illegal isEqualTo 1) exitWith {
    titleText[format[localize "STR_NOTF_PickedEvidence",_itemName,[round(ITEM_CONTRAPRICE(_itemInfo select 0) / 2)] call life_fnc_numberText],"PLAIN"];
    BANK = BANK + round(ITEM_CONTRAPRICE(_itemInfo select 0) / 2);
    deleteVehicle _this;
