...
#define CASH EMonkeys_cash
#define BANK EMonkeys_atmbank
...
#define EXTDB_SETTING(TYPE,SETTING) TYPE(configFile >> "EMonkeys_Server_Settings" >> "EXTDB" >> SETTING)
#define EXTDB_FAILED(MESSAGE) \
	EMonkeys_server_extDB_notLoaded = [true,##MESSAGE]; \
	PVAR_ALL("EMonkeys_server_extDB_notLoaded"); \
	diag_log MESSAGE;
...
#define ITEM_VARNAME(varName) format["EMonkeys_inv_%1",M_CONFIG(getText,"VirtualItems",varName,"variable")]
...
#define EMonkeys_SETTINGS(TYPE,SETTING) TYPE(missionConfigFile >> "EMonkeys_Settings" >> SETTING)
