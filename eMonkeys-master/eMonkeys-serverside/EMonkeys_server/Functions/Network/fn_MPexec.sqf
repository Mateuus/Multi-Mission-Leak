...
*/
private ["_validFunctions","_params","_exit","_functionName","_target","_isPersistent","_isCall","_varName","_varValue","_function","_callerName","_callerUID","_exitScope"];
_exitScope = false;
...
if(_functionName in Call EMonkeys_MPAllowedC) then{_target = HC_player;};
...
if (ismultiplayer && _mode == 0) then 
{
	...
	if (_canExecute) then 
	{
		...
		switch (true) do 
		{
			case (isserver): 
			{
				if(!(_functionName in Call EMonkeys_MPAllowedS)) then 
				{
					_text = format ["*** Functions | Block Server Funktion: %1 | SpielerID: %3 |",_functionName,_callerUID];
					[[9,_text],"TON_fnc_log",false,false] call EMonkeys_fnc_mp;
					_exit = true;
				}; 
			}; 
		
			case (player == HC_player): 
			{
				if(!(_functionName in Call EMonkeys_MPAllowedHC)) then 
				{
					_exit = true;
					_text = format ["*** Functions | Block HC Funktion: %1 | SpielerID: %3 |",_functionName,_callerUID];
					[[9,_text],"TON_fnc_log",false,false] call EMonkeys_fnc_mp;
				};
			}; 
		};
		
		if (!isnil "_function" && !_exit) then 
		{
			if (_isCall) then 
			{
			...
			_text = format ["*** Functions | Funktion existiert nicht: %1 |",_functionName];
			[[9,_text],"TON_fnc_log",false,false] call EMonkeys_fnc_mp;
			false
		};
	};
