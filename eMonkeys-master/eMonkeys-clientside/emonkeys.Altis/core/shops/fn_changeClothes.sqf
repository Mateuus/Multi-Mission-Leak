/*
	File: fn_changeClothes.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Used in the clothing store to show a 'preview' of the piece of clothing.
*/
disableSerialization;
private["_control","_selection","_data","_price","_total","_totalPrice"];
_control = (_this select 0) select 0;
_selection = (_this select 0) select 1;
_price = (findDisplay 3100) displayCtrl 3102;
_total = (findDisplay 3100) displayCtrl 3106;
if(_selection == -1) exitWith {hint "No selection";};
if(isNull _control) exitWith {hint "No Display"};
if(EMonkeys_cMenu_lock) exitWith {};
EMonkeys_cMenu_lock = true;

EMonkeys_clothing_purchase set[EMonkeys_clothing_filter,(_control lbValue _selection)];

_data = _control lbData _selection;

[_data,true,nil,nil,nil,nil,nil,true] call EMonkeys_fnc_handleItem;
EMonkeys_cMenu_lock = false;
_price ctrlSetStructuredText parseText format ["Preis: <t color='#8cff9b'>%1€</t>",[(_control lbValue _selection)] call EMonkeys_fnc_numberText];

_totalPrice = 0;
{
	if(_x != -1) then
	{
		_totalPrice = _totalPrice + _x;
	};
} foreach EMonkeys_clothing_purchase;

_total ctrlSetStructuredText parseText format ["Total: <t color='#8cff9b'>%1€</t>",[_totalPrice] call EMonkeys_fnc_numberText];