...
if(leader(group player) != player) exitWith {hint "Dir fehlt die Berechtigung"};
...
EMonkeys_gang_list set[_index,[_name,_group,_locked,str(_data),getPlayerUID _data]];
publicVariable "EMonkeys_gang_list";

_group selectLeader _data;
[[_data,_group], "clientGangLeader",_data,false] call EMonkeys_fnc_mp;
