...
lbClear _tInv;
lbClear _pInv;
_veh_data = [_veh] call EMonkeys_fnc_vehicleWeight;
...
//Player Inventory Items
{
		...
		else
	{
		...
		{
			_pInv lbAdd format["%1€",[EMonkeys_c164] call EMonkeys_fnc_numberText];
...
//Trunk Inventory Items
{
		...
		else
	{
		...
		if(_val > 0) then
		{
			_tInv lbAdd format["%1€",[_val] call EMonkeys_fnc_numberText];
