...
_details = [_item] call EMonkeys_fnc_fetchCfgDetails;
...
if(_bool) then
{
	switch((_details select 6)) do
	{
		...
		case "CfgVehicles":
		{
			...
			{ 
				{[_x,true,true,false,true] spawn EMonkeys_fnc_handleItem; } foreach _items;
			};
		};
				...
				switch(_details select 5) do
				{
					...
					case 801: 
					{
							...
							else
						{
							...
							{
								if(player isKindOf "Civilian" || player isKindOf "east") then {
							...
							else
						{
								...
								else
							{
									...
									else
								{
									...
									if(!isNil {_items}) then
									{
										{[_x,true,false,false,true] spawn EMonkeys_fnc_handleItem;} foreach _items;
									};
								};
							};
						};
					...
					case 201:
					{
						...
						{
							private["_type"];
							_type = [_item,201] call EMonkeys_fnc_accType;
							if(_ongun) then
							{
								switch (_type) do
								{
					...
					case 301:
					{
						...
						{
							private["_type"];
							_type = [_item,301] call EMonkeys_fnc_accType;
					...
					case 101:
					{
						...
						{
							private["_type"];
							_type = [_item,101] call EMonkeys_fnc_accType;
