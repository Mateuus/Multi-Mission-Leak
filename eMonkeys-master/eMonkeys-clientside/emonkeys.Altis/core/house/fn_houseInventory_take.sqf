...
if( _selectedindex == -1) exitWith {hint "Nichts ausgewählt.";};
if(!([_count] call fnc_isnumber)) exitWith {hint "Du musst eine Zahl eingeben.";};
if( _selectedindex == -1) exitWith {hint "Nichts ausgewählt.";};
if(EMonkeys_house_current_inv_running) exitWith {hint "Inventar aktualisiert gerade.";};
...
if(_item == "money") then {_item_count = EMonkeys_house_money};

_diff = [_item,_item_count,EMonkeys_carryWeight,EMonkeys_maxWeight] call EMonkeys_fnc_calWeightDiff;
...
{
	if(([true, _item, _count] call EMonkeys_fnc_handleInv) ) then
	{
...
if(_took_success) then
{
	...
	if(!_done) then
	{
		...
		EMonkeys_house_current_inv = []; //empty it for security reasons
		uisleep 3;
