...
switch (_state) do
{
	...
	case 2:
	{
		if(license_civ_driver OR license_civ_air OR license_civ_truck OR license_civ_boat) then {
			license_civ_driver = false;
			hint "Du hast alle deine Fahrzeuglizenzen verloren, da du einen Unfall mit Todesfolge verursacht hast.";
		};
	};
	...
	case 3:
	{
		if(license_civ_gun) then {
			license_civ_gun = false;
			hint "Du hast deinen Waffenschein verloren, da du jemanden getötet hast.";
		};
	};
};