...
_bool = [_this,1,false,[false]] call GTA_fnc_param;
_ispack = [_this,2,false,[false]] call GTA_fnc_param;
_ongun = [_this,3,false,[false]] call GTA_fnc_param;
_override = [_this,4,false,[false]] call GTA_fnc_param;
_toUniform = [_this,5,false,[false]] call GTA_fnc_param; //Manual override to send items specifically to a uniform.
_toVest = [_this,6,false,[false]] call GTA_fnc_param; //Manual override to send items specifically to a vest
_preview = [_this,7,false,[true]] call GTA_fnc_param;
				...
				switch(_details select 5) do
				{
					...
					case 605:
					{
							...
							else
						{
							...
							{
								if(!_preview && { headGear player == _item }) then
								{
									player addItem _item;
								}
									else
							...
							else
						{
								...
								else
							{
								if(player isKindOf "Civilian") then {
									if(!_preview && { uniform player == _item }) then{
										player addItem _item;
									} else {
					...
					case 701:
					{
							...
							else
						{
							...
							{
								if(!_preview && { vest player == _item }) then
								{
									player addItem _item;
								}
									else
								...
								else
							{
									...
									else
								{
									private["_wepItems","_action","_slotTaken"];
										...
										if(_wepItems select 3 != "") then {_slotTaken = true;};
