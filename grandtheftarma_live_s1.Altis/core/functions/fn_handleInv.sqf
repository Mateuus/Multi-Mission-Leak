...
_math = [_this,0,false,[false]] call GTA_fnc_param; //true = add; false = subtract;
_item = [_this,1,"",[""]] call GTA_fnc_param; //The item we are using to add or remove.
_num = [_this,2,0,[0]] call GTA_fnc_param; //Number of items to add or remove.
if(_item == "" OR _num == 0) exitWith {false};

//--- Force refresh of virtual inventory max load value
