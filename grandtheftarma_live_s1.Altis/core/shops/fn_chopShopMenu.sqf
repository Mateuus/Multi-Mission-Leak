...
*/
if(GTA_actionLock) exitWith {hint localize "STR_NOTF_ActionInProc"};
disableSerialization;
private["_nearVehicles","_control"];
_nearVehicles = nearestObjects [getMarkerPos (_this select 3), ["Car", "Truck", "Air", "Tank", "Slingload_01_Base_F"], 25];
...
_control = ((findDisplay 39400) displayCtrl 39402);
{
	if(alive _x) then {
		...
		if(_ind != -1 && count crew _x == 0) then {
			...
			_control lbSetData [(lbSize _control)-1,str([_forEachIndex,_displayName])];
