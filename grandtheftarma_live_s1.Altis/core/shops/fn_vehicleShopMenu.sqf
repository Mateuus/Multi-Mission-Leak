/*
	...
	Blah
*/
private["_shop","_sideCheck","_spawnPoints","_shopFlag","_disableBuy","_exit"];
_shop = [ _this select 3, 0, "", [""] ] call GTA_fnc_param;
_sideCheck = [ _this select 3, 1, sideUnknown, [civilian] ] call GTA_fnc_param;
_spawnPoints = [ _this select 3, 2, "", ["",[]] ] call GTA_fnc_param;
_shopFlag = [ _this select 3, 3, "", [""] ] call GTA_fnc_param;
_disableBuy = [ _this select 3, 5, false, [true] ] call GTA_fnc_param;
_exit = false;
...
if ( playerSide in [ west, independent ] || { _disableBuy } ) then {
