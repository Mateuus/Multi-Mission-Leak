...
//--- Rent only
if(_className in (__GETC__(life_vShop_rentalOnly)) && !license_civ_advrebel) then {
	ctrlEnable [2309,false];
} else {
	if (playerSide in [west, independent] || {life_veh_shop select 3}) then {
		ctrlEnable [2309, false];
	};
};

//--- Buy only
if (_className isKindOf "Slingload_01_Base_F") then {
