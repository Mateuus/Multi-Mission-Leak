...
GTA_actionLock = true; //Lock out the interaction menu for a bit..
_weight = [_vehicle] call life_fnc_vehicleWeight;
if((_weight select 1) >= (_weight select 0)) exitWith {hint localize "STR_NOTF_DeviceFull"; GTA_actionLock = false;};
_resourceZones = ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","heroin_1","cocaine_1","cocaine_2","weed_1","copper_1","iron_1","salt_1","sand_1","diamond_1","oil_1","oil_2","rock_1","rye_1","hops_1","yeast_1","watermelon_1","watermelon_2","silver_1"];
_zone = "";
...
_item = switch(true) do {
	...
	case (_zone in ["heroin_1"]): {"heroin_u"};
	case (_zone in ["rye_1"]): {"rye"};
	case (_zone in ["hops_1"]): {"hops"};
	case (_zone in ["yeast_1"]): {"yeast"};
	case (_zone in ["silver_1"]): {"silver_u"};
	case (_zone in ["cocaine_1","cocaine_2"]): {"cocaine"};
	case (_zone in ["watermelon_1","watermelon_2"]): {"watermelon"};
	case (_zone in ["weed_1"]): {"cannabis"};
	case (_zone in ["copper_1"]): {"copper_u"};
	case (_zone in ["iron_1"]): {"iron_u"};
	case (_zone in ["salt_1"]): {"salt_u"};
	case (_zone in ["sand_1"]): {"sand"};
	case (_zone in ["diamond_1"]): {"diamond_u"};
	case (_zone in ["oil_1","oil_2"]): {"oil_u"};
	case (_zone in ["rock_1"]): {"rock"};
	case (_zone in ["sugar_1"]): {"sugar_u"};
	case (_zone in ["rice_1"]): {"rice_u"};
	default {""};
};

if(_item == "") exitWith {hint "Bad Resource?"; GTA_actionLock = false;};
_vehicle setVariable ["mining",true,true]; //Lock the device
[_vehicle,"life_fnc_soundDevice",true,false] call GTA_fnc_remoteExec; //Broadcast the 'mining' sound of the device for nearby units.

GTA_actionLock = false; //Unlock it since it's going to do it's own thing...
...
while {true} do {
	...
	if(local _vehicle) then {
		...
		[[_vehicle,(fuel _vehicle)-0.04],"life_fnc_setFuel",_vehicle,false] call GTA_fnc_remoteExec;
	};
