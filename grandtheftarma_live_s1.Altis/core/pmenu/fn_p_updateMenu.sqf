...
/*if(__GETC__(GTA_adminLevel) < 1) then
{
	ctrlShow[2020,false];
	ctrlShow[2021,false];
};*/
...
{ 
	...
	if ( player distance _x < 10 && { !( _x getVariable [ "incapacitated", false ] ) } ) then {
...

{
	...
	{
		_near lbAdd format["%1 - %2",_x getVariable["nametag_title",name _x], side _x];
		_near lbSetData [(lbSize _near)-1,str(_x)];
		_near_i lbAdd format["%1 - %2",_x getVariable["nametag_title",name _x], side _x];
		_near_i lbSetData [(lbSize _near)-1,str(_x)];
	};
} foreach _near_units;

_mstatus ctrlSetStructuredText parseText format["<img size='1.3' image='icons\bank.paa'/> <t size='0.8px'>£%1</t><br/><img size='1.2' image='icons\money.paa'/> <t size='0.8'>£%2</t>",[GTA_money_bank] call life_fnc_numberText,[GTA_money_cash] call life_fnc_numberText];
