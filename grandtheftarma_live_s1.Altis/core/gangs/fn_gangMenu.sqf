...
lbClear _members;
{
	if((getPlayerUID _x) == _ownerID) then {
		_members lbAdd format["%1 " +(localize "STR_GNOTF_GangLeader"),(_x getVariable["nametag_title",name _x])];
		_members lbSetData [(lbSize _members)-1,str(_x)];
	} else {
		_members lbAdd format["%1",(_x getVariable["nametag_title",name _x])];
		_members lbSetData [(lbSize _members)-1,str(_x)];
	};
...
{
	_ctrl lbAdd format["%1",_x getVariable["nametag_title",name _x]];
