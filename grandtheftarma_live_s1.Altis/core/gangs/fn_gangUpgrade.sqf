...
if(_action) then {
	if(GTA_money_bank < _upgradePrice) exitWith {
		hint parseText format[
			...
			[GTA_money_bank] call life_fnc_numberText,
			[_upgradePrice - GTA_money_bank] call life_fnc_numberText
		];
	};
	__SUB__(GTA_money_bank,_upgradePrice);
	...
	[[2,grpPlayer],"TON_fnc_updateGang",false,false] call GTA_fnc_remoteExec;
} else {
