...
hint "WARNING\n\nDo not attempt to store a weapon directly from your backpack into your base, this will delete the weapon";
...
_vehData = _vehicle getVariable ["weapons",[[],0]];
...
if(_storedItems isEqualTo []) then {
	DS_bootVeh setVariable["weapons",[[],0],true];
...
_playersItems = [];
if((!(primaryWeapon player isEqualTo ""))&&(((currentWeapon player) isEqualTo (primaryWeapon player))))then{_playersItems pushBack (primaryWeapon player)};
if((!(secondaryWeapon player isEqualTo ""))&&(((currentWeapon player) isEqualTo (secondaryWeapon player))))then{_playersItems pushBack (secondaryWeapon player)};
if((!(handgunWeapon player isEqualTo ""))&&(((currentWeapon player) isEqualTo (handGunWeapon player))))then{_playersItems pushBack (handgunWeapon player)};
...
_btnStore buttonSetAction "[] spawn DS_civ_clanItemStoreWeapons;";
_btnTake buttonSetAction "[] spawn DS_civ_clanItemTakeWeapons;";
...
_vehicle spawn {
	...
	waitUntil {isNull (findDisplay 666668)};
	DS_waitToSync = false;
	[_this,1,(name player)] remoteExec ["HUNT_fnc_updateClanStorage",2];
