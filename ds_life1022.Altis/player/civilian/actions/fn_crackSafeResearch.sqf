...
if(!(knox_safe1 getVariable ["closed",false]))exitwith{hint "You can not crack this safe while the Fort Knox safe is being/been cracked"};
if(!(knox_safe2 getVariable ["closed",false]))exitwith{hint "You can not crack this safe while the Fort Knox safe is being/been cracked"};
if(knox_safe1 getVariable ["cracking",false])exitwith{hint "You can not crack this safe while the Fort Knox safe is being/been cracked"};
if(knox_safe2 getVariable ["cracking",false])exitwith{hint "You can not crack this safe while the Fort Knox safe is being/been cracked"};
...
if(!(safe_research getVariable ["closed",false]))exitwith{hint "This safe is already opened"};
if(safe_research getVariable ["cracking",false])exitwith{hint "This safe is already being cracked"};
...
if((DS_infoArray select 17) == 22)then { [2] call DS_fnc_questCompleted };
...
[(_infoArray select 3),(_infoArray select 0)] remoteExec ["HUNT_fnc_research",2];
DS_doneStuff pushBack [(name player),"used a cracker","Altis Research Facility",time];
