...
if(isNil "DS_houseBlock")then
	{
	DS_houseBlock = false;
	};
if(DS_houseBlock)exitWith
	{
	hint "You can not drop items at the moment because you have recently robbed something";
	};
...
if((!(playerSide isEqualTo west))&&([player,100,west] call DS_fnc_nearbyPlayers)) exitWith {titleText["You can not remove objects when police are nearby","PLAIN"]};
...
if(!isNull objectParent player)exitwith{hint "You can not remove items inside a vehicle"};
