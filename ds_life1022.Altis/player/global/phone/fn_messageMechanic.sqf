...
if(_receiver)then
	...
	if((side _from) isEqualTo civilian)then
		{
		hint parseText format ["<t color='#FFCC00' size='2' align='center'>Work Call<br/><br/><t color='#33CC33' align='left' size='1'>To: <t color='#ffffff'>Mechanics<br/><t color='#33CC33'>From: <t color='#ffffff'>%1<br/><t color='#33CC33'>Gang: <t color='#ffffff'>%2<br/><br/><t color='#33CC33'>Message:<br/><t color='#ffffff'>%3",(name _from),(group _from) getVariable ["gangName","No Gang"],_message];
		}
		else
		{
		hint parseText format ["<t color='#FFCC00' size='2' align='center'>Work Call<br/><br/><t color='#33CC33' align='left' size='1'>To: <t color='#ffffff'>Mechanics<br/><t color='#33CC33'>From: <t color='#ffffff'>%1<br/><br/><t color='#33CC33'>Message:<br/><t color='#ffffff'>%2",(name _from),_message];
		};
	player say3D "phoneCall";
	["mechanicCall",[format["Mechanic request from: %1",(name _from)]]] call BIS_fnc_showNotification;
	systemChat format ["Text message from (%1): %2",(name _from),_message];

	_log = format["Mechanic request from %1: %2",(name _from),_message];
	...
	_targets = allPlayers select {_x getVariable ["mechanic",false]};
	if(!(_targets isEqualTo []))then{[true,player,_message] remoteExec ["DS_fnc_messageMechanic",_targets];};
