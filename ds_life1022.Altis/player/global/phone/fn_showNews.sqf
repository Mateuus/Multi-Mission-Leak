...
_logText ctrlSetStructuredText parseText "<t color='#FFFFFF'>News Log</t>";

lbClear _log;

if(DS_newsLog isEqualTo []) then {
	_index = _log lbAdd "No news to display";
	_log lbSetData [_index,""];
} else {
	_counter = (count(DS_newsLog));
	while{!(_counter isEqualTo 0)}do {
		_index = _log lbAdd format ["%1",(DS_newsLog select ((_counter)-1))];
		_log lbSetData [_index,(DS_newsLog select ((_counter)-1))];
