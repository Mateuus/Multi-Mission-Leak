...
switch(DS_questFocus)do
	{
		case 0:{_progress = (DS_infoArray select 14)};
		case 1:{_progress = (DS_infoArray select 16)};
		case 2:{_progress = (DS_infoArray select 18)};
		case 3:
			{
			if(playerSide isEqualTo west)then
				{
				_progress = (DS_infoArray select 22);
				};
			if(player getVariable ["security",false])then
				{
				_progress = (DS_infoArray select 20);
				};
			if(player getVariable ["mechanic",false])then
				{
				_progress = (DS_infoArray select 24);
				};
			if(player getVariable ["medic",false])then
				{
				_progress = (DS_infoArray select 11);
				};
			};
	};

hint format ["Progress through current quest\n\n%1",_progress];