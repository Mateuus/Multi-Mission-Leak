...
switch(playerside)do
	...
	case west:
		...
		if(alive DS_cursorTarget)then
			...
			_btn3 ctrlSetTooltip "Search your target if they are restrained";
			_img4 ctrlSetText "extras\icons\escort.jpg";
			_btn4 ctrlSetTooltip "Un-escort your target or escort him if they are restrained";
			...
			_img9 ctrlSetText "extras\icons\blindfold.jpg";
			_btn9 ctrlSetTooltip "Blindfold/Un-blindfold this civilian (Only to be used if he is giving out information through voice)";
			...
			if((DS_cursorTarget getVariable ["restrained",false]))then
				{
			...
			_btn2 buttonSetAction "[player] remoteExec [""DS_fnc_checkLicenses"",DS_cursorTarget]";
			...
			if(!isNil "_mcuBoat")then
				{
				if(_mcuBoat getVariable ["isMCUWater",false])then
					{
					_Btn6 ctrlEnable true;
					_Btn6 buttonSetAction "[DS_cursorTarget] spawn DS_cop_jailMenu;";
					};
			...
			if(DS_cursorTarget getVariable ["restrained",false])then
				...
				_Btn7 buttonSetAction "[DS_cursorTarget] spawn DS_fnc_putInVeh;";
				};
			_Btn8 buttonSetAction "[DS_cursorTarget] spawn DS_cop_checkCharges;";
			if(DS_cursorTarget getVariable ["restrained",false])then
				{
				_btn9 ctrlEnable true;
				_Btn9 buttonSetAction "[DS_cursorTarget] spawn DS_cop_blindfoldCiv;";
				};
			}
			else
			{
	...
	case civilian:
		{
			if(!alive DS_cursorTarget)then
				...
				if((DSL_rebelAdv)||(DS_realMode))then
					{
					_btn1 ctrlEnable true;
					_btn1 buttonSetAction "[DS_cursorTarget] spawn DS_fnc_revive;";
					_btn2 ctrlEnable true;
				...
				if(DS_cursorTarget getVariable ["restrained",false])then
					{
					_btn3 ctrlEnable true;
					_Btn3 buttonSetAction "[DS_cursorTarget] spawn DS_fnc_putInVeh; closeDialog 0;";
					};
				if(DS_cursorTarget getVariable ["escorted",false])then
					{
					_btn2 ctrlEnable true;
				...
				if((player distance (getMarkerPos "hostage1") < 10)||(player distance (getMarkerPos "hostage2") < 10)||(player distance (getMarkerPos "hostage3") < 10))then
					{
					_btn4 ctrlEnable true;
	...
	case independent:
		{
			...
			if(player getVariable ["security",false])then
				...
				if(alive DS_cursorTarget)then
					...
					_btn1 ctrlSetTooltip "Remove or place your target in handcuffs if their hands are up or they are tazed";
					_img2 ctrlSetText "extras\icons\escort.jpg";
					_btn2 ctrlSetTooltip "Un-escort your target or escort him if they are restrained";
					...
					_btn7 ctrlSetTooltip "Send a highly wanted criminal to the local Police HQ";
					_img8 ctrlSetText "extras\icons\requestMoney.jpg";
					_btn8 ctrlSetTooltip "Charge for services";
					_btn8 ctrlEnable true;
					_btn8 buttonSetAction "[DS_cursorTarget] spawn DS_sec_askForPayment;";
					...
					if((player distance (getMarkerPos "secSpawn1") < 100)||(player distance (getMarkerPos "secSpawn2") < 100)||(player distance (getMarkerPos "secSpawn3") < 100))then
						{
						_btn4 ctrlEnable true;
					...
					if(DS_cursorTarget getVariable ["restrained",false])then
						...
						_Btn5 buttonSetAction "[DS_cursorTarget] spawn DS_fnc_putInVeh;";
						};
