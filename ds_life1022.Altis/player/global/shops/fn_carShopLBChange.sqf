...
//DS_price = 0;
//Vehicle Shops
if(DS_shopIndex in [0,1,2,3,30,31,32,33,34,35,61,62,36,83,95,96,120,123,127,148])then
	{
	_noLicense = [DS_shopIndex] call DS_fnc_checkLicense;
	_price = [_className,DS_shopIndex,3] call DS_fnc_getPrice;
	if(playerSide isEqualTo west)then{_price = [_price] call DS_fnc_policeDiscount;};
	...
	if(_noLicense != "")then
		...
		_btn1 ctrlSetStructuredText parseText format["<t size='0.85'>Rent $%1</t>",[_price] call DS_fnc_numberText];
		_btn1 buttonSetAction "[false] spawn DS_fnc_buyVehicle;";
		_btn2 ctrlSetStructuredText parseText format["<t size='0.85'>Purchase $%1</t>",[_price*1.5] call DS_fnc_numberText];
...
//Weapons Shops
if(DS_shopIndex in [5,6,7,14,15,16,17,18,19,20,21,57,59,60,63,121,122,156,164])then
	{
	...
	if(_noLicense != "")then
		...
		if(_price <= DS_coin)then
			...
			_btn3 ctrlSetStructuredText parseText format["<t size='0.85'>Purchase ($%1)</t>",[_price] call DS_fnc_numberText];
			DS_className = _className;
			DS_price = _price;
			}
			else
...
//Weapons Shops Token
if(DS_shopIndex in [158,161])then
	{
	_price = [_className,DS_shopIndex,0] call DS_fnc_getPrice;
	_noLicense = [DS_shopIndex] call DS_fnc_checkLicense;
	ctrlShow[66654,true];
	if(_noLicense != "")then//set quest level thingy here
		{
		ctrlEnable [66654,false];
		_btn3 ctrlSetText "Unavailable";
		hint format ["%1",_noLicense];
		...
		switch(DS_perkLevel)do
			{
			case 1:{_price = (round(_price*1.05))};
			case 2:{_price = (round(_price*1.1))};
			case 3:{_price = (round(_price*1.15))};
			case 4:{_price = (round(_price*1.2))};
			case 5:{_price = (round(_price*1.25))};
			default {_price = _price};
			};
		if(_price <= (DS_realEstateArray select 6))then
			{
			ctrlEnable [66654,true];
			_btn3 ctrlSetStructuredText parseText format["<t size='0.85'>Purchase (%1 Tokens)</t>",[_price] call DS_fnc_numberText];
			DS_className = _className;
			DS_price = _price;
			}
			else
			...
			_btn3 buttonSetAction "[true] call DS_fnc_buyWeapon;";
		};
	};
//License Shops
if(DS_shopIndex in [8,39,52,58,93])then
	...
	if(DS_coin >= _price)then
		...
		if(_price == 999)then
			{
			ctrlEnable [66654,true];
			_btn3 ctrlSetText format ["Take Test",[_price] call DS_fnc_numberText];
			}
			else
			{
			_btn3 ctrlSetText format ["Purchase $%1",[_price] call DS_fnc_numberText];
...
//Clothing Shops
if(DS_shopIndex in [9,10,11,12,13,73,74,75,76,77,124,147,157,165])then
	{
...
if(DS_shopIndex in [54])then
	{
	if(!([] call DS_civ_myHideout))exitwith{hint "You must own this gang hideout to use this shop";DS_doingStuff = false;};
	ctrlShow[66654,true];
	ctrlShow[777,true];
	ctrlShow[778,true];
	_index = [_className] call DS_fnc_itemIndex;
...
if(DS_shopIndex in [55,84,86])then
	...
	{
		if(count(nearestObjects[(getMarkerPos (_spawn select _spawnIndex)),["Car","Ship","Air","Tank"],8]) > 0)then{_spawnIndex = _spawnIndex + 1};
	}forEach DS_spawnPoint;
	if(_spawnIndex >= _SpawnMax)exitwith{hint "There is no spawn point free at this time"};
	_tempSpawn = (_spawn select _spawnIndex);

	if(isNil "_vehicle") exitWith {hint "No vehicle selected"};
	if(count(nearestObjects[(getMarkerPos _tempSpawn),["Car","Ship","Air","Tank"],8]) > 0)exitwith{hint "There is a vehicle blocking this spawn point";DS_doingStuff = false;};
	ctrlShow[66654,true];
	ctrlShow[778,true];
	...
	//[player,_query,0,_vehID] remoteExec ["HUNT_fnc_returnVehicleList",2];
...
if(DS_shopIndex in [149])then
	...
	_btn3 ctrlSetText "Retrieve Vehicle";
	_btn3 buttonSetAction "[true] spawn DS_fnc_retrieveVehicleHouse;";
	};
...
if(DS_shopIndex in [103,166])then
	{
	_price = [_className,DS_shopIndex,3] call DS_fnc_getPrice;
	if(DS_coin < _price)then{_btn1 ctrlEnable false}else{_btn1 ctrlEnable true};
	ctrlShow[66654,false];
...
//Rebel Tokens
if(DS_shopIndex in [130,162])then
	{
	DS_className = _className;
	ctrlShow[66654,true];
	_btn3 ctrlSetText "Purchase";
