...
if(!(_vehicle getVariable ["policeVehicle",false])) exitWith {hint "You must have a police vehicle nearby to put the body into"};
	...
	_upp = format ["Removing %1's body",_victim getVariable ["name","Random guy"]];
	...
	if(player distance _victim > 10)exitwith {};
	if(_vehicle distance _victim > 10)exitwith {};
	titleText [format["Success, you have moved %1's body into your vehicle",_victim getVariable ["name","This random guy"]],"PLAIN"];
	hint format ["Success, you have moved %1's body into your vehicle",_victim getVariable ["name","This random guy"]];
	_victim setVariable ["revive",false,true];
	hideBody _victim;
	[_victim] remoteExec ["DS_fnc_clientCleanup",0];
