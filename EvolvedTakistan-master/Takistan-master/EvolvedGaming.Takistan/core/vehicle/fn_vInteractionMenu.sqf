...
if(playerSide == west) then {_Btn1 ctrlEnable true;} else {_Btn1 ctrlEnable true;};
...
if(playerSide == west) then {
	...
	switch(true) do {
		...
		default {_Btn3 ctrlShow false;};
	
	};
	
	/*
	if(typeOf _curTarget in ["O_Truck_03_device_F"]) then {
		_Btn3 ctrlSetText localize "STR_vInAct_DeviceMine";
		_Btn3 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_deviceMine";
		if(!isNil {(_curTarget getVariable "mining")} OR !local _curTarget && {_curTarget in life_vehicles}) then {
			_Btn3 ctrlEnable false;
	...
	if(typeOf _curTarget in ["O_Truck_02_fuel_F","O_Truck_03_fuel_F","B_Truck_01_fuel_F"]) then {
		_Btn3 ctrlSetText localize "STR_vInAct_DeviceMine";
		_Btn3 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_deviceOil";
		if(!isNil {(_curTarget getVariable "mining")} OR !local _curTarget && {_curTarget in life_vehicles}) then {
			_Btn3 ctrlEnable false;
