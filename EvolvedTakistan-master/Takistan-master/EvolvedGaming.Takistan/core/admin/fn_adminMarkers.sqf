...
if(life_markers) then {
	...
	if(FETCH_CONST(life_adminlevel) < 5) then {[[0,format["%1 has turned on player markers.",str profileName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;};
...
FinishedLoop = true;
	...
	if(FETCH_CONST(life_adminlevel) < 5) then {[[0,format["%1 has turned off player markers.",str profileName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;};
