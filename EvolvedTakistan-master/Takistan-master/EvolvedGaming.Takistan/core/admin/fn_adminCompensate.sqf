...
if(_action) then {
	...
	if(FETCH_CONST(life_adminlevel) < 5) then {[[0,format["%1 has compensated a player $%2.",str profileName,[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;};
	closeDialog 0;
} else {
