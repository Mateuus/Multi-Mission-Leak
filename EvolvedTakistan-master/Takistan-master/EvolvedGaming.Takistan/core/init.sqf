...
A3L_TFEnabled = true;
A3L_onTsServer = "Evolved Gaming - Evolvedgamingclan.com" == (call TFAR_fnc_getTeamSpeakServerName);
...
[] spawn {
	...
	while {true} do {
				...
				if (!(_isadmin)) then {
					...
					_onTsServer = "Evolved Gaming - Evolvedgamingclan.com" == (call TFAR_fnc_getTeamSpeakServerName);
					if (!(_onTsServer)) then {
						titleText ["Please join ts3.evolvedga.me teamspeak server!", "BLACK"];
