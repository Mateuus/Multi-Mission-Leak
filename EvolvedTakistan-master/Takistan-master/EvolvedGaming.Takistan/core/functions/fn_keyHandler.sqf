...
switch (_code) do {
	...
	case 19:
	{
		...
		if(_shift && playerSide == west && !isNull cursorTarget && cursorTarget isKindOf "Man" && !(player GVAR["tied",false]) && (isPlayer cursorTarget) && (side cursorTarget == civilian) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget GVAR "Escorting") && !(cursorTarget GVAR "restrained") && speed cursorTarget < 1) then
		{
	...
	case 34:
	{
		...
		{
			if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == RIFLE OR currentWeapon player == PISTOL) && currentWeapon player != "" && !life_knockout && !(player GVAR["restrained",false]) && !(player GVAR["tied",false]) && !life_istazed && !(player GVAR["surrender",false])) then
			{
