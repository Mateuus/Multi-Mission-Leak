...
switch(playerSide) do {
	...
	case civilian: {
		...
		civ_position = SEL(_this,11);
		life_civ_position = civ_position;
		life_EXPLevel = SEL(_this,12);
		life_EXPCount = SEL(_this,13);
		life_EXPKills = SEL(_this,14);
		life_EXPDeaths = SEL(_this,15);
		life_EXPZombiekills = SEL(_this,16);
		life_civTime = SEL(_this,17);
		life_houses = SEL(_this,18);
		if(EQUAL(LIFE_SETTINGS(getNumber,"save_playerStats"),1)) then {
...
if(count (SEL(_this,20)) > 0) then {
	{life_vehicles pushBack _x;} foreach (SEL(_this,20));
};
