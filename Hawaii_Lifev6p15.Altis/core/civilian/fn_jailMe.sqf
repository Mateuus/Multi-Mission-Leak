...
if(count _ret > 0) then { life_bail_amount = SEL(_ret,3); } else { life_bail_amount = 15000;};
_esc = false;
_bail = false;
...
while {true} do {
	...
	if(player distance (getMarkerPos "jail_marker") > 65) then {
		if(vehicle player == player) then {
			player setPos (getMarkerPos "jail_marker");
			0 cutText["Wie kommst du da denn ohne Fahrzeug raus?","PLAIN"];
		};
	};
