...
switch (_code) do {
	...
	case 19: {
		...
		if(_shift) then {
			if(playerSide == west OR playerSide == civilian) then {
	...
	case _speakKey:
	{
		if(getPlayerChannel player == 7) then {
			setCurrentChannel 5;
			systemchat "Bitte nicht im SideChat Reden!";
		};
		if(getPlayerChannel player == 9) then {
			setCurrentChannel 5;
			systemchat "Bitte nicht im Civ Cop THW Chat Reden!";
		};
		if(getPlayerChannel player == 10) then {
			setCurrentChannel 5;
			systemchat "Bitte nicht im Cop THW Chat Reden!";
		};
	};
	...
	case 38: {
		...
		if(_shift) then {
			if(playerSide in [west,independent]) then {
				if(vehicle player != player) then {
					if((typeOf vehicle player) in ["B_Truck_01_transport_F","C_Offroad_01_repair_F","C_Hatchback_01_F","B_Quadbike_01_F","C_Van_01_box_F","B_Truck_01_mover_F","C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","B_MRAP_01_hmg_F","I_MRAP_03_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","O_Heli_Light_02_unarmed_F","B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F","I_Truck_02_box_F"]) then {
