...
[0,"STR_NOTF_BeingDeleted",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
...
if(EQUAL(count crew _vehicle,0)) then {
	...
	[0,"STR_NOTF_HasDeleted",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
	ADD(BANK,_price);
} else {
