...
if ((lbCurSel 2703) isEqualTo -1) exitWith {
    [localize "STR_ATM_NoneSelected",true,"fast"] call life_fnc_notification_system;
};

if (isNil "_unit") exitWith {
    [localize "STR_ATM_DoesntExist",true,"fast"] call life_fnc_notification_system;
};
...
if (_value > BANK) exitWith {
    [localize "STR_ATM_NotEnoughFunds",true,"fast"] call life_fnc_notification_system;
};

_tax = _value * LIFE_SETTINGS(getNumber,"bank_transferTax");
if ((_value + _tax) > BANK) exitWith {
    //[format [localize "STR_ATM_SentMoneyFail",[_value],_tax],true,"fast"] call life_fnc_notification_system;
    format  [localize "STR_ATM_SentMoneySuccess",[_value] call life_fnc_numberText,_unit getVariable ["realname",name _unit],[_tax] call life_fnc_numberText];
};
