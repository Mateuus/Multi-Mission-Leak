...
private["_maxGather","_resource","_amount","_maxGather","_requiredItem","_cpRate"];
...
*/
    ...
    sleep 0.1;
	//_cP = _cP + _cpRate;
    _cP = _cP + (0.01 * (missionNamespace getVariable ["mav_ttm_var_farmspeedMultiplier", 1])); //Leveling System
...
If (speedcheck> 10) exitWith{ 
    ["BugUser", FALSE, TRUE] call BIS_fnc_endMission;
    Sleep 3;
    [[1, format ["% 1 just tried to exploit the farm bug!", Nameplayer]], "life_fnc_broadcast", true, false] spawn life_fnc_MP;
};
...
//-- Adding XP
["Gathering"] spawn mav_ttm_fnc_addExp;
