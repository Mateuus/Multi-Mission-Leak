...
if (playerSide isEqualTo west && _illegal isEqualTo 1) exitWith {
    [format[localize "STR_NOTF_PickedEvidence",_itemName,[round(ITEM_SELLPRICE(_itemInfo select 0) / 2)] call life_fnc_numberText] ,false,"fast"] call life_fnc_notification_system;
...
if (!(_diff isEqualTo (_itemInfo select 1))) then {
    if ([true,(_itemInfo select 0),_diff] call life_fnc_handleInv) then {
        ...
        [format[localize "STR_NOTF_Picked",_diff,localize _itemName],false,"fast"] call life_fnc_notification_system;
        INUSE(_this);
    } else {
        INUSE(_this);
    };
