...
_cashStored = 50000 + round(random 60000); //The amount the shop has to rob, you could make this a parameter of the call 
_action = [_this,2] call BIS_fnc_param;//Action name
_name = [_this,3] call BIS_fnc_param;//Shop Name  

//-- More Money from gas stations
if ([life_currentExpPerks, "thug"] call mav_ttm_fnc_hasPerk) then {
    _cashStored = _cashStored * 1.5;
};

//Checks
if({side _x == west} count playableUnits < 3) exitWith {hint "There are not enough cops online for you to rob this! (3 Minimum)"}; 
...
//Var checks
if(_shop getVariable ["rob_empty",false]) exitWith {hint "This shop was robbed recently and cannot be robbed again for a while!";};
if(_shop getVariable ["rob_ip",false]) exitWith {hint "This shop has already being robbed!";};
...
_progress progressSetPosition 0.015;
_cP = 0.01; 
...
if(_cP >= 1) then {
	...
	//Gain XP from robbery
	["Shoprobbing"] spawn mav_ttm_fnc_addExp;
	life_use_atm = false;	
	sleep (30 + random(180));
	life_use_atm = true;
	if!(alive _robber) exitWith {};
