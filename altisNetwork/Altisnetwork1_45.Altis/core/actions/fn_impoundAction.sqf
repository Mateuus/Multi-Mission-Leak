...
for "_i" from 0 to 1 step 0 do {
    sleep 0.09;
    //_cP = _cP + 0.01;
    _cP = _cP + (0.01 * (missionNamespace getVariable ["mav_ttm_var_copimpoundingMultiplier", 1])); //Level System
...
if (count crew _vehicle isEqualTo 0) then {
    ...
    if (playerSide isEqualTo west) then {
            ...
            if (_vehicle in life_vehicles) then {
                [format[localize "STR_NOTF_Impounded",[_value] call life_fnc_numberText,_type],false,"fast"] call life_fnc_notification_system;
                BANK = BANK + _value;
            };
...
["CopImpounding"] spawn mav_ttm_fnc_addExp;