...
*/
private["_vendor","_type","_itemInfo","_oldItem","_newItemWeight","_newItem","_oldItemWeight","_cost","_upp","_hasLicense","_itemName","_oldVal","_ui","_progress","_pgText","_cP","_materialsRequired","_materialsGiven","_noLicenseCost","_text","_filter","_totalConversions","_minimumConversions","_cpRate"];
...
/*
if ([life_currentExpPerks, "Processor"] call mav_ttm_fnc_hasPerk) then {
        _cpRate = 0.10; //-- 0.01
	} else {
        _cpRate = 0.010; //--0.0115
*/
...
if (_hasLicense) then {
    for "_i" from 0 to 1 step 0 do {
        sleep  0.28;
        //_cP = _cP + _cpRate
        _cP = _cP + (0.01 * (missionNamespace getVariable ["mav_ttm_var_processMultiplier", 1])); //Leveling System
    ...
    if (player distance _vendor > 10) exitWith {
        [localize "STR_Process_Stay",true,"fast"] call life_fnc_notification_system; life_is_processing = false; life_action_inUse = false;
    };

    if((vehicle player) != player) exitWith {
        ["You can not get into a vehicle while you process something!",true,"fast"] call life_fnc_notification_system; 
        _id cutText ["","PLAIN"]; 
    ...
    if (_minimumConversions isEqualTo (_totalConversions call BIS_fnc_lowestNum)) then {
        [localize "STR_NOTF_ItemProcess",true,"fast"] call life_fnc_notification_system;
    } else {
        [localize "STR_Process_Partial",true,"fast"] call life_fnc_notification_system;
        };

    ["ItemProcessed"] spawn mav_ttm_fnc_addExp; //Leveling System
    
    life_is_processing = false; life_action_inUse = false;
} else {
    if (CASH < _cost) exitWith {
        [localize "STR_Process_License",[_cost] call life_fnc_numberText],true,"fast"] call life_fnc_notification_system; life_is_processing = false; life_action_inUse = false;
    };
    ...
    if (player distance _vendor > 10) exitWith {
        [localize "STR_Process_Stay",true,"fast"] call life_fnc_notification_system; 
        life_is_processing = false; 
        life_action_inUse = false;
    };
    if (CASH < _cost) exitWith {
        [format[localize "STR_Process_License",[_cost] call life_fnc_numberText],true,"fast"] call life_fnc_notification_system; 
        life_is_processing = false; 
        life_action_inUse = false;
    };
