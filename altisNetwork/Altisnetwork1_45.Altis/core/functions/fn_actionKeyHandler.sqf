...
if((currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && animationState cursorObject == "Incapacitated" && !dialog && playerSide isEqualTo civilian) then {
    [] call life_fnc_civInteractionMenuZip;
};
...
//Fed Bank
//-- Main Building : Land_MilOffices_V1_F : 6068.969,12603.45,0.391
//-- Safe Building : Land_Medevac_house_V1_F : 6061.714,12590.1,0.801
if(_curObject isKindOf "House_F" && {player distance _curObject < 12} || ((nearestObject [[6068.969,12603.45,0.391],"Land_MilOffices_V1_F"]) == _curObject || (nearestObject [[6061.714,12590.1,0.801],"Land_Medevac_house_V1_F"]) == _curObject)) exitWith {
...
if (_curObject isKindOf "Man" && !(_curObject isKindOf "Animal") && {!alive _curObject} && !(_curObject getVariable ["Revive",false]) && {playerSide in [west,independent]}) exitWith {
    //Hotfix code by ins0
    if (((playerSide isEqualTo west && {(LIFE_SETTINGS(getNumber,"revive_cops") isEqualTo 1)}) || playerSide isEqualTo independent)) then {
    if (life_inv_defibrillator > 0 && {side _x == independent} count playableUnits < 1) then {
        hint "You are unable to Revive if their are medics online!"
    } else {
        [_curObject] call life_fnc_revivePlayer;
        };
    };
...
if ((!(_curObject getVariable ["restrained",false])) && !dialog && playerSide isEqualTo civilian) then {
    ...
    if(_isVehicle) then {
		if(!dialog) then {
			if(playerSide == west && player distance _curObject < (((boundingBox _curObject select 1) select 0) + 2) && (!(player getVariable ["restrained",false])) && (!(player getVariable ["playerSurrender",false])) && !life_isknocked && !life_istazed) then {
				[_curObject] call life_fnc_vInteractionMenu;
			};
			if(playerSide == civilian && player distance _curObject < (((boundingBox _curObject select 1) select 0) + 2) && (!(player getVariable ["restrained",false])) && (!(player getVariable ["playerSurrender",false])) && !life_isknocked && !life_istazed) then {
			    [_curObject] call life_fnc_civVInteractionMenu;
			};
			if(playerSide == independent && player distance _curObject < (((boundingBox _curObject select 1) select 0) + 2) && (!(player getVariable ["restrained",false])) && (!(player getVariable ["playerSurrender",false])) && !life_isknocked && !life_istazed) then {
			    [_curObject] call life_fnc_medVInteractionMenu;
			};
		};
	} else {
