...
if (!(count (actionKeys "User10") isEqualTo 0) && {(inputAction "User10" > 0)}) exitWith {
    ...
    true;
       ...
       // O, police gate opener        case 24:	{		if (!_shift && !_alt && !_ctrlKey && (playerSide == west) && (vehicle player != player)) then {			[] call life_fnc_copOpener;		};	};
...
/*
    //F5 Runing the Undercover Script
	case 63: {
		if ((license_cop_UC) && (playerside == west)) then
		{
			[] spawn life_fnc_undercover;
			_handled = true;
		};
	};
*/
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","C_Hatchback_01_F","B_Heli_Light_01_F","B_Heli_Transport_01_F"]) then {
	...
	case 24: {
		...
		if(_shift && (playerSide isEqualTo west)) then {
...
//Open placeables using C
    case 46	: {  
            ...
            hint "You have placed the object!";  
   	};
*/
