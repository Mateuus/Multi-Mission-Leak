...
while {true} do {
	...
	sleep .15;
	_cP = _cP + 0.01;
	//_cP = _cP + (0.01 * (missionNamespace getVariable ["mav_ttm_var_cprMultiplier", 1]));
...
if(_target getVariable ["Revive",FALSE]) exitWith {[localize "STR_Medic_RevivedRespawned",5,"gray"] call RPG_Fnc_Msg;};
...
if (_break < 85) exitWith {[] spawn life_fnc_cprkit;};

//[player,"defib"] call life_fnc_globalSound;
[_target,"defib"] spawn life_fnc_nearestSound;
