...
if (((nearestObject [[6068.969,12603.45,0.391],"Land_MilOffices_V1_F"]) == _building || (nearestObject [[6061.714,12590.1,0.801],"Land_Medevac_house_V1_F"]) == _building) && (west countSide playableUnits < (LIFE_SETTINGS(getNumber,"minimum_cops")))) exitWith {
    hint format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"minimum_cops"))]
};
if ((typeOf _building) == "Land_Medevac_house_V1_F" && (nearestObject [[6068.969,12603.45,0.391],"Land_MilOffices_V1_F"]) getVariable ["locked",true]) exitWith {hint localize "STR_ISTR_Bolt_Exploit"};
...
if ((nearestObject [[6068.969,12603.45,0.391],"Land_MilOffices_V1_F"]) == _building || (nearestObject [[6061.714,12590.1,0.801],"Land_Medevac_house_V1_F"]) == _building) then {
    [[1,2],"STR_ISTR_Bolt_AlertFed",true,[]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
    [1,format["DISPATCH: ALTIS GOLD RESERVE IS BEING BROKEN INTO!",_storename]] remoteExecCall ["life_fnc_broadcast", west];
    [0,format["DISPATCH: ALTIS GOLD RESERVE IS BEING BROKEN INTO!",_storename]] remoteExec ["life_fnc_broadcast",west];
    ["dbank", false] remoteExec ["life_fnc_dispatchbank",west];
} else {
...
switch (typeOf _building) do {
    case "Land_MilOffices_V1_F": {_cpRate = 0.003;}; //-- Main Building - 0.001
    case "Land_Medevac_house_V1_F": {_cpRate = 0.003;};//-- Safe Building - 0.002
	case "House_F": {_cpRate = 0.003;};
    //default {_cpRate = 1.50;} //0.08
};
...
for "_i" from 0 to 1 step 0 do {
	...
	if (life_isdowned) exitWith {}; //Rubbered
};
