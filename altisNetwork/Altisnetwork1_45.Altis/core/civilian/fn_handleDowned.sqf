...
if(_shooter isKindOf "Man" && alive player) then {
	if(!life_isdowned) then {
		...
		[player,"AcinPercMrunSrasWrflDf_agony"] remoteExec ["life_fnc_animSync",RANY];
		[0,format["%1 was knocked out by Rubber Bullets!", name _unit, name _shooter]] remoteExec ["life_fnc_broadcast",RANY];
		_unit attachTo [_obj,[0,0,0]];
		disableUserInput true;
		sleep 8;
		if(!(player getVariable "restrained")) then {
		[player,"amovppnemstpsraswrfldnon"] remoteExec ["life_fnc_animSync",RANY];
			sleep 10;
		};
