...
if (count crew _vehicle isEqualTo 0) then {
	...
	if (playerSide isEqualTo independent) then {
            ...
            [0,"STR_NOTF_HasImpounded_med",true,[profileName,((_vehicleData select 0) select 1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
            if (_vehicle in life_vehicles) then {
                hint format [localize "STR_NOTF_OwnImpounded_med",[_value] call life_fnc_numberText,_type];
                BANK = BANK - _value;
            } else {
