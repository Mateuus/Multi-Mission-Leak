            ...
            if (count _sPos > 1 && {_distance < 15}) then {
                _text = switch (true) do {
                    ...
                    case (side _x isEqualTo west && {!isNil {_x getVariable "rank"}}): {format ["<img image='%1' size='1'></img> %3 %2",switch ((_x getVariable "rank")) do {
                        ...
                        case 2: {"[Wachtmeister]"};
                        case 3: {"[Oberwachmeister]"};
                        case 4: {"[Hauptwachmeister]"};
                        case 5: {"[Kommissar]"};
                        case 6: {"[Oberkomissar]"};
                        case 7: {"[Hauptkommissar]"};
                        case 8: {"[Polizeidirektor]"};
                        default {"[Ordnungsbeamter]"};
                        }]};

                    case (side _x isEqualTo independent && {!isNil {_x getVariable "rank"}}): {format ["<t color='#FF0000'><img image='%1' size='1.5'></img></t> %3 %2",switch ((_x getVariable "rank")) do {
                        ...
                        case 10: {"a3\ui_f\data\map\MapControl\hospital_ca.paa"};
                        default {"a3\ui_f\data\map\MapControl\hospital_ca.paa"};
                        },_x getVariable ["realname",name _x],switch ((_x getVariable "rank")) do {
                        case 2: {"[Feuerwehrmann]"};
                        case 3: {"[Oberfeuerwehrmann]"};
                        case 4: {"[Hauptfeuerwehrmann]"};
                        case 5: {"[Löschmeister]"};
                        case 6: {"[Oberlöschmeister]"};
                        case 7: {"[Hauptlöschmeister]"};
                        case 8: {"[Brandmeister]"};
                        case 9: {"[Oberbrandmeister]"};
                        case 10: {"[Hauptbrandmeister]"};
                        default {"[Feuerwehranwärter]"};
                        }]};
