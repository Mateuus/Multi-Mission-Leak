...
switch (_code) do {
        ...
        if (_shift && playerSide isEqualTo civilian && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
            if ((animationState cursorObject) != "Incapacitated" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable ["restrained",false]) && !life_istazed && !life_isknocked) then {
                ...
                if (life_HC_isActive) then {
                    [getPlayerUID _who,_who getVariable ["realname",name _who],"28"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
                } else {
                    [getPlayerUID _who,_who getVariable ["realname",name _who],"28"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
                };
                */
            ...
            if("ItemRadio" in assignedItems cursorTarget) then {
				...
				hint "Das Smartphone der Person liegt auf dem Boden.";
				_defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);
				} else { hint "Die Person hat kein Smartphone mehr!"};
     		//ADD THIS
            ...
            if (!(isNull objectParent player) && alive vehicle player) then {
                ...
                if (!(isNil "_list")) then {
                    ...
                    if (KINDOF_ARRAY(cursorObject,_list) && {player distance cursorObject < 7} && {isNull objectParent player} && {alive cursorObject} && {!life_action_inUse}) then {
                      ...
                      if (cursorTarget isKindOf "Land_Wreck_Traw_F" || cursorTarget isKindOf "Land_Wreck_Traw2_F") then {
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (!(isNull objectParent player) && (typeOf vehicle player) in ["C_Offroad_01_F","C_Hatchback_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_MRAP_01_F","I_MRAP_03_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F"]) then {
    ...
    case 39: {
        if (playerSide in [west] && {vehicle player != player} && {!life_siren2_active} && {((driver vehicle player) == player)}) then {
            ...
            _veh = vehicle player;
            if (isNil {_veh getVariable "warning"}) then {_veh setVariable ["warning",false,true];};
            if ((_veh getVariable "warning")) then {
                titleText [localize "STR_MISC_WarnOFF","PLAIN"];
                _veh setVariable ["warning",false,true];
            } else {
                titleText [localize "STR_MISC_WarnON","PLAIN"];
                _veh setVariable ["warning",true,true];
                if (playerSide isEqualTo west) then {
    ...
    case 24: {
     if(_shift) then {
         ...
         case 10: {hintsilent "Ear Plugs 80%"; 1 fadeSound 0.2; player setVariable ["Earplugs", 20]; };
         case 20: {hintsilent "Ear Plugs 70%"; 1 fadeSound 0.3; player setVariable ["Earplugs", 30]; };
			      case 30: {hintsilent "Ear Plugs 60%"; 1 fadeSound 0.4; player setVariable ["Earplugs", 40]; };
         case 40: {hintsilent "Ear Plugs 50%"; 1 fadeSound 0.5; player setVariable ["Earplugs", 50]; };
         case 50: {hintsilent "Ear Plugs 40%"; 1 fadeSound 0.6; player setVariable ["Earplugs", 60]; };
			      case 60: {hintsilent "Ear Plugs 30%"; 1 fadeSound 0.7; player setVariable ["Earplugs", 70]; };
         case 70: {hintsilent "Ear Plugs 20%"; 1 fadeSound 0.8; player setVariable ["Earplugs", 80]; };
         case 80: {hintsilent "Ear Plugs 10%"; 1 fadeSound 0.9; player setVariable ["Earplugs", 90]; };
			      case 90: {hintsilent "Ear Plugs Removed"; 1 fadeSound 1; player setVariable ["Earplugs", 0]; };
		    };
	   };
   };
	...
	if(_shift) then {_handled = true;};
		if ((_shift) && (vehicle player == player)) then
		{
			cutText [format["Liegestützen"], "PLAIN DOWN"];
	...
	if(_shift) then {_handled = true;};
		...
		{
			if(!life_action_inUse) then {
				cutText [format["Nicken"], "PLAIN DOWN"];
