...
if(life_inv_zipties < 1) exitWith {
	hintSilent "Du brauchst Kabelbinder um dies zu tun!!!";
};
...
if(!((_unit getVariable ["playerSurrender",false]) || (_unit getVariable ["knocked",false]))) exitWith {
	hintSilent "Die Person muss erst niedergeschlagen sein oder sich ergeben!!!";
};
