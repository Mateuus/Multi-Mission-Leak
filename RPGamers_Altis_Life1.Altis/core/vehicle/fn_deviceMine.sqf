...
if (!isNil {_vehicle getVariable "mining"}) exitWith {
    [localize "STR_NOTF_DeviceIsMining",5,"gray"] call RPG_Fnc_Msg;
};

if (fuel _vehicle isEqualTo 0) exitWith {
    [localize "STR_NOTF_OutOfFuel",5,"gray"] call RPG_Fnc_Msg;
};
...
if ((_weight select 1) >= (_weight select 0)) exitWith {
    [localize "STR_NOTF_DeviceFull",5,"gray"] call RPG_Fnc_Msg;
...
if (_zone isEqualTo "") exitWith {
    [localize "STR_NOTF_notNearResource",5,"gray"] call RPG_Fnc_Msg;
    life_action_inUse = false;
};
...
for "_i" from 0 to 1 step 0 do {
    ...
    if ((isEngineOn _vehicle) || ((speed _vehicle) > 5)) exitWith {
        [localize "STR_NOTF_MiningStopped",5,"gray"] call RPG_Fnc_Msg;
    };
    ...
    [localize "STR_NOTF_DeviceMining",5,"blue"] call RPG_Fnc_Msg;
    _time = time + 27;
    ...
    [format [localize "STR_NOTF_DeviceMined",_sum,(localize _itemName)],5,"blue"] call RPG_Fnc_Msg;
