...
if ((_vehicle getVariable ["trunk_in_use",false])) exitWith {[localize "STR_MISC_VehInvUse",5,"gray"] call RPG_Fnc_Msg;};
_vehicle setVariable ["trunk_in_use",true,true];
_vehicle setVariable ["trunk_in_use_by",player,true];
if (!createDialog "TrunkMenu") exitWith {[localize "STR_MISC_DialogError",5,"gray"] call RPG_Fnc_Msg;}; //Couldn't create the menu?
...
if (_veh_data select 0 isEqualTo -1) exitWith {closeDialog 0; _vehicle setVariable ["trunk_in_use",false,true]; [localize "STR_MISC_NoStorageVeh",5,"gray"] call RPG_Fnc_Msg;};
...
_vehicle spawn {
    ...
    [_this,"trunk_open"] spawn life_fnc_nearestSound;

    waitUntil {isNull (findDisplay 3500)};

    _this setVariable ["trunk_in_use",false,true];

    [_this,"trunk_close"] spawn life_fnc_nearestSound;
