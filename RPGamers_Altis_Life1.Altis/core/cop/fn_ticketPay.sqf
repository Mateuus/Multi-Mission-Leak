...
if (CASH < life_ticket_val) exitWith {
    if (BANK < life_ticket_val) exitWith {
        [localize "STR_Cop_Ticket_NotEnough",5,"gray"] call RPG_Fnc_Msg;
        [1,"STR_Cop_Ticket_NotEnoughNOTF",true,[profileName]] remoteExecCall ["life_fnc_broadcast",life_ticket_cop];
        closeDialog 0;
    };

    [format [localize "STR_Cop_Ticket_Paid",[life_ticket_val] call life_fnc_numberText],5,"blue"] call RPG_Fnc_Msg;
...
[life_ticket_cop, life_ticket_val] spawn life_fnc_copSplit;
[[2,format["$%1 has been spread amongst the active police force for a ticket paid by %2",[life_ticket_val] call life_fnc_numberText,name player]],"life_fnc_broadcast",life_ticket_cop,false] spawn BIS_fnc_MP;
