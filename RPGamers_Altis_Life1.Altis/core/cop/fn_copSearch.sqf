...
if (!alive _civ || player distance _civ > 5) exitWith {[format [localize "STR_Cop_CouldntSearch",_civ getVariable ["realname",name _civ]],5,"gray"] call RPG_Fnc_Msg;};
//hint format ["%1",_this];
...
,(_civ getVariable ["realname",name _civ]),_inv,if (_robber) then {"Matches description of robbery suspect."} else {""}];
