...
if (life_action_inUse) exitWith {[localize "STR_NOTF_ActionInProc",5,"gray"] call RPG_Fnc_Msg;};
if !(playerSide isEqualTo civilian) exitWith {[localize "STR_NOTF_notAllowed",5,"gray"] call RPG_Fnc_Msg;};
...
if (count _nearUnits > 1) exitWith {[localize "STR_NOTF_PlayerNear",5,"gray"] call RPG_Fnc_Msg;};

life_chopShop = _this select 3;
//Error check
if (count _nearVehicles isEqualTo 0) exitWith {titleText[localize "STR_Shop_NoVehNear","PLAIN"];};
if (!(createDialog "Chop_Shop")) exitWith {[localize "STR_Shop_ChopShopError",5,"gray"] call RPG_Fnc_Msg;};
