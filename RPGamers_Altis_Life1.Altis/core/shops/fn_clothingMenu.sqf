...
if !(_exit) exitWith {[localize "STR_Shop_Veh_NoLicense",5,"gray"] call RPG_Fnc_Msg;};
...
//Cop / Civ Pre Check
if (_shop in ["bruce","dive","reb","kart"] && {!(playerSide isEqualTo civilian)}) exitWith {[localize "STR_Shop_NotaCiv",5,"gray"] call RPG_Fnc_Msg; closeDialog 0;};
if (_shop == "reb" && {!license_civ_rebel}) exitWith {[localize "STR_Shop_NotaReb",5,"gray"] call RPG_Fnc_Msg; closeDialog 0;};
if (_shop == "cop" && {!(playerSide isEqualTo west)}) exitWith {[localize "STR_Shop_NotaCop",5,"gray"] call RPG_Fnc_Msg; closeDialog 0;};
if (_shop == "dive" && {!license_civ_dive}) exitWith {[localize "STR_Shop_NotaDive",5,"gray"] call RPG_Fnc_Msg; closeDialog 0;};
...
if (isClass(missionConfigFile >> "Licenses" >> life_clothing_store)) then {
    ...
    if !(LICENSE_VALUE(life_clothing_store,_flag)) exitWith {
        [format [localize "STR_Shop_YouNeed",localize _displayName],5,"gray"] call RPG_Fnc_Msg;
        closeDialog 0;
    };
};
