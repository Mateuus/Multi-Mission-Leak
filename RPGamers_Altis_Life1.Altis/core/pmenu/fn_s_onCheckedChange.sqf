...
switch (_option) do {
    ...
    case "sound": {
        if (_state isEqualTo 1) then {
            life_settings_phonesound = true;
            profileNamespace setVariable ["life_settings_phonesound",true];
        } else {
            life_settings_phonesound = false;
            profileNamespace setVariable ["life_settings_phonesound",false];
        };
    };
