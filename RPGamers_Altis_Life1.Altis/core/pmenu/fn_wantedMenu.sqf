...
private _list3 = CONTROL(2400,2469);
lbClear _list3; //Purge the list
...
private _mult = LIFE_SETTINGS(getArray,"multiplier");
{
  if (isLocalized (_x select 0)) then {
    _list3 lbAdd format ["%1",localize (_x select 0)];
  } else {
    _list3 lbAdd format ["%1",(_x select 0)];
  };
    _list3 lbSetData [(lbSize _list3)-1,(_x select 0)];
} forEach _mult;


ctrlSetText[2404,"Establishing connection..."];

if (FETCH_CONST(life_coplevel) < 0 && {FETCH_CONST(life_adminlevel) isEqualTo 0}) then {
    ctrlShow[2405,false];
};
