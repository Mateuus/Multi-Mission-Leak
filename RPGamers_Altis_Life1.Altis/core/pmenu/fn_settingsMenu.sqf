...
if (isNull (findDisplay 2900)) then {
    if (!createDialog "SettingsMenu") exitWith {[localize "STR_NOTF_menuWillNotOpen",5,"gray"] call RPG_Fnc_Msg;};
};
...
if (isNil "life_settings_revealObjects") then {
    ...
    life_settings_phonesound = profileNamespace setVariable ["life_settings_phonesound",true];
};

if(isNil "life_settings_phonesound") then {
    life_settings_phonesound = profileNamespace setVariable ["life_settings_phonesound",true];
};
...
CONTROL(2900,2969) cbSetChecked life_settings_phonesound;