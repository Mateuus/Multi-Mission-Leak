...
if (!(_building isKindOf "House_F")) exitWith {[localize "STR_ISTR_Bolt_NotNear",5,"gray"] call RPG_Fnc_Msg;};
...
if ((typeOf _building) == _vaultHouse && (nearestObject [_pos,"Land_Dome_Big_F"]) getVariable ["locked",true]) exitWith {[localize "STR_ISTR_Bolt_Exploit",5,"gray"] call RPG_Fnc_Msg;};
...
if (_door isEqualTo 0) exitWith {[localize "STR_Cop_NotaDoor",5,"gray"] call RPG_Fnc_Msg;}; //Not near a door to be broken into.
if ((_building getVariable [format ["bis_disabled_Door_%1",_door],0]) isEqualTo 0) exitWith {[localize "STR_House_Raid_DoorUnlocked",5,"blue"] call RPG_Fnc_Msg;};

if ((nearestObject [_pos,"Land_Dome_Big_F"]) == _building || (nearestObject [_pos,_vaultHouse]) == _building) then {
    [[1,2],"STR_ISTR_Bolt_AlertFed",true,[]] remoteExecCall ["life_fnc_broadcast", civilian];
    [1,format["911 DISPATCH: FEDERAL RESERVE IS BEING BROKEN INTO!",_storename]] remoteExecCall ["life_fnc_broadcast", west];
    [0,format["911 DISPATCH: FEDERAL RESERVE IS BEING BROKEN INTO!",_storename]] remoteExec ["life_fnc_broadcast",west];
...
[[_building], "life_fnc_bankalarmSound", true, false] spawn BIS_fnc_MP;
//[[_building],"life_fnc_bankalarmsound",nil,true] spawn BIS_fnc_MP;
