...
_escSync = {
    ...
    _syncManager = {
        ...
        _abortButton ctrlSetText "Quit RPGamers Altis Life";
        _abortButton ctrlCommit 0;
    };
...
_canUseControls = {
    ...
    if ((player getVariable ["restrained",false]) || (player getVariable ["restrainedz",false]) || (player getVariable ["Escorting",false]) || (player getVariable ["transporting",false]) || (life_is_arrested) || (life_istazed) || (life_isknocked)) then {false} else {true};
};
