...
if(isPlayer _curObject && _curObject isKindOf "Man") then {
    ...
    if((_curObject getVariable ["restrainedz",false]) && !dialog && playerSide isEqualTo west) then {
        [_curObject] call life_fnc_copInteractionMenu;
    };
    if ((_curObject getVariable ["restrainedz",false]) && !dialog && playerSide isEqualTo civilian) then {
    ...
    _miscItems = ["Land_BottlePlastic_V1_F","Land_TacticalBacon_F","Land_Can_V3_F","Land_CanisterFuel_F","Land_Defibrillator_F","Land_Suitcase_F","Land_MetalWire_F","Land_Battery_F"];

    if(playerSide != independent && !dialog && (typeOf _curObject) isEqualTo "Land_Suitcase_F") exitWith {
    ...
    if (_isVehicle) then {
        if (!dialog) then {
            if (player distance _curObject < ((boundingBox _curObject select 1) select 0)+2 && (!(player getVariable ["restrained",false])) && (!(player getVariable ["restrainedz",false])) && (!(player getVariable ["playerSurrender",false])) && !life_isknocked && !life_istazed) then {
