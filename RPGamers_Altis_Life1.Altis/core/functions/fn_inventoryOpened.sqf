...
if (_isPack isEqualTo 1) exitWith {
    [localize "STR_MISC_Backpack",5,"gray"] call RPG_Fnc_Msg;
    true;
};
...
if (KINDOF_ARRAY(_container,_list)) exitWith {
    if (!(_container in life_vehicles) && {locked _container isEqualTo 2}) exitWith {
        [localize "STR_MISC_VehInventory",5,"gray"] call RPG_Fnc_Msg;
        true;
    };
};
...
if (_container isKindOf "Man" && !alive _container) exitWith {
    [localize "STR_NOTF_NoLootingPerson",5,"gray"] call RPG_Fnc_Msg;
    true;
};