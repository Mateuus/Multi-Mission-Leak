...
if (_length > _allowedLength) exitWith {[format [localize "STR_News_HeaderLength",_allowedLength],5,"gray"] call RPG_Fnc_Msg;};

{
    if (!(_x in _allowed)) exitWith {_badCharacter = true;};
} forEach _characterByte;

if (_badCharacter) exitWith {[localize "STR_News_UnsupportedCharacter",5,"gray"] call RPG_Fnc_Msg;};
