...
if (!(_diff isEqualTo (parseNumber _val))) then {
    if ([true,_item,_diff] call life_fnc_handleInv) then {
        [format [localize "STR_MISC_TooMuch_3",_from getVariable ["realname",name _from],_val,_diff,((parseNumber _val) - _diff)],5,"green"] call RPG_Fnc_Msg;
    ...
    if ([true,_item,(parseNumber _val)] call life_fnc_handleInv) then {
        ...
        [format [localize "STR_NOTF_GivenItem",_from getVariable ["realname",name _from],_val,(localize _type)],5,"green"] call RPG_Fnc_Msg;
    } else {
        [_from,_item,_val,_unit,false] remoteExecCall ["life_fnc_giveDiff",_from];
    };
};