...
if (_bool) then {
    if ([true,_item,(parseNumber _val)] call life_fnc_handleInv) then {
        [format [localize "STR_MISC_TooMuch",_from getVariable ["realname",name _from],_val,(localize _type)],5,"blue"] call RPG_Fnc_Msg;
    };
} else {
    if ([true,_item,(parseNumber _val)] call life_fnc_handleInv) then {
        [format [localize "STR_MISC_TooMuch_2",_from getVariable ["realname",name _from],_val,(localize _type)],5,"blue"] call RPG_Fnc_Msg;
    };
};