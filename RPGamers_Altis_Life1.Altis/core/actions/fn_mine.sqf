...
if (player getVariable "restrained") exitWith {
    [localize "STR_NOTF_isrestrained",5,"gray"] call RPG_Fnc_Msg;
};
if (player getVariable "restrainedz") exitWith {
    [localize "STR_NOTF_isrestrained",5,"gray"] call RPG_Fnc_Msg;
};
_exit = false;
if (player getVariable "playerSurrender") exitWith {
    [localize "STR_NOTF_surrender",5,"gray"] call RPG_Fnc_Msg;
};
...
if (_requiredItem != "") then {
    ...
    if (_valItem < 1) exitWith {
        switch (_requiredItem) do {
            case "pickaxe": {
                [localize "STR_NOTF_Pickaxe",5,"gray"] call RPG_Fnc_Msg;
            };
        };
        life_action_inUse = false;
        _exit = true;
...
if (([true, _mined, _diff] call life_fnc_handleInv)) then {
    ...
    [format ["Mined %2 %1(s)", (localize _itemName), _diff],5,"blue"] call RPG_Fnc_Msg;
};
