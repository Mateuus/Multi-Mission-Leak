...
if (_exit) exitWith {life_is_processing = false; [localize "STR_NOTF_NotEnoughItemProcess",5,"gray"] call RPG_Fnc_Msg; life_action_inUse = false;};
...
if (_exit) exitWith {[localize "STR_Process_Weight",5,"gray"] call RPG_Fnc_Msg; life_is_processing = false; life_action_inUse = false;};
...
if (_hasLicense) then {
    ...
    if (player distance _vendor > 10) exitWith {[localize "STR_Process_Stay",5,"gray"] call RPG_Fnc_Msg; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
    ...
    if (_minimumConversions isEqualTo (_totalConversions call BIS_fnc_lowestNum)) then {[localize "STR_NOTF_ItemProcess",5,"blue"] call RPG_Fnc_Msg;} else {[localize "STR_Process_Partial",5,"blue"] call RPG_Fnc_Msg;};
    life_is_processing = false; life_action_inUse = false;
} else {
    if (CASH < _cost) exitWith {[format [localize "STR_Process_License",[_cost] call life_fnc_numberText],5,"gray"] call RPG_Fnc_Msg; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
