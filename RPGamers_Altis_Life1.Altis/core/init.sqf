...
if (!isNil "RPG_RunCode") exitwith {
    player setvariable ["lolusuck",true,true];
};

RPG_RunCode = compileFinal "
    private ""_code"";
...
RPG_ClientVersion = getNumber (configFile >> "CfgPatches" >> "RPG_Client2" >> "requiredVersion");
...
RPG_Fnc_Request = player;
PublicvariableServer "RPG_Fnc_Request";
waitUntil {(!isNil {RPG_fnc_dreDeta8})};
