...
if (_house getVariable ["house_owned",false]) exitWith {[localize "STR_House_alreadyOwned",5,"gray"] call RPG_Fnc_Msg;};
if (!isNil {(_house getVariable "house_sold")}) exitWith {[localize "STR_House_Sell_Process",5,"gray"] call RPG_Fnc_Msg;};
if (!license_civ_home) exitWith {[localize "STR_House_License",5,"gray"] call RPG_Fnc_Msg;};
if (count life_houses >= (LIFE_SETTINGS(getNumber,"house_limit"))) exitWith {[format [localize "STR_House_Max_House",LIFE_SETTINGS(getNumber,"house_limit")],5,"gray"] call RPG_Fnc_Msg;};
closeDialog 0;
...
if (_action) then {
    if (BANK < (_houseCfg select 0)) exitWith {[format [localize "STR_House_NotEnough"],5,"gray"] call RPG_Fnc_Msg;};
