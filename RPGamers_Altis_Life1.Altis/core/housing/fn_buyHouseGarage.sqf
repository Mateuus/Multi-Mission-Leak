...
if (_house getVariable ["garageBought",false]) exitWith {[localize "STR_Garage_alreadyOwned",5,"gray"] call RPG_Fnc_Msg;};
if ((_house getVariable "house_owner") select 0 != getPlayerUID player) exitWith {[localize "STR_Garage_NotOwner",5,"gray"] call RPG_Fnc_Msg;};
...
if (_action) then {
    ...
    if (BANK < _price) exitWith {[localize "STR_House_NotEnough",5,"gray"] call RPG_Fnc_Msg;};
    BANK = BANK - _price;
