...
if (life_gps) then {
	/*_marker1 = createMarker [format["%1_marker",_player], position player];
	_marker1 setMarkerType "o_med";
	_marker1 setMarkerText format["Hilfe: %1",name player];*/
	_pos = mapGridPosition player;
} else {
	_pos = "GPS Deaktiviert";
};
...
switch(_type) do{ 
	...
	case 1: { 
		...
		[life_smartphoneTarget,_msg,player,0,_pos] remoteExec ["TON_fnc_handleMessages",2];
	...
	//copmessage 
	case 2: { 
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["The police is currently inaccessible. Please try again later."];};
		ctrlShow[888895,false];
		if(_msg == "") exitWith {hint "You must enter a message to be sent!";
			ctrlShow[888895,true];
		};
		[ObjNull,_msg,player,1,_pos] remoteExec ["TON_fnc_handleMessages",2];
		_to = "Police";
	...
	case 3: { 
		...
		[ObjNull,_msg,player,2,_pos] remoteExec ["TON_fnc_handleMessages",2];
		_to = "The Admins";
		hint format["You sent %1 a message: %2",_to,_msg];
		ctrlShow[888896,true];
		closeDialog 887890;
	...
	//emsrequest 
	case 4: { 
		if(({side _x == independent} count playableUnits) == 0) exitWith {hint format["Currently there is no EMS on duty . Please try again later."];};
		ctrlShow[888899,false];
		if(_msg == "") exitWith {hint "You must enter a message to be sent!";
			ctrlShow[888899,true];
		};
		[ObjNull,_msg,player,3,_pos] remoteExec ["TON_fnc_handleMessages",2];
