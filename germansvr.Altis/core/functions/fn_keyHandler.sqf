...
switch (_code) do {
    ...
    case 57: {
		...
		if(life_jumpActionTime <= time) then
		{
			if(_shift && {animationState player != "AovrPercMrunSrasWrflDf"} && {isTouchingGround player} && {stance player == "STAND"} && {speed player > 2} && {!life_is_arrested} && {(velocity player) select 2 < 2.5}) then {
				...
				life_jumpMessageCooldown = true;
				[player,true] spawn life_fnc_jumpFnc; //Local execution
				[player,false] remoteExec ["life_fnc_jumpFnc", -2, FALSE];
				_handled = true;
			};
		} else {
			if(life_jumpMessageCooldown) then
			{
				systemChat "Du kannst nur alle 5 Sekunden springen!";
				life_jumpMessageCooldown = false;
			};
		};
	};
    ...
    case _mapKey: {
        switch (playerSide) do {
            case west: {if (!visibleMap) then {[] spawn life_fnc_copMarkers;[] spawn life_fnc_selfMapMarker;}};
            case independent: {if (!visibleMap) then {[] spawn life_fnc_medicMarkers;[] spawn life_fnc_selfMapMarker;}};
            case civilian: {if (!visibleMap) then {[] spawn life_fnc_civMarkers;[] spawn life_fnc_selfMapMarker;}};
            case east: {if (!visibleMap) then {[] spawn life_fnc_civMarkers;[] spawn life_fnc_selfMapMarker;}};
        };
		[] spawn life_fnc_selfMapMarker;
    };
    ...
    case 34: {
        if (_shift) then {_handled = true;};
        if (_shift && (playerSide isEqualTo civilian || playerSide isEqualTo east) && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Van_01_box_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","I_MRAP_03_F"]) then {
    ...
    case 22: {
        if (!_alt && !_ctrlKey) then {
			if (life_look_active) exitWith {};
			[] spawn {
                life_look_active = true;
                sleep 2;
                life_look_active = false;
            };
            ...
            if (_veh isKindOf "House_F" && (playerSide isEqualTo civilian || playerSide isEqualTo east)) then {
                ...
                if (_veh in life_vehicles && {player distance _veh < 20}) then {
                    if (_locked isEqualTo 2) then {
                        ...
                        [_veh,"lockSound"] remoteExec ["life_fnc_say3D",RANY];
                    } else {
                        if (local _veh) then {
                            _veh lock 2;
	...
	case 7:
	{
		if(vehicle player isEqualTo player) then {
			if(!life_action_inUse) then {
				player playActionNow "acts_briefing_sb_in";
			};
		};
	};
	case 8:
