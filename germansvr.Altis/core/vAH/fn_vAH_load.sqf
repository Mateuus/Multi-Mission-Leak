...
if (count all_ah_items == 0) then {
		_myListbox lbAdd format ["Aktuell gibts nichts in der Auktion"];
	...
	} else {
{
	...
	_time = round (_time / 3);
	if (_status == 0)then {
	_myListbox lbAdd format ["%5 X %3 für €%2 von %1 (%4 Stunden noch im Angebot)",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText,[_amount] call life_fnc_numberText];
