...
if (count _houseCfg isEqualTo 0 && playerSide in [civilian,east]) exitWith {};
...
if (_curTarget isKindOf "House_F" && playerSide isEqualTo west) exitWith {
    ...
    if ((nearestObject [[16543,12792,0],"Land_Offices_01_V1_F"]) isEqualTo _curTarget || (nearestObject [[16543,12792,0],"Land_Medevac_house_V1_F"]) isEqualTo _curTarget) then {
...
if (!(_curTarget in life_vehicles) || isNil {_curTarget getVariable "house_owner"}) then {
    ...
    if (isClass (missionConfigFile >> "Garages" >> worldName >> (typeOf _curTarget))) then {
		...
		if (_curTarget getVariable ["alarme",true]) then { // Hinzugefügter Code
			_Btn6 ctrlSetText "Alarm ausschalten";
			_Btn6 buttonSetAction "life_pInact_curTarget setVariable ['alarme',false,true]; closeDialog 0;";
			_Btn6 ctrlshow true;
		} else {
			_Btn6 ctrlSetText "Alarm anschalten";
			_Btn6 buttonSetAction "life_pInact_curTarget setVariable ['alarme',true,true]; closeDialog 0;";
			_Btn6 ctrlshow true;
		};
    };
};
