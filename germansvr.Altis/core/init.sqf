...
[0,localize "STR_Init_ServerReady"] call mav_introcam_fnc_updateCinematicStatus;
...
if (_extDB_notLoaded isEqualType []) exitWith {
	...
	[0, localize "STR_Init_ExtdbFail"] call mav_introcam_fnc_updateCinematicStatus;
};

[] call SOCK_fnc_dataQuery;
waitUntil {life_session_completed};
[0, localize "STR_Init_ClientFinish"] call mav_introcam_fnc_updateCinematicStatus;


diag_log "::Client:: Pruefe Personalausweisdaten."; // Checking identity card
[0, "Ueberpruefe Daten des Personalausweises..."] call mav_introcam_fnc_updateCinematicStatus; // Checking identity card
[] call life_fnc_perso_laden;
waitUntil{life_persoReady};

[1, "PRESS SPACE TO CONTINUE"] call mav_introcam_fnc_updateCinematicStatus;
waitUntil {mav_introcam_continue}; // complete
...
switch (playerSide) do {
	...
	case east: {
		life_paycheck = LIFE_SETTINGS(getNumber,"paycheck_east");
	};
};
...
{if ((_x select 5 == _uid) && (_x select 7 == 2)) then {_total = _total + (_x select 4);_toDel pushBack (_x select 0)};}forEach all_ah_items;
 ...
 if (_total > 0) then {
	{
		[1,_x] remoteExec ["TON_fnc_vAH_update",2];
	}forEach _toDel;
...
//Winter
[Jz] execVM "scripts\effects\jahreszeit.sqf";
diag_log "----------------------------------------------------------------------------------------------------";
