...
*/
private["_dialog","_inv","_mats","_item","_struct","_klbroh","_klb","_str","_str2","_invSize","_matsNeed","_matsNum","_config","_itemFilter"]; //Declare all private variables
...
_config = [_itemFilter] call life_fnc_craftCfg;
{
	if(_item == _x select 0)then
	{
		...
		for [{_i = 0},{_i < _invSize - 1},{_i = _i + 2}] do {
			...
			_struct = _struct + format["%1 x %2<br/>",_matsNum,localize _str];
		};
	};
} foreach (_config);
if(_struct == "") then
