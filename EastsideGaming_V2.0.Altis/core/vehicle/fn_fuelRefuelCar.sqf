...
if ((BANK - (_fueltoput * life_fuelPrices))> 0)then {
    ...
    for "_i" from 0 to 1 step 0 do {
        ...
        if (_cP >= 1) exitWith {};
        if (player distance _car > 10) exitWith {_t2 = "Abgebrochen";};
        if (vehicle player != player) exitWith {_t2 = "Abgebrochen";};
        if !((BANK - round(0.01 * _totalcost))> 0) exitWith {_t2 = "Abgebrochen";};
