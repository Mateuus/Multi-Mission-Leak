...
switch (true) do {
    ...
    case (_item in ["apfel","pfirsich","kirsche","banane","weintrauben","fisch","mullet","catshark","turtle_soup","hen","rooster","sheep","goat","donuts","tbacon","peach"]): {
	...
	case (_item in ["apfelsaft","pfirsichsaft","kirschsaft","bananensaft","kiba"]): {
        if (!(M_CONFIG(getNumber,"VirtualItems",_item,"edible") isEqualTo -1)) then {
            if ([false,_item,1] call life_fnc_handleInv) then {
                _val = M_CONFIG(getNumber,"VirtualItems",_item,"edible");
