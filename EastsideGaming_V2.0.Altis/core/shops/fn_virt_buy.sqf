...
if(ESG_stop_dupe)exitWith{hint parsetext "Bitte warte noch kurz!<br/>Tipp:<br/>Geb direkt die Anzahl an, die du kaufen möchtest!"};
...
if ([true,_type,_amount] call life_fnc_handleInv) then {
    if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
        ...
        if (_action) then {
			...
			_log = format["%1 (%2) hat %3(%5 Mal) für %4$ gekauft, vom Gang Geld.", player getVariable["realname", name player], getPlayerUID player, (localize _name), [(_price * _amount)]call life_fnc_numberText, [_amount]call life_fnc_numberText];
			["VIRTUALSHOP_LOG", _log] remoteExecCall ["ESG_fnc_logging", 2];
			[]spawn {
				ESG_stop_dupe = true;
				sleep 15;
				ESG_stop_dupe = false;
			...
			_log = format["%1 (%2) hat %3(%5 Mal) für %4$ gekauft.", player getVariable["realname", name player], getPlayerUID player, (localize _name), [(_price * _amount)]call life_fnc_numberText, [_amount]call life_fnc_numberText];
			["VIRTUALSHOP_LOG", _log] remoteExecCall ["ESG_fnc_logging", 2];
			[]spawn {
				ESG_stop_dupe = true;
				sleep 15;
