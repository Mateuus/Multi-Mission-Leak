...
if(side _cop isEqualTo independent) then {
...
[] spawn {
    ...
    if (isNull (findDisplay 2600) && !life_ticket_paid) then {
		if(side life_ticket_cop isEqualTo independent) then {
			[0,"%1 weigert sich, die Rechnung zu bezahlen.",true,[profileName]] remoteExecCall ["life_fnc_broadcast",independent];
			[1,"%1 weigert sich, die Rechnung zu bezahlen.",true,[profileName]] remoteExecCall ["life_fnc_broadcast",life_ticket_cop];
		} else {
