...
class Life_RscXListbox
{
	...
	idc = -1; // Control identification (without it, the control won't be displayed)
	type = 42; // Type
	style = 0x400 + 0x02 + 0x10; // Style
	default = 0; // Control selected by default (only one within a display can be used)
	blinkingPeriod = 0; // Time in which control will fade out and back in. Use 0 to disable the effect.
	colorBackground[]={0,0,0,0.3};

	text = "";
	x = 12 * (GUI_GRID_CENTER_WAbs / 40) + (safezoneX + (safezoneW - GUI_GRID_CENTER_WAbs)/2); // Horizontal coordinates
	y = 17 * (GUI_GRID_CENTER_HAbs / 25) + (safezoneY + (safezoneH - GUI_GRID_CENTER_HAbs)/2); // Vertical coordinates
	w = 10 * (GUI_GRID_CENTER_WAbs / 40); // Width
	h = 1 * (GUI_GRID_CENTER_HAbs / 25); // Height

			color[] = {1,1,1,1}; // Arrow color
	colorActive[] = {1,1,1,1}; // Selected arrow color

	sizeEx = (GUI_GRID_CENTER_HAbs / 25); // Text size
...
class ESG_RscCloseButton : Life_RscButtonMenu {
	...
	colorBackground2[] = {0.753,0.145,0.145,0};
	animTextureOver = "#(argb,8,8,3)color(0.753,0.145,0.145,0)";
    colorBackground[] = {0,0,0,0};
    color[] = {1,1,1,0};
...
class ESG_RscTree
		{
			...
			colorBackground[] = {0,0,0,0.5}; // Fill color
			colorSelect[] = {0.753,0.145,0.145,1}; // Selected item fill color (when multiselectEnabled is 0)
			colorMarked[] = {0.753,0.145,0.145,1}; // Marked item fill color (when multiselectEnabled is 1)
			colorMarkedSelected[] = {0.753,0.145,0.145,1}; // Selected item fill color (when multiselectEnabled is 1)
      colorSelectBackground[] = {0, 0, 0, 0.5};
      colorPictureDisabled[] = {1,1,1,1};
      colorPictureRight[] = {1,1,1,1};
      ...
      colorPictureSelected[] = {0,0,0,1};
			...
			class ScrollBar
			{
				...
				scrollSpeed = 0.06; // scroll speed of ScrollBar
