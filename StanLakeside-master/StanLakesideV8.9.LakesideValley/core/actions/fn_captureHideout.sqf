...
while {true} do
{
	...
	if(_cP >= 1 OR deadPlayer) exitWith {_hideout setVariable["inCapture",false,true];};
...
if(deadPlayer OR life_istazed) exitWith {life_action_inUse = false;_hideout setVariable["inCapture",false,true];};
...
[[0,1],format[localize "STR_GNOTF_CaptureSuccess",name player,(group player) getVariable "gang_name" ]] remoteExecCall ["life_fnc_broadcast", -2];
