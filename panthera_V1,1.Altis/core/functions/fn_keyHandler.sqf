...
switch (_code) do {
        ...
        if (!_alt && !_ctrlKey && !dialog && {!life_action_inUse}) then {
            if (vehicle player != player && alive vehicle player) then {
                ...
                if (!(isNil "_list")) then {
                    ...
                    if (KINDOF_ARRAY(cursorObject,_list) && {player distance cursorObject < 14} && {isNull objectParent player} && {alive cursorObject} && {!life_action_inUse}) then {
    ...
    case 24: {	
	if(_shift) then
	{		switch (player getVariable["Earplugs",0])
	do {			
	case 0: 
	{hintSilent "Ear Plugs 90%";
	1 fadeSound 0.1;
	player setVariable ["Earplugs", 10];
	};
	case 10:
	{hintSilent "Ear Plugs 60%";
	1 fadeSound 0.4;
	player setVariable ["Earplugs", 40];
	};	
	case 40: 
	{hintSilent "Ear Plugs 30%";
	1 fadeSound 0.7;
	player setVariable ["Earplugs", 70];
	};	
	case 70: 
	{hintSilent "Ear Plugs Removed";
	1 fadeSound 1;
