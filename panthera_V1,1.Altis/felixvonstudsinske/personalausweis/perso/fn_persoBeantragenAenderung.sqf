...
if(life_cash < _preis) exitWith {hint parseText format ["Du hast nicht genug Geld auf der Hand.<br/>Aktuell: $%2<br/>Fehlend: $%1",(_preis - life_cash), life_cash];};
...
if(fvs_namen_a && !(profileName isEqualTo (ctrlText 70002)) && (_erlaubeMehrereNamen isEqualTo 0)) then {
	hint parseText format ["Du hast deinen Personalausweis geaendert. Du wirst automatisch nach 5 Minuten aus dem Spiel entfernt, damit du dein Profilnamen aendern kannst. Es wurde eine Gebuehr von $ %17 erhoben.<br/><br/>Daten des Personalausweises im Ueberblick:<br/><br/>Name: %1<br/>Strasse: %2 %3<br/>Plz/Ort: %4 %5<br/><br/>Email: %6<br/>Handynummer: 0%7<br/>Koerpergroesse: %8cm<br/>Gewicht: %9kg<br/>Alter: %10 Jahre<br/>Geburtsdatum: %11 %12 %13<br/>Blutgruppe: %14<br/><br/>Fotobezeichnung: %15 %16",_name,_stra,_hsnr,_plz,_wohnort,_email,fvs_handynr,_cm,_kg,_alter,_t,_m,_j,_bltg,_bildID,_bildName,_preis];
	...
	hint parseText format ["Du hast deinen Personalausweis geaendert. Es wurde eine Gebuehr von $ %17 erhoben. <br/><br/>Daten des Personalausweises im Ueberblick:<br/><br/>Name: %1<br/>Strasse: %2 %3<br/>Plz/Ort: %4 %5<br/><br/>Email: %6<br/>Handynummer: 0%7<br/>Koerpergroesse: %8cm<br/>Gewicht: %9kg<br/>Alter: %10 Jahre<br/>Geburtsdatum: %11 %12 %13<br/>Blutgruppe: %14<br/><br/>Fotobezeichnung: %15 %16",_name,_stra,_hsnr,_plz,_wohnort,_email,fvs_handynr,_cm,_kg,_alter,_t,_m,_j,_bltg,_bildID,_bildName,_preis];
	player setVariable ["personaltext",_insert,true];
	fvs_namen_a = false;
	fvs_checking_a = false;
	life_cash = life_cash - _preis;
