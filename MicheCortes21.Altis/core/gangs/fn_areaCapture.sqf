...
if ((player distance _area) > 10) exitWith { hint "Du musst näher an der Flagge sein!"; }; //too far away
...
//Police officer checks
{
    ...
    if (_areaContested && _xname isEqualTo "Rebels") then {
        hint "Die Rebellen sind noch in der Nähe!";
    };
...
if (!(_areaContested)) then {
    for "_i" from 0 to 1 step 0 do {
        ...
        if (_areaContested) exitWith {hint "Bereich bestritten";}; //Future testing
...
_pText = format["Neutralisierter Bereich"];
...
_xname = ""; //setup for popo name
_nearUnits = _area nearEntities ["Man",500]; //searches area for players
if (isNil {group player getVariable "gang_name"}) exitWith { hint "Du musst in einer Gang sein!"; }; //not in a gang
if (_group isEqualTo group player) exitWith { hint "Deine Gang hat dieses Bereich schon eingenommen!"; }; //already own it
if ((_area getVariable ["inCapture",FALSE])) exitWith {hint "Nur eine Person kann das Gebiet einnehmen!"; }; //stops 2 people capturing at the same time
...
//check if area is contested
{
    ...
    if (_areaContested) exitWith {
        if (_xname isEqualTo "Police Force") then {
            hint "Die Polizei ist in der Nähe!";
        } else {
            hint "Andere Rebellen sind in der Nähe!";
        };
    };
...
if (!(_areaContested)) then {
    for "_i" from 0 to 1 step 0 do {
        ...
        if (_cP >= 1 || !alive player) exitWith {_area setVariable ["inCapture",false,true];};
        if (player distance _area > 100) exitWith {_area setVariable ["inCapture",false,true];}; //how far they can go from the hideout while capturing
        if (_areaContested) exitWith {hint "Bereich bestritten";};
        if (life_istazed) exitWith {_area setVariable ["inCapture",false,true];}; //Tazed
        if (life_isknocked) exitWith {_area setVariable ["inCapture",false,true];}; //Knocked
        if (life_interrupted) exitWith {_area setVariable ["inCapture",false,true];}; //interupted
    };
...
_gName = group player getVariable "gang_name";
_Text = format["Gebiet erobert von : %1",_gName];
