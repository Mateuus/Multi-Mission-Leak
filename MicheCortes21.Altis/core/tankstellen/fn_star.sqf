...
_star = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
_countCop = (west countSide playableUnits);
	...
	if (player distance _star > 5) exitWith { hint "Du bist ziemlich weit wech!!"; };
	...
	if (isNull player OR isNull _star) exitWith {};
	if (life_startanke) exitWith { hint "Die Kasse ist leer."; };
...
_pgText ctrlSetText format["Du raubst die Star-Tankstelle aus (1%1)...","%"];
...
while{true} do
{
	...
	if (!alive player) exitWith {};
	if (player distance _star > 5) exitWith {hint "Du hast dich zu weit entfernt!"; 5 cutText ["","PLAIN"];life_startanke = false; publicVariable "life_startanke";deleteMarker "Marker200"; deleteMarker "Marker900";};		
	if(_cP >= 1) exitWith {};
}; 

if (player distance _star > 5) exitWith { hint "Du hast dich zu weit entfernt!";life_startanke = false;publicVariable "life_startanke";deleteMarker "Marker200"; deleteMarker "Marker900";};

5 cutText ["Du hast die Star-Tankstelle erfolgreich ausgeraubt!","PLAIN"];
