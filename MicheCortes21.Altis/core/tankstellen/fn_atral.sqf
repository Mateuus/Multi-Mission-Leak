...
*/
private["_ui","_progress","_atral","_pgText","_cp","_this","_countCop","_kassa"];
_atral = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
_countCop = (west countSide playableUnits);
	...
	if (player distance _atral > 5) exitWith { hint "Du bist ziemlich weit wech!!"; };
	...
	if (isNull player OR isNull _atral) exitWith {};
	if (life_atraltanke) exitWith { hint "Die Kasse ist leer."; };
...
_pgText ctrlSetText format["Du raubst die Atral-Tankstelle aus (1%1)...","%"];
...
while{true} do
{
	...
	_pgText ctrlSetText format["Du raubst die Atral-Tankstelle aus (%1%2)...",round(_cP * 100),"%"];
	...
	if (!alive player) exitWith {};
	if (player distance _atral > 5) exitWith {hint "Du hast dich zu weit entfernt!"; 5 cutText ["","PLAIN"];life_atraltanke = false; publicVariable "life_atraltanke";deleteMarker "Marker200"; deleteMarker "Marker900";};		
	if(_cP >= 1) exitWith {};
}; 

if (player distance _atral > 5) exitWith { hint "Du hast dich zu weit entfernt!";life_atraltanke = false;publicVariable "life_atraltanke";deleteMarker "Marker200"; deleteMarker "Marker900";};

5 cutText ["Du hast die Atral-Tankstelle erfolgreich ausgeraubt!","PLAIN"];
