...
3 fadeSound 1;
playSound "alarm";
if(_isEvent == 0)then{hint parseText getText(missionConfigFile >> "Maverick_ServerRestartEvent" >> "MessageRestartInit");
}else{hint parseText getText(missionConfigFile >> "Maverick_ServerRestartEvent" >> "MessageEventInit");};
uiSleep 2.5;
playSound "alarm";
uiSleep 2.5;
if(!isServer && (getText(missionConfigFile >> "Maverick_ServerRestartEvent" >> "RestartSound") != "")) then {playSound getText(missionConfigFile >> "Maverick_ServerRestartEvent" >> "RestartSound");};
