...
try {
	...
	_updateGUI = {
		...
		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1/%2, Perk Punkte verfügbar: %3", life_currentExp - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, life_currentPerkPoints];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1, Maximum Level, Perk Punkte verfügbar: %2", life_currentExp - _currentLevelBottomExp, life_currentPerkPoints];
	...
	switch (_mode) do
	{
		case "onLoad":
		{
			...
			if (!life_perksInitialized) exitWith {
				hint "Deine Perks werden geladen, versuche es später";
			};
					...
					if (_hasParents) then {
						if (!_ownsPerk) then {
							if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) then {
								_ctrlBtnPurchase ctrlSetText "INSUFFIZIENTE PUNKTE";
							...
							_ctrlBtnPurchase ctrlSetText "BEREITS GEKAUFT";
							_ctrlBtnPurchase ctrlEnable false;
						};
					} else {
						_ctrlBtnPurchase ctrlSetText "NICHT KAUFBAR";
						_ctrlBtnPurchase ctrlEnable false;
					};
				} else {
					_ctrlBtnPurchase ctrlSetText "Höheres LEVEL REQ.";
					_ctrlBtnPurchase ctrlEnable false;
				};
			} else {
		...
		case "onButtonPurchaseClick":
		{
			...
			_ctrlBtnPurchase ctrlSetText "GEKAUFT!";
			_ctrlBtnPurchase ctrlEnable false;
