...
_value = 0;
{
	...
	if(EQUAL(ITEM_ILLEGAL(_var),1)) then {
		if(!(EQUAL(ITEM_SELLPRICE(_var),-1))) then {
			...
			SUB(_houseInvData,[-1]);
			SUB(_houseInvVal,(([_var] call life_fnc_pruvubezust) * _val));
...
if(_value > 0) then {
	[[0,"STR_House_Raid_Successful",true,[[_value] call life_fnc_rupadudejat]],"life_fnc_brusathusek",true,false] call life_fnc_MP;
