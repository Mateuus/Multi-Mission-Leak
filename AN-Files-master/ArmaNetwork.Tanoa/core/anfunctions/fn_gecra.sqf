/*
  ArmA.Network
  Rathbone
  Search Function for Admin Menu
  (c) ArmA.Network 2016
...
*/
_searchbox = (((findDisplay 2900) displayCtrl 1400));
_playerList = (((findDisplay 2900) displayCtrl 1500));
_nameList = [];
if!((ctrlText _searchbox) isEqualTo "") then {
	{
		lbClear _playerList;
	...
	} forEach Admin_Search;
} else {
	lbClear _playerList;
	_nameList = Admin_Search;
};
