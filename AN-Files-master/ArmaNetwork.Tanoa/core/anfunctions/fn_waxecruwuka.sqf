...
_details = [_item] call life_fnc_thagawrepru;
		...
		case "CfgVehicles":
		{
			if (_item iskindof "Man") then {
				if(player isKindOf "Civilian") then {
					if(uniform player == (_details select 16)) then {
						player addItem (_details select 16);
					} else {
						if(uniform player != "") then {
							_items = uniformItems player;
							removeUniform player;
						};
						player forceaddUniform (_details select 16);
					...
					if(!(player isUniformAllowed (_details select 16))) then {
						player forceAddUniform (_details select 16);
					} else {
						player addUniform (_details select 16);
					};
					if(!isNil "_items") then {
						{player addItemToUniform _x} foreach _items;
					};
				...
				if(!isNil {_items}) then
				{
					{[_x,true,true,false,true] spawn life_fnc_waxecruwuka; } forEach _items;
				};
			};
		};
				...
				else
			{
				switch(_details select 5) do
				{
							...
							else
						{
								...
								else
							{
									...
									else
								{
									...
									if(!isNil {_items}) then
									{
										{[_x,true,false,false,true] spawn life_fnc_waxecruwuka;} forEach _items;
									};
								};
							};
						};
					...
					case 201:
					{
						...
						{
							private["_type"];
							_type = [_item,201] call life_fnc_druqakastup;
							if(_ongun) then
							{
								switch (_type) do
								{
								...
								else
							{
									...
									else
								{
									private["_wepItems","_action","_slotTaken"];
										...
										if(_slotTaken) then {/*
											_action = [
												localize "STR_MISC_AttachmentMSG",
												localize "STR_MISC_Attachment",
												localize "STR_MISC_Weapon",
												localize "STR_MISC_Inventory"
					...
					case 301:
					{
						...
						{
							private["_type"];
							_type = [_item,301] call life_fnc_druqakastup;
					...
					case 101:
					{
						...
						{
							private["_type"];
							_type = [_item,101] call life_fnc_druqakastup;
