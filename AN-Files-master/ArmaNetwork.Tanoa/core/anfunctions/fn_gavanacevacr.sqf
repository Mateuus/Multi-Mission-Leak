...
if(_vehicle getVariable["purpose",""] isEqualTo "truck_mission") then
	{
		...
		[0,"STR_NOTF_BeingImpounded",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_brusathusek",-2];
	};
...
for [{_i = 0},{_i < 1},{_i = _i + 0.006}] do
{
	...
	if(player distance (position _vehicle) > 10) exitWith {
		titleText ["You're too far to continue your Impound!","PLAIN"];
		life_action_inUse = false;
		[false] call life_fnc_tufruxatra;
		breakOut "impounding";
	};
	...
	if(_vehicle getVariable["purpose",""] == "truck_mission") then
		{
			...
			[0,"STR_NOTF_HasImpounded",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_brusathusek",-2];
			[_price,1,0] call life_fnc_sewawruk;
		};
} else {
