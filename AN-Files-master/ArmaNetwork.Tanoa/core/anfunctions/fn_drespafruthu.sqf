...
[] spawn {
	...
	if(isNull (findDisplay 2600) && !life_ticket_paid) then {
		[0,"STR_Cop_Ticket_Refuse",true,[profileName]] remoteExecCall ["life_fnc_brusathusek",west];
		[1,"STR_Cop_Ticket_Refuse",true,[profileName]] remoteExecCall ["life_fnc_brusathusek",life_ticket_cop];
	};
};
