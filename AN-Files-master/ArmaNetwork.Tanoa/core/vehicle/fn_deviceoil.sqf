...
_resourceZones = ["oil_1","oil_2","gas_1"];
_zone = "";
...
_item = switch(true) do {
	...
	case (_zone in ["gas_1"]): {"oilu"};
	default {""};
};

if(_item == "") exitWith {hint "Bad Resource?"; life_action_inUse = false;};
_vehicle setVariable ["mining",true,true]; //Lock the device
[_vehicle] remoteExec ["life_fnc_soundDevice",-2]; //Broadcast the 'mining' sound of the device for nearby units.
