...
_resourceZones = ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","heroin_1","cocaine_1","weed_1","diamond_1","mushrooms_1","khat_1","meth_1","sm_1","sm_2","sm_3","sm_4","sm_5","sm_6","mm_1","mm_2","mm_3","mm_4","mm_5","lm_1","lm_2"];
_zone = "";
...
_item = switch(true) do {
	...
	case (_zone in ["mushrooms_1"]): {"mushrooms"};
	case (_zone in ["sm_1"]): {"rock"};
	case (_zone in ["sm_2"]): {"sand"};
	case (_zone in ["sm_3"]): {"zincore"};
	case (_zone in ["sm_4"]): {"salt"};
	case (_zone in ["sm_5"]): {"lead"};
	case (_zone in ["sm_6"]): {"ironore"};
	case (_zone in ["mm_1"]): {"lead"};
	case (_zone in ["mm_2"]): {"copperore"};
	case (_zone in ["mm_3"]): {"rock"};
	case (_zone in ["mm_4"]): {"zincore"};
	case (_zone in ["mm_5"]): {"ruby"};
	case (_zone in ["lm_1"]): {"copperore"};
	case (_zone in ["lm_2"]): {"platinumore"};
	default {""};
};
...
for "_i" from 0 to 1 step 0 do {
	...
	_itemIndex = [_item,_items] call life_fnc_kuthaxadrep;
	_weight = [_vehicle] call life_fnc_vehicleWeight;
	_sum = [_item,15,_weight select 1,_weight select 0] call life_fnc_wacruspekuc; //Get a sum base of the remaining weight..
	if(_sum < 1) exitWith {titleText[localize "STR_NOTF_DeviceFull","PLAIN"];};
	_itemWeight = ([_item] call life_fnc_pruvubezust) * _sum;
	...
	if(local _vehicle) then {
		...
		[_vehicle,(fuel _vehicle)-0.04] remoteExecCall ["setFuel",_vehicle];
	};
	...
	_weight = [_vehicle] call life_fnc_vehicleWeight;
	_sum = [_item,15,_weight select 1,_weight select 0] call life_fnc_wacruspekuc;
	if(_sum < 1) exitWith {titleText[localize "STR_NOTF_DeviceFull","PLAIN"];};
	sleep 2;
};
