...
_timetoWait = time + 30;
_action = [format ["Live Übertragung der Überwachungskamera deines Hauses anschauen?"],"Einbruchsschutz System","Ja","Nein"] call BIS_fnc_guiMessage;
if(_action) then {
	if(time >= _timetoWait) exitWith {hint "Du hast zu lange gewartet!";};
	hint "Übertragung der Überwachungskamera wird eingerichtet...";
