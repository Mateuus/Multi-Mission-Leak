...
for "_i" from 0 to ((count PGSERV_res_loot) - 1) do
{
    ...
    {
        _mine = SEL(PGSERV_res_loot, _i) select 1; // select shovel loot array at position 1
    };
};
...
if (!life_interrupted) then {
    if(([true,_item,_diff] call life_fnc_handleInv)) then {
        ...
        titleText[format[localize "STR_MS_Shovel_Success",(localize _itemName),_diff],"PLAIN"];
    };
};
