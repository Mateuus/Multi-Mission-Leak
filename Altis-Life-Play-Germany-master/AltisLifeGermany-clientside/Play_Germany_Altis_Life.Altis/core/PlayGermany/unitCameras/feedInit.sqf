...
if (isnil "_feedArray") then {
	_feedArray=[];
	{
		if (isnil "_x") then {
		} else {
			_feedArray = _feedArray + [_x];
		};
	} forEach _newArray;
...
if (!isserver || !isdedicated) then {
	{
		null = [_x,_feedArray,server] execVM "core\PlayGermany\unitCameras\livefeed.sqf";
...
[_monitorArray,_MonitorName] spawn {
	...
	while {true} do {
		...
		if (_newArray != _oldArray || (count _feedArray) <= 1) then {
			{
				null = [_x,_feedArray] execVM "core\PlayGermany\unitCameras\livefeed.sqf";
			}foreach _monitorArray;
		};
	};
};