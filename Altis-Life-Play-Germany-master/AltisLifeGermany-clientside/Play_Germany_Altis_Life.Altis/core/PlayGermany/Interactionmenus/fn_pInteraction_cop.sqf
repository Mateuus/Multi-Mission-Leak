...
if(!dialog) then {
	createDialog "d_pInteraction_cop";
};
...
if(_curTarget getVariable["restrained",false]) then {
	...
	_bUnrestrain ctrlEnable false;
	_bShowLicenses ctrlEnable false;
};
...
if(!(((player distance (getMarkerPos "cop_spawn_1") < 30) OR
  (player distance (getMarkerPos "cop_spawn_2") < 30) OR 
  (player distance (getMarkerPos "cop_spawn_3") < 30) OR 
  (player distance (getMarkerPos "cop_spawn_4") < 30) OR
  (player distance (getMarkerPos "cop_spawn_5") < 30)) && _curTarget getVariable["restrained",false])) then {
