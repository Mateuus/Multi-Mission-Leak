...
// enable igiLoad only for alac/medics
//if ((FETCH_CONST(life_alacLevel) < 1 && FETCH_CONST(life_thwLevel) < 1) || !license_civ_cargo) exitWith {};
...
_null = [Player] execVM "core\PlayGermany\IgiLoad\IgiLoad.sqf";
...

{
	if ((typeOf _x) in (IL_Supported_Vehicles_All)) then
	{
		...
		_null = [_x] execVM "core\PlayGermany\IgiLoad\IgiLoad.sqf";
		waitUntil {scriptDone _null};
	};
} forEach (vehicles);
