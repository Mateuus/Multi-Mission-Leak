...
*/
private ["_post","_num","_color","_action","_price","_smoke1","_smoke2","_light","_flare1","_flare2","_previousVol","_i","_camera","_heli","_jumpers","_smokeOn"];
_post = _this select 0;

if(playerSide != civilian) exitWith {hint "Du kannst nicht beim Skydiving teilnehmen während du im Dienst bist.";};

_price = 75000;

if(CASH < _price) exitWith {hint "Du hast nicht genug Geld dabei!";};

_action = [
	"Achtung, du wirst gleich in schwindelerregende Höhen geflogen (Kosten: 75.000€), weiter?",
	"Skydiving",
	"Ja",
	"Nein"
...
if(_action) then {
	...
	if (isNull _heli) then {
		...
		_heli setObjectTextureGlobal [0,"textures\civ\civ_huron_alg_0.paa"];
		_heli setObjectTextureGlobal [1,"textures\civ\civ_huron_alg_1.paa"];
		_heli animateDoor ['Door_R_source',1];
		_heli animateDoor ['Door_L_source',1];
		_heli animateDoor ['Door_rear_source',1];
	};
	[[_heli],"PGSERV_fnc_monitorHeli",false,false] spawn life_fnc_MP;
	...
	hint "Skydiving abgebrochen - vielleicht beim nächsten Mal?";
};