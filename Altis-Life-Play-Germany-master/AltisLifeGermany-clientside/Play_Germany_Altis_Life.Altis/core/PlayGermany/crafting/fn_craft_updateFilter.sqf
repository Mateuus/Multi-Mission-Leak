...
_config = [_itemFilter] call PG_fnc_craftCfg;
{
	if(_item == _x select 0)then
	{
		...
		for [{_i = 0},{_i < _invSize - 1},{_i = _i + 2}] do {
			_str = [format ["life_inv_%1",_matsNeed select _i]] call life_fnc_varToStr;
			_matsNum = _matsNeed select _i+1;
			//_str = localize getText(missionConfigfile>>"VirtualItems">>_matsNeed select _i>>"displayName");
