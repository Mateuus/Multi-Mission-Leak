...
if(_shooter isKindOf "Man" && alive player) then
{
	if(!life_isdowned) then
	{
		...
		[[0,format["%1 was incapacitated by rubber bullets.", name _unit, name _shooter]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		_unit attachTo [_obj,[0,0,0]];
		disableUserInput true;
		player setVariable["rubber",true,true];
		sleep 8;
		...
		detach player;
		life_isdowned = false;
		player setVariable["rubber",false,true];
