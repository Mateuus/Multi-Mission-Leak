...
if( !(isNull _cursorObject) && { player distance2D _cursorObject < 5 } && { (typeOf _cursorObject) isEqualTo "Land_Suitcase_F" } ) exitWith {
    [_cursorObject] call XY_fnc_pickupItem;
};
if( !(isNull _cursorObject) && { player distance2D _cursorObject < 5 } && { (typeOf _cursorObject) in ["Land_Money_F", "Land_Ammobox_rounds_F"] } ) exitWith {
...
if( _curTarget isKindOf "Man" ) exitWith {
    if( !(alive _curTarget) && { playerSide in [west, independent] } && { "Medikit" in (items player) } ) exitWith {
        if( playerSide == west && { (independent countSide playableUnits) > 0 } ) exitWith {
            hint "Bitte wende dich an die Sanitäter um jemanden zu reanimieren";
        };
        [_curTarget] call XY_fnc_revivePlayer;
    };
