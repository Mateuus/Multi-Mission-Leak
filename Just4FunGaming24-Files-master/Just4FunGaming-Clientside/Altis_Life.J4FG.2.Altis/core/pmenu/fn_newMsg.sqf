...
switch(_type) do {
    ...
    case 4: {
        ...
        _msg = format["%1 - Koordinaten %2", _msg, mapGridPosition player ];
        [objNull, _msg, player, 3] remoteExec ["XYDB_fnc_handleMessages", XYDB];
    ...
    //adminToPerson
    case 5: {
        if( XY_adminLevel < 1 ) exitWith {hint "Du bist kein Admin!";};
        if(isNULL XY_smartphoneTarget) exitWith {hint format["Keine Person ausgwählt!"];};
        if(_msg == "") exitWith {hint "Du musst eine Nachricht schreiben, die du schicken möchtest!";};
        [XY_smartphoneTarget, _msg, player, 4] remoteExec ["XYDB_fnc_handleMessages", XYDB];
    ...
    case 6: {
        ...
        if( XY_thwLevel < 3 ) then {
            ctrlShow[888903,false];
        } else {
            ctrlShow[888903,true];
        };
    ...
    //THWrequest
    case 8: {
        if(({side _x == east} count playableUnits) == 0) exitWith {hint format["Zurzeit ist kein THW im Dienst. Bitte probiere es später nochmal."];};
        ctrlShow[888900,false];
        if(_msg == "") exitWith {hint "Du musst eine Nachricht schreiben, die du schicken möchtes!";ctrlShow[888900,true];};
        _msg = format["%1 - Koordinaten %2", _msg, mapGridPosition player ];
        [objNull, _msg, player, 6] remoteExec ["XYDB_fnc_handleMessages", XYDB];
        _to = "An den THW";
        hint format["Du hast %1 eine Nachricht geschickt: %2",_to,_msg];
    ...
    // THW broadcast
    case 11: {
        if( XY_thwLevel < 3 ) exitWith {hint "Du bist kein qualifizierter THW!";};
        if(_msg == "") exitWith {hint "Du musst eine Nachricht schreiben, die du schicken möchtest!";};
        [objNull, _msg, player, 9] remoteExec ["XYDB_fnc_handleMessages", XYDB];
        hint format["THW-Meldung an ALLE: %1", _msg];
        closeDialog 887890;
    };
};