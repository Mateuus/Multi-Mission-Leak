...
waitUntil { !(isNil "XY_spawnedOnce") && { XY_spawnedOnce } };
    ...
    while{ true } do {
        ...
        if( !(_bp isEqualTo "") ) then {
            if( playerSide isEqualTo civilian ) then {
                _load = round( (getNumber(configFile >> "CfgVehicles" >> _bp >> "maximumload") / 8) * XY_ssv_backbackMultiplier );
            } else {
                _load = 175;
            };
        };

        XY_maxWeightCurrent = XY_maxWeightBase + _load;

        if( XY_carryWeight > XY_maxWeightCurrent ) then {
            if( !_notified )then {
                hint "Du trägst zu viel";
                _notified = true;
            };
            player setFatigue 1;
        } else {
...
"KidneyTracker" spawn { scriptName _this;
    while{true} do {
        ...
        waitUntil {sleep 5; (player getVariable "missingOrgan")};
