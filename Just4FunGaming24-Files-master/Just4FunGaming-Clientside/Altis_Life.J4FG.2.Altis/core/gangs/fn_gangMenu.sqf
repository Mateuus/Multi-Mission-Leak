...
private _ownerID = _group getVariable["gang.owner", ""];
if( _ownerID isEqualTo "" ) exitWith {
    closeDialog 0;
};
private _gangMaxMembers = _group getVariable[ "gang.maxMembers", 4 ];
private _gangMaxStorehouses = _group getVariable[ "gang.maxStorehouses", 0 ];
private _gangBank = _group getVariable[ "gang.bank", 0 ];
...
switch( _mode ) do {
    ...
    case "kick": {
...

        {
            if( (getPlayerUID _x) isEqualTo (_playerInfo select 0) ) exitWith {
                [ _x, _group ] remoteExec ["XY_fnc_gangKick", _x];
            };
        } forEach allPlayers;
    ...
    case "moderator": {
        ...
        _group setVariable["gang.moderators", _moderators, true];
        [_group] remoteExec ["XYDB_fnc_updateGang", XYDB];
        [ "refresh" ] call XY_fnc_gangMenu;
    };
    ...
    case "invite": {
        ...
        if( _perm && ((group _player) getVariable ["gang.id", -1]) >= 0 ) exitWith {
            hint "Der Spieler ist schon in einer Gang"
        };
        if( _player getVariable ["restrained", false] || _player getVariable["gang.invited", false] ) exitWith {
        ...
        _player setVariable["gang.invited", true, true];
    ...
    case "postDeposit": {
        ...
        if( _amount < 100 ) exitWith {
            hint "Du musst mindenstens 100€ einzahlen";
        };
    ...
    case "postWithdrawal": {
        private _amount = parseNumber( ctrlText ((findDisplay 2526) displayCtrl 2502) );
        if( _amount < 100 ) exitWith {
            hint "Du musst mindenstens 100€ abheben";
        };
    ...
    case "expandSlots": {
        ...
        private _upgradePrice = round(_targetMax * 250000);
        ...
        _group setVariable["gang.maxMembers", _targetMax, true];
        [_group] remoteExec ["XYDB_fnc_updateGang", XYDB];
