...
if( !(playerSide in [independent, west, east]) ) exitWith{};
...
while {true} do {
    ...
    if( _actionLevel > 0 && _actions isEqualTo [] ) then {
        ...
        _actions pushBack ( player addAction[ "<t color='#FF0000'>Demontieren</t>", {
            _objects = nearestObjects[player, [], 3];
            {
                if( _x getVariable["deployed", 0] == 1 ) exitWith {
                    deleteVehicle _x;
                };
            } forEach _objects;
        }, "", 0.5, false, true, "", "_objects = nearestObjects[player, [], 5]; _found = false; { if( _x getVariable[""deployed"", 0] isEqualTo 1 ) exitWith { _found = true; }; } forEach _objects; !XY_actionInUse && _found;" ] );
        ...
        _actions pushBack ( player addAction[ "<t color='#FF0000'>Alles demontieren (50m)</t>", {
            ...
            _objects = nearestObjects[player, [], 50];
            {
