...
if(!_preset) then {
    ...
    switch (_animate) do {
        ...
        case "reb_littlebird": {
            // AddDoors, AddBackseats, AddTread, AddTread_Short, BenchL_Up, BenchR_Up, AddHoldingFrame, AddGunHolder, AddGunHolder, AddCivilian_hide, AddMusicUnit	
            _vehicle animate ["AddDoors", 0];
            _vehicle animate ["AddBackseats", 1];
            _vehicle animate ["AddTread", 0];
            _vehicle animate ["AddTread_Short", 0];
            _vehicle animate ["BenchL_Up", 0];
            _vehicle animate ["BenchR_Up", 0];
            _vehicle animate ["AddHoldingFrame", 0];
            _vehicle animate ["AddGunHolder", 0];
            _vehicle animate ["AddCivilian_hide", 0];
            _vehicle animate ["AddMusicUnit", 0];
