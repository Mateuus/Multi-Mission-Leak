...
if( count ((group player) getVariable ["gang.name", ""]) < 1 ) exitWith {
...
if( !(_group isEqualTo grpNull) ) then {
    _gangName = _group getVariable ["gang.name",""];
    _action = [
...
[ [0, 1], format[ localize "STR_GNOTF_CaptureSuccess", profileName, (group player) getVariable "gang.name" ] ] remoteExec ["XY_fnc_broadcast"];