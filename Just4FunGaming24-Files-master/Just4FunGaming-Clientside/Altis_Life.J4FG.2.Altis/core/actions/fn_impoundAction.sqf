...
if( playerSide == west && { (_vehicle getVariable["side", sideUnknown]) != west } && { (east countSide playableUnits) > 0 } ) exitWith {
    hint "Beschlagnahmen von Fahrzeugen ist Aufgabe des THW, bitte rufe diesen zur Hilfe";
};
...
if( _costfulImpound && { !([10000] call XY_fnc_pay) } ) exitWith {
...
_price = switch (true) do {
    case(_vehicle isKindOf "Car"):  { 10000 };
    case(_vehicle isKindOf "Ship"): { 15000 };
    case(_vehicle isKindOf "Air"):  { 25000 };
};
