...
if( !(_vehicle getVariable["owner", ""] isEqualTo (getPlayerUID player)) && playerSide != east ) exitWith {
...
while { alive player && !XY_isTazed && !XY_interrupted && !(player getVariable["restrained", false]) && player distance _vehicle <= 10 } do {
...
if( _interrupted || { !([false, _item, 1] call XY_fnc_handleInv) } ) exitWith {
