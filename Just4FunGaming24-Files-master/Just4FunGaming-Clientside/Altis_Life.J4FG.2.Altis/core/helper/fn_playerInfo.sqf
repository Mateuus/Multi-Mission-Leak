...
// If we are a medic or THW, remove "clantag" from friendsname
if( playerSide isEqualTo independent ) then {
    _friendsName = [_friendsName, "[MEDIC]", ""] call KRON_Replace;
};
if( playerSide isEqualTo east ) then {
    _friendsName = [_friendsName, "[THW]", ""] call KRON_Replace;
};

switch( side _player ) do {
    case civilian: {
        _externalName = [_friendsName, "Maskiert"] select ([_player] call XY_fnc_playerMasked);
        _color = ["ECECEC", "7FD9AC"] select _isFriend;
    };
    case west: {
        _externalName = "Polizist";
        _color = "004D7F";
