...
_taxi setObjectTextureGlobal[0, "textures\vehicles\civ\m900\taxi.paa"];
...
while { (vehicle _player) != _taxi && _timeout > time } do {
    ...
    if( player distance _taxi < 2.5 && !XY_actionInUse && !XY_isTazed && alive player && !(player getVariable["restrained", false]) ) exitWith {
        player action ["GetInCargo", _taxi];
    };
};
