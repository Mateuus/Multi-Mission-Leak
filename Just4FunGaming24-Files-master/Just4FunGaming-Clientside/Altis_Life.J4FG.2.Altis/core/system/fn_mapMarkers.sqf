...
while { visibleMap || visibleGPS } do {
    ...
    // Add damaged vehicles for THW:
    if( playerSide == east ) then {

        {
            if( (getText(configFile >> "CfgVehicles" >> (typeOf _x) >> "vehicleClass")) in ["Car", "Air", "Ship"] ) then {

                if( (speed _x) < 1 && {
                    _x getHitPointDamage "HitEngine"   >= 0.7  ||
                    _x getHitPointDamage "HitLFWheel"  >= 0.98 ||
                    _x getHitPointDamage "HitLF2Wheel" >= 0.98 ||
                    _x getHitPointDamage "HitRFWheel"  >= 0.98 ||
                    _x getHitPointDamage "HitRF2Wheel" >= 0.98 }
                ) exitWith {

                    private _marker = createMarkerLocal [ format[ "%1_vehicle_marker", _x ], visiblePosition _x ];
                    _marker setMarkerColorLocal "ColorRed";
                    _marker setMarkerTypeLocal "o_med";
                    _marker setMarkerTextLocal format [ "Defektes Fahrzeug (%1)", _x getVariable["ownerName", "Unbekannt"] ];
                    _markers pushBack [_marker, _x];

                };
				if( (speed _x) < 1 && { (crew _x) isEqualTo [] } && { (serverTime - (_x getVariable[ "lastUseTime", serverTime ])) > 600 } ) then {
					private _marker = createMarkerLocal [ format[ "%1_vehicle_marker", _x ], visiblePosition _x ];
