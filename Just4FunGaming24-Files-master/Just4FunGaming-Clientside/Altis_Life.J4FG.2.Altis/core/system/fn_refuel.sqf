...
private _oilConfig = ["oilp"] call XY_fnc_itemConfig;
...
private _oilPrice = (_oilConfig select 4) / (_oilConfig select 1) / XY_ssv_FPD;
// round price to 1 dec
_oilPrice = (round(_oilPrice * 10)) / 10;
// cap to 1...50
_oilPrice = (1 max (50 min _oilPrice));

if( _isAir ) then {
    _oilPrice = _oilPrice * XY_ssv_FPAF;
};

while { _fuel < 1 && alive player && player == vehicle player && !XY_isTazed && !(player getVariable["restrained", false]) && player distance _vehicle <= _maxDistance && !(isEngineOn _vehicle) } do {
...
if( _isAir ) then {
    ...
    {
        if( _x > 0 ) then {
            _repairCosts = _repairCosts + 2500;
        };
