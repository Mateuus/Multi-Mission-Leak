...
if( (player distance (getMarkerPos "poi_pvp_zone")) < 2200 ) exitWith {
...
if( _mode isEqualTo "init" ) exitWith {
...

    {
        ...
        if( _distance > 0.5 && ( ((_x select 0) find "_reb" < 0) || license_civ_rebel )) then {

            private _price = (round(10000 + (ceil(_distance) * 1250)));
            // Cheaper for noobies
...
if( _mode isEqualTo "call" ) exitWith {
    ...
    [ player, _destination, _price ] spawn XY_fnc_callTaxi;
    XY_taxiCalled = time;
