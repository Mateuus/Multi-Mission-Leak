...
// Return norm price for resource 
private _fnc_normPrice = { 
    ...
    _normPrice * (([_item] call XY_fnc_sourceConfig) select 1); 
}; 
...
while { true } do { 
    ...
    { 
        ...
        private _normPrice = [_shortName] call _fnc_normPrice; 
        ...
        if( _salesVolume > 0 ) then { 
            ...
            [format["INSERT INTO volumelog (resource, volume) VALUE ('%1', '%2')", _shortName, [_volumeRow select 1] call XYDB_fnc_numberSafe ], 1] remoteExec ["XYDB_fnc_asyncCall", XYDB]; 
 
            private _volumeFactor = switch(true) do { 
                case(_price > _normPrice * 1.7): { 3.25 }; 
                case(_price > _normPrice * 1.5): { 2.25 }; 
                case(_price > _normPrice): { 2 }; 
                default { 1.25 }; 
            }; 
 
            private _newPrice = (1 + (random 100)) max (_price - (_volume * _volumeFactor)); 
 
            if( _shortName isEqualTo "copper_r" ) then { 
                // noob-friendly min. price for copper, masked by some randomness 
                _newPrice = ((800 + (random 250)) max _newPrice); 
            }; 
 
            _lowCount = _lowCount + 1; 
 
            _mod = round( _newPrice - _price ); 
            ...
            if( _price < _normPrice ) then { 
                ...
                if( _price > ( _normPrice * 1.5 ) ) then { 
                    _mod = (count allPlayers) * ((random 0.75) max 0.25); 
                    _branch = "> 150%"; 
                } else { 
                    _mod = (count allPlayers) * 1.65; 
                    _branch = ">= NORM"; 
                }; 
    ...
    // Save changes 
    { 
        [ format["INSERT INTO pricelog (resource, price) VALUE ('%1', '%2')", _x select 0, [_x select 1] call XYDB_fnc_numberSafe ], 1] remoteExec ["XYDB_fnc_asyncCall", XYDB]; 
    } forEach XY_marketPrices; 
