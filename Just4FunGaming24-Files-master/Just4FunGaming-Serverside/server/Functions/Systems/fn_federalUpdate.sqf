...
while {true} do { 
    ...
    if( !(fed_bank getVariable["chargeplaced", false]) && !(fed_bank getVariable["safe_open", false]) )then { 
        ...
        private _random = random ((count playableUnits) / 3.0); 
        private _newFunds = 180 min round((fed_bank getVariable["safe", 0]) + _random); 
        diag_log format["FUNDS: %1, RANDOM: %2, NF: %3", fed_bank getVariable["safe", 0], _random, _newFunds ]; 
        fed_bank setVariable["safe", _newFunds, true]; 
 
        _counter = _counter + 1; 
        if( (_counter mod 3) isEqualTo 0 ) then { 
