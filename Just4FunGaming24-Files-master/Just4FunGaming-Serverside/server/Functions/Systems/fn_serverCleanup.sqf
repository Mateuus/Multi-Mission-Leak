...
private _fnc_processQueue = { 
    ...
    { 
        ...
        if( (_time - _found) > 1800 || (((_time - _found) > 300) && !_isVehicle) )then { 
            ...
            if( _isVehicle ) then { 
                private _vid = _item getVariable["dbInfo", -1]; 
                if( _vid >= 0 ) then { 
                    diag_log format ["<CLEANUP> Returning disabled vehicle: %1", _vid]; 
                    [_item] call XYDB_fnc_updateVehicle; 
                    [ format[ "UPDATE vehicles SET active = '0', ts_modified = CURRENT_TIMESTAMP WHERE id = '%1'", _vid ], 1] call XYDB_fnc_asyncCall; 
                    _item setVariable["dbInfo", -1, true]; 
                }; 
            }; 
            deleteVehicle _item; 
    ...
    if( (XYDB isEqualTo 2 && isServer) || (XYDB != 2 && !isServer) ) then { 
        ...
        { 
            if( (_x isKindOf "Car" || _x isKindOf "Air" || _x isKindOf "Ship") && _x getVariable["despawn", true] ) then { 
                ...
                _vid = _x getVariable["dbInfo", -1]; 
                if( _vid >= 0 ) then { 
                    diag_log format [ "<CLEANUP> Destroyed vehicle: %1 (%2) @ %3, insured: %4", _vid, _x, mapGridPosition _x, _x getVariable [ "insured", false ] ]; 
                    if( _x getVariable [ "insured", false ] ) then { 
                        [ format[ "UPDATE vehicles SET damage = '', inventory = '""[]""', ts_modified = CURRENT_TIMESTAMP WHERE id = '%1'", _vid ], 1] call XYDB_fnc_asyncCall; 
                    } else { 
                        [ format[ "UPDATE vehicles SET active = '0', alive = '0', inventory = '""[]""', ts_modified = CURRENT_TIMESTAMP WHERE id = '%1'", _vid ], 1] call XYDB_fnc_asyncCall; 
                    }; 
                }; 
                _x setVariable["dbInfo", -1, true]; 
