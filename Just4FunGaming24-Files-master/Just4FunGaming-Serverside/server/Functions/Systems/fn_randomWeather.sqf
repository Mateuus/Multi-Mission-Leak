...
if( (XY_ssv_enableSnow isEqualTo 1) && (XY_ssv_enableRain isEqualTo 0) ) exitWith { 
    ...
    private _weather = floor random 3; // 0 = fair, 1 = cloudy, 2 = snow 
    while { true } do { 
        ...
        if( time >= _nextChange ) then { 
            ...
            _weather = 0 max (_weather min 2); 
            _nextChange = time + 1200; 
        ...
        switch (_weather) do { 
            ...
            case 2: { 
                ...
                300 setFog (0.25 + random 0.25); 
            }; 
        }; 
 
        XY_snow = 0 max (1 min ([ 0, XY_snow + ((random 0.5) - 0.25) ] select (_weather isEqualTo 2 && (overcast > 0.5 || fog >  0.25)))); 
...
while {true} do { 
    ...
    call (_weather call BIS_fnc_selectRandom); 
