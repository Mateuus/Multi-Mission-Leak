...
_playerID = [ _this, 0, "", [""] ] call BIS_fnc_param; 
 
if( _playerID == "" ) exitWith { [] }; 
 
_query = format["SELECT pid, pos, inventory, containers FROM houses WHERE pid = '%1' AND owned = '1'", _playerID]; 
_houses = [_query, 2, true] call XYDB_fnc_asyncCall; 
 
_return = []; 
{ 
    _pos = call compile format["%1", _x select 1]; 
    _house = nearestBuilding _pos; 
    [ _house, false ] remoteExecCall [ "allowDamage" ]; 
    ...
    _trunk = [_x select 2] call XYDB_fnc_mresToArray; 
    if( typeName _trunk == "STRING" ) then { 
        _trunk = call compile format["%1", _trunk]; 
    }; 
 
    _containerData = [_x select 3] call XYDB_fnc_mresToArray; 
    ...
    { 
        ...
        [ _container, false ] remoteExecCall [ "allowDamage" ]; 
