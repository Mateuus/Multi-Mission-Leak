...
lhm_clothing_filter = _selection;

if(_selection == 4) then
{
	lhm_shop_cam camSetTarget (player modelToWorld [0,-.15,1.3]);
	lhm_shop_cam camSetPos (player modelToWorld [1,-4,2]);
	lhm_shop_cam camCommit 1;
}
	else
{
...
_clothes = switch (lhm_clothing_store) do
{
	case "bruce": {[_selection] call lhm_fnc_clothing_bruce;};
	case "cop": {[_selection] call lhm_fnc_clothing_cop;};
	case "reb": {[_selection] call lhm_fnc_clothing_reb;};
	case "dive": {[_selection] call lhm_fnc_clothing_dive;};
	case "kart": {[_selection] call lhm_fnc_clothing_kart;};
	case "sec": {[_selection] call lhm_fnc_clothing_sec;};
	case "medic": {[_selection] call lhm_fnc_clothing_med;};
	default {[_selection] call lhm_fnc_clothing_bruce;};
};

if(count _clothes == 0) exitWith {};
{
	_details = [_x select 0] call lhm_fnc_fetchCfgDetails;
