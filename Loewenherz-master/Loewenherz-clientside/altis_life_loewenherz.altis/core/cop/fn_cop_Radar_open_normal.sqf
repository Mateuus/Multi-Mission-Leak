...
private ["_display","_lBOX","_map","_btn_radar_small","_objects"];
if(vehicle player == player) exitWith {};
if(playerSide != west) exitWith {hint "Du bist kein Cop"};
if(!(typeof (vehicle player) in ["B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F"])) exitWith {hint "Das geht nicht in diesem Helikopter"};
disableSerialization;

closeDialog 0;
createDialog "Radar_heli";

waitUntil {!isnull(findDisplay 5488)};
