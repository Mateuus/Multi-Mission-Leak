...
[] spawn {
    ...
    for "_i" from 0 to 1 step 0 do {
        ...
        if (!([west,getPos player,30] call lhm_fnc_nearUnits) && (player getVariable ["restrained",false]) && isNull objectParent player) exitWith {
...
while {player getVariable  "restrained"} do {
    ...
    if (vehicle player != player && lhm_disable_getIn) then {
    ...
    if (isNull objectParent player && lhm_disable_getOut) then {
        player moveInCargo _vehicle;
    };

    if ((vehicle player != player) && lhm_disable_getOut && (driver (vehicle player) isEqualTo player)) then {
        player action["eject",vehicle player];
        player moveInCargo _vehicle;
    };

    if (vehicle player != player && lhm_disable_getOut) then {
