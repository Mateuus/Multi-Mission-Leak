        ...
        while {king_gamble_get_data_thread_is_running == 1} do
        {
            ...
            if(king_gamble_player_is_blocked == 1) then
            {
                ...
                else
                {
					...
					_playButton = _disp displayCtrl 5105;
            ...
            if(_jackpot != -1) then
            {
                _gambling_jackpot_text ctrlSetStructuredText parseText format["Jackpot: $%1", [_jackpot] call lhm_fnc_numberText];
            }
            else
            {
...
_dice_rules1 ctrlSetStructuredText parseText format["Sum 7 = x%1", king_gamble_dice_win_factor_sum_seven select king_gamble_casino_idx];
_dice_rules2 ctrlSetStructuredText parseText format["6er Double = x%1", king_gamble_dice_win_factor_two_of_a_kind select king_gamble_casino_idx];

_bandit_rules1 ctrlSetStructuredText parseText format["two equal = x%1", king_gamble_bandit_win_factor_two_of_a_kind select king_gamble_casino_idx];
_bandit_rules2 ctrlSetStructuredText parseText format["three equal = x%1", king_gamble_bandit_win_factor_three_of_a_kind select king_gamble_casino_idx];
...
_text_your_money ctrlSetStructuredText parseText format["<img size='1.6' image='core\textures\icons\misc\money.paa'/> $ %1",[lhm_cash] call lhm_fnc_numberText];
