...
if (!isNull _house) then {
    _message = 1;
    if (([player] call lhm_fnc_PlayerInBuilding) && {([_container] call lhm_fnc_PlayerInBuilding)}) then {
        _message = 2;
        if ((_house in lhm_vehicles) && !(isNil {_house getVariable "house_owner"})) then {
            _message = 3;
            if (!_isFloating) then {
                _message = 4;
                ...
                if (count _containers < (_houseCfg select 1)) then {
                    _isPlaced = true;
                    //if (lhm_HC_isActive) then {
                        //[_uid,_container] remoteExec ["HC_fnc_addContainer",HC_lhm];
                    //} else {
                        //[_uid,_container] remoteExec ["TON_fnc_addContainer",RSERV];
                    //};
					[[_uid,_container],"TON_fnc_addContainer",false,false] call lhm_fnc_mp;
                    _container setVariable ["Trunk",[[],0],true];
                    _container setVariable ["container_owner",[_uid],true];
                    _containers pushBack _container;
                    _house setVariable["containers",_containers,true];
                    //sleep 1;
					[[_container],"lhm_fnc_simDisable",true,false] call lhm_fnc_mp;
                };
            };
        };
    };
...
[true,_type,1] call lhm_fnc_handleInv;
