...
if(isNull _house) exitWith {life_action_inUse = false;};
if(!(_house isKindOf "House_F")) exitWith {life_action_inUse = false;};
if(isNil {_house getVariable "house_owner"}) exitWith {hint "There is no owner for this house.";life_action_inUse = false;};
closeDialog 0;

_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(EQUAL(count _houseCfg,0)) exitWith {life_action_inUse = false;};
