...
if(!(EQUAL(_shopSide,""))) then {
	_flag = switch(playerSide) do {case west: {"cop"}; case independent: {"med"}; case east: {"armer"}; default {"civ"};};
...
(findDisplay 3100) displaySetEventHandler ["KeyDown","if((_this select 1) == 1) then {closeDialog 0; [] call life_fnc_skin;}"]; //Fix Custom Skin after ESC
...
if((SEL(_this,3) in ["armer"] && playerSide != east)) exitWith {hint localize "STR_Shop_NotaArmer"; closeDialog 0;};
...
life_shop_cam = "CAMERA" camCreate getPosWorld player;
