...
switch(playerSide) do {
  ...
  case east: {
		[_vehicle,"armer_offroad",true] spawn life_fnc_vehicleAnimate;
	};
};
...
if(_mode) then {
	if(!(_className in ["B_G_Offroad_01_armed_F"])) then {
		if(life_job > 0 && life_adac_onduty) then {
			[[(getPlayerUID player),east,_vehicle,_colorIndex],"TON_fnc_vehicleCreate",false,false] spawn life_fnc_MP;
		} else {
