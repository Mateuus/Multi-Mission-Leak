...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if(({alive _x} count crew _vehicle) == 0) then
	{
		...
		switch (true) do
		{
			case (_vehicle isKindOf "Car"): {_price = 10000;};
			case (_vehicle isKindOf "Ship"): {_price = 15000;};
			case (_vehicle isKindOf "Air"): {_price = 20000;};
		};
        ...
        // <Original>%1 a mis en fourrière le %3 de %2.</Original>
		...
		/*_unit = (_vehicleData select 0) select 1;
		_playerID = getPlayerUID player;
		_playerName = name player;
		_byPlayerID = getPlayerUID _unit;
		_byPlayerName = (_vehicleData select 0) select 1;
		_type = 5;
		_amount = _vehicleName;
