...
switch (_code) do {
		...
		switch (playerSide) do {
			...
			case civilian: {if(!visibleMap) then {
					...
					if(life_job > 0 && life_adac_onduty)then{
						[] spawn life_fnc_adacMarker;
					};
				};
			};
			case east: {if(!visibleMap) then {[] spawn life_fnc_armerMarkers;}};
		};
	};
	...
	case 19: {
		...
		if(_shift && (playerSide == west || playerside == east) && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {(side cursorTarget in [civilian,independent])} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget GVAR "Escorting")} && {!(cursorTarget GVAR "restrained")} && {speed cursorTarget < 1}) then {
			[] call life_fnc_restrainAction;
		};
		if(_shift && playerSide != west && life_knockout && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget GVAR "Escorting")} && {!(cursorTarget GVAR "restrained")} && {speed cursorTarget < 1}) then {
			if (ITEM_VALUE("menottes") > 0) then 	// On connait l'item (menottes) on prends ça valeur et on compare directement (pas de variable de mort)
			{
				[] call life_fnc_restrainAction; 		// Boom terminé
	...
	case 34: {
		...
		if(_shift && playerSide in [civilian,east] && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then {
			...
			if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player GVAR ["restrained",false]) && !life_istazed && !life_isknocked) then {
	...
	case 38: {
		...
		if(_shift && playerSide == civilian && life_adac_onduty) then {
			if(_veh != player && (typeOf _veh) in ["C_Offroad_01_F","C_SUV_01_F","B_Truck_01_mover_F"]) then {
	...
	case 21: {
		...
		if((player GVAR ["restrained",true]) || (player GVAR ["cagoule",true]) || (player GVAR ["baillon",true])) then {
			hint "Tu ne peux pas voir ton inventaire pour l'instant !";};
		};
	};
	...
	case 33: {
		if(_shift) then {
            if(playerSide == west && vehicle player != player && !life_siren_active2 && ((driver vehicle player) == player)) then {
                [] spawn {
        ...
        if (!_shift) then {
			...
			if((_veh GVAR "siren")) then {
				...
				switch(playerSide) do {
					case west: { [_veh] remoteExec ["life_fnc_copSiren",RCLIENT]; };
					case independent: { [_veh] remoteExec ["life_fnc_medicSiren",RCLIENT]; };
					case east: { [_veh] remoteExec ["life_fnc_armerSiren",RCLIENT]; };
				};
			};
        };
    };
	...
	case 24: {
		if(_shift) then {
			if (soundVolume != 1) then {
				1 fadeSound 1;
				hint composeText [ image "icons\bq_off.paa", " Vous avez enlevé vos boules quies" ];
			} else {
				1 fadeSound 0.1;
				hint composeText [ image "icons\bq_on.paa", " Vous avez mis vos boules quies" ];
			};
		};
			...
			if(_veh isKindOf "House_F" && {playerSide == civilian}) then {
				if(_veh in life_vehicles && player distance _veh < 8) then {
					...
					if(EQUAL(_locked,0)) then {
						...
						hint composeText [ image "icons\lock.paa", "  Porte vérouillée" ];
					} else {
						_veh SVAR [format["bis_disabled_Door_%1",_door],0,true];
						_veh animate [format["door_%1_rot",_door],1];
						hint composeText [ image "icons\unlock.paa", "  Porte déverouillée" ];
					};
				};
			} else {
				_locked = locked _veh;
	...
	case 46:
    {
        if(_shift && (!life_action_inUse) && (isNull objectParent player) ) then
        {
			if (ITEM_VALUE("pickaxe") > 0) then 	// On connait l'item (pickaxe) on prends ça valeur et on compare directement (pas de variable de mort)
			{
				[] spawn life_fnc_pickAxeUse; 		// Boom terminé
			};

			if (ITEM_VALUE("hache") > 0) then 	// On connait l'item (hache) on prends ça valeur et on compare directement (pas de variable de mort)
			{
				[] spawn life_fnc_AxeUse; 		// Boom terminé
			};
        };
    };
