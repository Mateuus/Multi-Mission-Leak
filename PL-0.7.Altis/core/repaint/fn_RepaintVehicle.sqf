...
if(!isNull (findDisplay 2300)) then {_veh = nearestObject [getPosWorld player, "LandVehicle"];
_color = lbcursel 2303;_color_index = lbValue[2303,_color];
closeDialog 0;_baseprice = 10000;  
//Price for repaint
...
if(isNil {_vehicleData}) exitWith {hint "Cheated car?."};if ((getPlayerUID player) != _vehOwner) exitWith {hint "You aren't the owner!"};		
...
titleText["Cancelled","PLAIN"]; life_action_inUse = false;};		
...
//Sync silently because it's obviously silently..			
