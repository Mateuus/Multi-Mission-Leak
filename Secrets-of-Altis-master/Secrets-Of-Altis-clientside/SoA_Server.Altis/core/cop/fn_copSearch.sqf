...
private["_civ","_invs","_license","_robber","_guns","_gun","_price"];
...
{
	{
		...
		//_index = [_x select 0,__GETC__(sell_array)] call SOA_fnc_index;
		_price = [_x select 0,playerside] call life_fnc_classPrices;
		if(_price != -1) then
		{
			_illegal = _illegal + ((_x select 1) * (_price/2));
			//_illegal = _illegal + ((_x select 1) * ((__GETC__(sell_array) select _index) select 1));
		};
	} foreach _invs;
	if(_illegal > 6000) then
	{
		[getPlayerUID _civ,_civ getVariable["realname",name _civ],"482"] remoteExecCall ["SOA_fnc_wantedAdd",2];
	};
	
	[getPlayerUID _civ,_civ getVariable["realname",name _civ],"481"] remoteExecCall ["SOA_fnc_wantedAdd",2];
	...
	life_beatbankgeld = life_beatbankgeld + _illegal;
}
	else
{
...
if(_robber) then
{
	[0,"STR_Cop_Robber",true,[(_civ getVariable["realname",name _civ])]] remoteExecCall ["life_fnc_broadcast",0];
};