...
if(_ownerID == "") exitWith {closeDialog 0;}; //Bad jojo
...
if(_ownerID != steamid) then {
	...
	(getControl(2620,2634)) ctrlEnable false; //Gang-Tag
	(getControl(2620,2635)) ctrlEnable false; //Gang-Tag-Button	
};

(getControl(2620,2629)) ctrlSetText format["%1 verwalten",_gangName];
(getControl(2620,601)) ctrlSetText format["%1€",[_gangBank] call life_fnc_numberText];
(getControl(2620,2634)) ctrlSetText format["%1",(grpPlayer getVariable ["gang_tag","Dein Gang-Tag"])];
...
lbClear _members;
{
	if((getPlayerUID _x) == _ownerID) then {
		...
		_members lbSetPicture [(lbSize _members)-1,"icons\ganglead.paa"];
		_members lbSetData [(lbSize _members)-1,str(_x)];
	} else {
		_members lbAdd format["%1",(_x getVariable["realname",name _x])];
		_members lbSetPicture [(lbSize _members)-1,"icons\gang.paa"];
		_members lbSetData [(lbSize _members)-1,str(_x)];
	};
