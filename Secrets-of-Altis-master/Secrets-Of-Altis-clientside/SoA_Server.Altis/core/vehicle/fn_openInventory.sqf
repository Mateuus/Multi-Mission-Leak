...
if(isNull _vehicle OR !(_vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "House_F" OR _vehicle isKindof "Land_Cargo40_military_green_F" OR _vehicle isKindOf "Land_Suitcase_F")) exitWith {}; //Either a null or invalid vehicle type.
if(_vehicle iskindOf "Land_Suitcase_F" && (_vehicle getVariable ["Trunk",[]]) isEqualTo []) exitWith {};

//Clan actions
if(_vehicle == esc_container && !license_civ_esc) exitWith {hint "Du bist nicht berechtigt auf diesen Clan-Container zuzugreifen"};
if(_vehicle == ims_container && !license_civ_ims) exitWith {hint "Du bist nicht berechtigt auf diesen Clan-Container zuzugreifen"};
if(_vehicle == imf_container && !license_civ_imf) exitWith {hint "Du bist nicht berechtigt auf diesen Clan-Container zuzugreifen"};
if(_vehicle == slg_container && !license_civ_slg) exitWith {hint "Du bist nicht berechtigt auf diesen Clan-Container zuzugreifen"};
...
_vehicle spawn
{
	...
	if(_this isKindOf "House_F") then {
		[_this] remoteExecCall ["SOA_fnc_updateHouseTrunk",2];
	};

	if(_this isKindOf "Land_Suitcase_F") then {
		if(_this getVariable["Trunk",[[],0]] isEqualTo [[],0]) then {deletevehicle _this};
	};
};