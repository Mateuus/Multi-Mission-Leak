...
if(!([_num] call SOA_fnc_isnumber)) exitWith {hint localize "STR_MISC_WrongNumFormat";};
_num = parseNumber(_num);
if(_num < 1) exitWith {hint localize "STR_MISC_Under1";};

_index = [_ctrl,((life_trunk_vehicle getVariable "Trunk") select 0)] call SOA_fnc_index;
...
if(_mode == 0) then {
	...
	if([true,_ctrl,_ctrlintparsed] call life_fnc_handleInv) then
	{
		if(_num == _value) then
		{
			_data set[_index,-1];
		...
		{
			_data set[_index,[_ctrl,(_value - _ctrlintparsed)]];
		};
		life_trunk_vehicle setVariable["Trunk",[_data,(_old select 1) - _weight],true];
		[life_trunk_vehicle] call life_fnc_vehInventory;
	}
