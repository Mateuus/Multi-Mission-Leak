...
if(fuel _vehicle <= 0.1) exitWith {titleText[localize "STR_NOTF_OutOfFuel","PLAIN"]; closeDialog 0;};
...
_resourceZones = ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","mohn_1","coca","hanf_1","lead_1","iron_1","salt_1","sand_1","oil_1","oil_2","rock_1","rye_1","hops_1","yeast_1","plutonium_area","copper_1"];
_zone = "";
...
_item = switch(true) do {
	...
	case (_zone in ["mohn_1"]): {"heroinu"};
	case (_zone in ["coca"]): {"cocaine"};
	case (_zone in ["hanf_1"]): {"cannabis"};
	case (_zone in ["copper_1"]): {"copperore"};
	...
	case (_zone in ["plutonium_area"]): {"plutoniumu"};
	default {""};
};

if(_item == "") exitWith {hint "Dieser Rohstoff ist mit dem Gerät nicht abbaubar!"; life_action_inUse = false;};
_vehicle setVariable ["mining",true,true]; //Lock the device
_vehicle spawn life_fnc_soundDevice;
...
while {true} do {
	...
	if(fuel _vehicle <= 0.1) exitWith {titleText[localize "STR_NOTF_OutOfFuel","PLAIN"];};
