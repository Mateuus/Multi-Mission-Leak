/*
	File: fn_vehInventory.sqf
	Author: Bryan "Tonic" Boardwine

	Description:
	Used a refresher for the vehicle inventory / trunk menu items.
*/
private["_veh","_tInv","_pInv","_veh_data","_icon"];
_veh = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
if(isNull _veh OR !alive _veh) exitWith {closeDialog 0;}; //If null / dead exit menu
disableSerialization;
if(life_action_inUse) exitWith {closedialog 0; closedialog 0;};
if(life_action_gathering) exitWith {closedialog 0; closedialog 0;};

_tInv = (findDisplay 3500) displayCtrl 3502;
_pInv = (findDisplay 3500) displayCtrl 3503;
lbClear _tInv;
lbClear _pInv;

if(_veh isKindOf "House_F") then {
	private["_mWeight"];
	_mWeight = 0;
	{_mWeight = _mWeight + ([(typeOf _x)] call life_fnc_vehicleWeightCfg);} foreach (_veh getVariable["containers",[]]);
	_veh_data = [_mWeight,(_veh getVariable["Trunk",[[],0]]) select 1];
} else {
	_veh_data = [_veh] call life_fnc_vehicleWeight;
};

if(_veh_data select 0 == -1) exitWith {closeDialog 0};

ctrlSetText[3504,format["Kapazität: %1/%2",_veh_data select 1,_veh_data select 0]];
_data = _veh getVariable ["Trunk",[]];
if(count _data == 0) then {_veh setVariable["Trunk",[[],0],true]; _data = [];} else {_data = _data select 0;};
//Player Inventory Items
{
	//Money Handle
	if(_x != "life_beatgeld") then
	{
		_val = missionNameSpace getVariable ((life_itemInfo select _forEachIndex) select 0);
		if(_val > 0) then
		{
			_pInv lbAdd format["[%1] - %2",_val,((life_itemInfo select _forEachIndex) select 1)];
			_pInv lbSetData [(lbSize _pInv)-1,((life_itemInfo select _forEachIndex) select 2)];
			_pInv lbSetPicture [(lbSize _pInv)-1,((life_itemInfo select _forEachIndex) select 3)];
			_pInv lbSetColor [(lbSize _pInv)-1,((life_itemInfo select _forEachIndex) select 4) select 1];
			_pInv lbSetTooltip [(lbSize _pInv)-1,((life_itemInfo select _forEachIndex) select 5)];
		};
	}
		else
	{
		if(life_beatgeld > 0) then
		{
			_pInv lbAdd format["%1€",[life_beatgeld] call life_fnc_numberText];
			_pInv lbSetData [(lbSize _pInv)-1,"money"];
		};
	};
} foreach life_inv_items;

//Trunk Inventory Items
{
	if((_x select 0) != "money") then
	{
		_var = [_x select 0,0] call life_fnc_varHandle;
		_name = [_var] call life_fnc_varToStr;
		_icon = [_var] call life_fnc_itemIcon;
		_val = _x select 1;
		if(_val > 0) then
		{
			_tInv lbAdd format["[%1] - %2",_val,_name];
			_tInv lbSetData [(lbSize _tInv)-1,_x select 0];
			_tInv lbSetPicture [(lbSize _tInv)-1,_icon];
		};
	}
		else
	{
		_val = _x select 1;
		if(_val > 0) then
		{
			_tInv lbAdd format["%1€",[_val] call life_fnc_numberText];
			_tInv lbSetData [(lbSize _pInv)-1,"money"];
		};
	};
} foreach _data;