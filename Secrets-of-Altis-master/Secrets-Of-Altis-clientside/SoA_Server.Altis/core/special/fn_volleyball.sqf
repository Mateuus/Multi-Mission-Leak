...
switch(_mode) do {
	...
	//Volleyball rausholen
	case 0: 
	{
		if(!isnull volleyball) exitWith {hint "Du hast deinen Volleyball bereits ausgepackt!";};
		[false,"volleyball",1] call life_fnc_handleinv;

		volleyball = "land_volleyball_01_f" createvehicle [0,0,0];
		volleyball addaction ["Volleyball nehmen",{player playaction "putdown"; sleep 1.2; _this select 0 attachto[player,[0,0,0],"righthand"]; volleythrow = (finddisplay 46) displayAddEventHandler ["keyDown","if ((_this select 1) == 19) then {[1,cursorobject] spawn life_fnc_volleyball; true};"];},0,0,false,false,'',"currentWeapon player == '' && isnull (attachedTo volleyball)",3];
		volleydelete = player addaction ["Volleyball einstecken",{if(isnull volleyball) exitWith {}; deletevehicle volleyball; player removeAction volleydelete; [true,"volleyball",1] call life_fnc_handleinv;}];
	...
	case 1: 
	{
		...
		if(currentWeapon _target != "") exitWith {hint "Die Person hält bereits etwas in der Hand!";};
