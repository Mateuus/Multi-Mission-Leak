...
if(isNil {_house getVariable "house_owner"}) exitWith {hint "Das Haus besitzt keinen Eigentümer!"};
closeDialog 0;
...
if(_action) then {
	...
	[_house] remoteExecCall ["SOA_fnc_sellHouse",2];
	...
	_sellmoney = (round((_houseCfg select 0)/2));
	
	life_beatbankgeld = life_beatbankgeld + _sellmoney;
	...
	_index = [str(getPosATL _house),life_houses] call SOA_fnc_index;
	if(_index != -1) then {
	...
	[name player,[_sellmoney] call life_fnc_numberText,9] remoteExecCall ["life_fnc_logdat",2];
};
