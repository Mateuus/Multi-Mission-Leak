...
if(!(["life_fnc_",_functionName] call BIS_fnc_inString) && {!(["SPY_fnc_",_functionName] call BIS_fnc_inString)} && {!(["DB_fnc_",_functionName] call BIS_fnc_inString)} && {!(["SOA_fnc_",_functionName] call BIS_fnc_inString)} &&
...
if (ismultiplayer && _mode == 0) then {
	if (isserver) then {
		if (typename _target == typename []) then {
			...
			life_fnc_BEAT_execMP = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];

			if (_ownerID < 0) then {
				publicvariable "life_fnc_BEAT_execMP";
			} else {
				if (_ownerID != _serverID) then {
					_ownerID publicvariableclient "life_fnc_BEAT_execMP";
				};
			};

			if (_ownerID < 0 || _ownerID == _serverID) then {
				["life_fnc_BEAT_execMP",life_fnc_BEAT_execMP] spawn life_fnc_MPexec;
			};
