...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart","winter","bandit"] && playerSide != civilian) exitWith {hint localize "STR_Shop_NotaCiv"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
if((_this select 3) == "donator" && (__GETC__(life_donator) == 0)) exitWith {hint "Du bist kein Freund von Don!"; closeDialog 0;};
if((_this select 3) == "gruppe" && !license_civ_gruppe) exitWith {hint "Du besitzt keine Gruppierungskauflizenz!"; closeDialog 0;};
if((_this select 3) == "presse" && !license_civ_presse) exitWith {hint "Du besitzt keinen Presseausweis!"; closeDialog 0;};
if((_this select 3) == "jagd" && !license_civ_hunter) exitWith {hint "Du besitzt keinen Jagdschein!"; closeDialog 0;};
if((_this select 3) == "cop" && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) == "cop_krawall" && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) == "dive" && !license_civ_dive) exitWith {hint localize "STR_Shop_NotaDive"; closeDialog 0;};
if((_this select 3) == "med" && playerSide != independent) exitWith {hint "Du gehörst nicht zum AHW"; closeDialog 0;};
if((_this select 3) == "president" && !license_civ_president) exitWith {hint "Du bist nicht der Präsident von Altis!"; closeDialog 0;};
...
_pos = [3326.09,13047.30,19.9];  
_oldDir = getDir player;  
_oldPos = visiblePositionASL player;  
_testLogic = "Logic" createVehicleLocal _pos; 
_flag = "Flag_Blueking_F" createVehicleLocal [3327,13050,17.4038];
_flag setposATL [3327,13050,17.4038];
_flag setflagTexture "images\beat_fahne.paa";
_testlogic setdir 90; 
_testLogic setPosATL _pos;    
_light = "#lightpoint" createVehicleLocal _pos;    
_light setLightBrightness 1;    
_light setLightAmbient [1.0, 1.0, 1.0];  
_light lightAttachObject [_testLogic, [0,0,10]];   
...
life_shop_cam camSetPos (player modelToWorld [2,4,1]); 
life_shop_cam camSetFOV .77; 
