...
switch (_index) do
{
	case 0:
	{
		...
		*/
		...
		for "_i" from 1 to (count _config)-1 do 
		{
			_item = _config select _i;

			_itemInfo = [_item select 0] call life_fnc_fetchCfgDetails;
			_itemList lbAdd format["%1",if(isNil {_item select 1}) then {_itemInfo select 1} else {_item select 1}];
			_itemList lbSetData[(lbSize _itemList)-1,_itemInfo select 0];
			_itemList lbSetPicture[(lbSize _itemList)-1,_itemInfo select 2];
			_itemList lbSetValue[(lbSize _itemList)-1,[_item select 0,playerside] call life_fnc_classPrices];
		};

		lbSortByValue _itemList;
