class RPP_PoliceVehicleDB
{
    idd = 30045;
    movingenable = true;
    onLoad = "uiNamespace setVariable ['RPP_PoliceVehicleDB', (_this select 0)]";
	
	class Controls
	{

class background: RPP_RscPicture
{
	idc = 1800;
	text = "#(argb,8,8,3)color(0,0,0,0.8)";
	x = 0.35 * safezoneW + safezoneX;
	y = 0.32 * safezoneH + safezoneY;
	w = 0.3 * safezoneW;
	h = 0.28 * safezoneH;
};
class RscListbox_1500: RPP_RscListbox
{
	idc = 1500;
	x = 0.3625 * safezoneW + safezoneX;
	y = 0.36 * safezoneH + safezoneY;
	w = 0.275 * safezoneW;
	h = 0.16 * safezoneH;
};
class License_Plate: RPP_RscEdit
{
	idc = 1400;
	x = 0.425 * safezoneW + safezoneX;
	y = 0.56 * safezoneH + safezoneY;
	w = 0.075 * safezoneW;
	h = 0.02 * safezoneH;
};
class RscText_1000: RPP_RscText
{
	idc = 1000;
	text = "License Plate:"; //--- ToDo: Localize;
	x = 0.3625 * safezoneW + safezoneX;
	y = 0.56 * safezoneH + safezoneY;
	w = 0.0875 * safezoneW;
	h = 0.02 * safezoneH;
};
class RscButton_1600: RPP_RscButton
{
	idc = 1600;
	text = "Search"; //--- ToDo: Localize;
	x = 0.525 * safezoneW + safezoneX;
	y = 0.54 * safezoneH + safezoneY;
	w = 0.1 * safezoneW;
	h = 0.04 * safezoneH;
};
class RscText_1001: RPP_RscText
{
	idc = 1001;
	text = "Police Database Info"; //--- ToDo: Localize;
	x = 0.45 * safezoneW + safezoneX;
	y = 0.34 * safezoneH + safezoneY;
	w = 0.0875 * safezoneW;
	h = 0.02 * safezoneH;
};
class Header: RPP_RscPicture
{
	idc = 1200;
	text = "#(argb,8,8,3)color(1,0,0,1.0,CO)";
	x = 0.35 * safezoneW + safezoneX;
	y = 0.26 * safezoneH + safezoneY;
	w = 0.3 * safezoneW;
	h = 0.06 * safezoneH;
};
class RscText_1002: RPP_RscText
{
	idc = 1002;
	text = "Police Vehicle Database"; //--- ToDo: Localize;
	x = 0.3625 * safezoneW + safezoneX;
	y = 0.26 * safezoneH + safezoneY;
	w = 0.15 * safezoneW;
	h = 0.06 * safezoneH;
};
};
};
