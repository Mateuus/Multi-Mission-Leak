...
if (count playerInventoryArray == 1) then {
		_myInvListbox lbAdd format ["Du hast keine Items in deinem Inventar oder im Auktionshaus gelistet!"];
	} else {
{
	_status = [_x,2,3,[3]] call BIS_fnc_param;
	switch (_status) do {
		case 0: {_stat = ": AH: Zum verkauf"}; //AH: for sale
		case 1: {_stat = ": AH: Abgelaufen"}; //AH: expired
		case 2: {_stat = ": offlineVerkauft"}; //sold
		case 3: {_stat = ": Im inventar"}; //MY INV
		case 4: {_stat = ": Verkauft"}; //MY INV
	};
	...
	_index = _index + 1;
	if (_item == "yolo") then {
			_myInvListbox lbAdd format ["--------------------im Auktionshaus gelistete Items-----------------------"];
			_myInvListbox lbSetData [(lbSize _myInvListbox)-1,format ["%1",[_x select 0]]];
		} else {
	switch (_type) do {
		case "imenu": {
			_itemDisplayName = ([_item] call DWEV_fnc_fetchCfgDetails) select 1;
			_pic = [_item] call DWEV_fnc_fetchCfgDetails select 2;
			_price = [_x,5,0,[0]] call BIS_fnc_param;
			_time = [_x,4,0,[0]] call BIS_fnc_param;
			_time = round (_time / 3);
			if (_time <= 0) then {_price = 0; if (_status == 1) then {_toDel = round(_time + 216); _timeDisplay = format[": gelöscht in %1 Stunden",_toDel]} else {_timeDisplay = "";};} else {_timeDisplay = format[": noch %1 Stunden",_time]};
			if (_price == 0) then {_priceDisplay = ""} else {_priceDisplay = format[": Preis %1 €",[_price]call DWEV_fnc_numberText];};
			};
		case "ymenu": {
			_itemDisplayName = M_CONFIG(getText,"VirtualItems",_item,"displayName");
