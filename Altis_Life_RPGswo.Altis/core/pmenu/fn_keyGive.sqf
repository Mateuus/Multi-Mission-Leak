...
_vehicle = DWEV_vehicles select parseNumber(_vehicle);
...
_index = [_uid,_owners] call DWF_fnc_index;
if(_index == -1) then 
{
	_owners pushBack [_uid,getPlayerName(_unit)];
	_vehicle setVariable["vehicle_info_owners",_owners,true];
};

hint format["Du hast %1 die Schlüssel zu deinem %2 gegeben.",getPlayerDName(_unit),typeOf _vehicle];
[_vehicle,_unit,getMyDName] remoteExec ["DWF_fnc_clientGetKey",_unit];