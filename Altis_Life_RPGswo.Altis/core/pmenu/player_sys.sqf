...
{
	ctrlShow[505054,false]; // ganggrafik
	ctrlShow[505044,false]; // gangbutton
};
...
_foodtext ctrlSettext format["%1 / 100", round(DWEV_hunger)];
_watertext ctrlSetText format["%1 / 100", round(DWEV_thirst)];
_carry ctrlSetText format["Tragegewicht: %1 / %2", DWEV_carryWeight, DWEV_maxWeight];
_moneytext ctrlSetText format["%1", dwf_cash];
