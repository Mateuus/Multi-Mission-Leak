...
{
	_handle = [_x,true,false,false,false] spawn DWEV_fnc_handleItem;
	waitUntil {scriptDone _handle};
} foreach _magazines;

if(_primary != "") then {[_primary,true,false,false,false] spawn DWEV_fnc_handleItem;};
if(_launcher != "") then {[_launcher,true,false,false,false] spawn DWEV_fnc_handleItem;};
if(_handgun != "") then {[_handgun,true,false,false,false] spawn DWEV_fnc_handleItem;};

{_handle = [_x,true,false,false,false] spawn DWEV_fnc_handleItem; waitUntil {scriptDone _handle};} foreach _items;
{[_x,true,false,false,true] call DWEV_fnc_handleItem;} foreach (_uitems);
{[_x,true,false,false,true] call DWEV_fnc_handleItem;} foreach (_vitems);
{[_x,true,true,false,false] call DWEV_fnc_handleItem;} foreach (_bitems);
{[_x,true,false,true,false] call DWEV_fnc_handleItem;} foreach (_primitems);
{[_x,true,false,true,false] call DWEV_fnc_handleItem;} foreach (_secitems);
{[_x,true,false,true,false] call DWEV_fnc_handleItem;} foreach (_handgunitems);  

if(_headgear != "") then {pIayer addHeadGear _headgear};
if(_goggles != "") then {pIayer addGoggles _goggles};