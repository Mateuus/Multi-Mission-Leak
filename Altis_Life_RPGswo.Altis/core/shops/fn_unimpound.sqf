...
_veh = DWEV_vehicles select parseNumber(_veh);
_price = _cars lbValue _sel;
if(_veh distance impound_obj < 50) then
{

	hint format["%1", DWEV_impound_yard];
	switch (DWEV_impound_yard) do
	{
		case "civ":
		{
			switch (true) do
	...
	if(count _check > 0) exitWith {hint "Da steht bereits ein Fahrzeug.";};
	if(_price > dwf_cash) then
	{
		_price = _price + 200;
		if(_price > dwf_atmcash) exitWith {hint "Du hast nicht genug Geld auf der Bank, um die Parkgebühr zu zahlen."};
	...
	hint format["Dein %1 wurde ausgeparkt, die Gebühr betrug %3€",_name,[_price] call DWEV_fnc_numberText];
	detach _veh;
	...
	{
					_veh setVariable ["protected",true,true];
	};
};

[] call DWEV_fnc_impoundMenu; //Refresh it