...
if(dwf_cash < _betamt) exitWith {hint format["Du hast nicht genug Geld dabei, zum spielen brauchst du %1€.",_betamt];};
...
switch ( _betamt ) do 
{
	...
	case 4000: { _bet10KTEXT ctrlSetText ""; _bet5KTEXT ctrlSetText ""; _bet50KTEXT ctrlSetText ""; };
	case 8000: { _bet10KTEXT ctrlSetText ""; _bet25KTEXT ctrlSetText ""; _bet5KTEXT ctrlSetText ""; };
...
else
{
	...
	if( _number == 13 ) then
	{
		...
		{		
			...
			_winnings = _display displayCtrl 6001;
			_winnings ctrlSetText "Du hast verloren. Dealer hat Black Jack.";
			_bet1 ctrlEnable true;
			_bet2 ctrlEnable true;
			_bet3 ctrlEnable true;
			_bet4 ctrlEnable true;
