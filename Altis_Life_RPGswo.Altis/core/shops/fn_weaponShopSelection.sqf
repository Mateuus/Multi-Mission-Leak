...
if ((uiNamespace getVariable ["Weapon_Shop_Filter",0]) isEqualTo 1) then {
    ...
    _item = [_item,_itemArray] call DWF_fnc_index;
    _price = ((_itemArray select _item) select 3);
    _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Preis: <t color='#8cff9b'>$%1</t></t>",[(_price)] call DWEV_fnc_numberText];
    _control lbSetValue[_index,_price];
} else {
    _price = _control lbValue _index;
    _item = _control lbData _index;
    if (_price > dwf_cash) then {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Preis: <t color='#ff0000'>$%1</t><br/>Dir fehlen: <t color='#8cff9b'>$%2</t></t>",[(_price)] call DWEV_fnc_numberText,[(_price - dwf_cash)] call DWEV_fnc_numberText];
    } else {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Preis: <t color='#8cff9b'>$%1</t></t>",[(_price)] call DWEV_fnc_numberText];
    };
