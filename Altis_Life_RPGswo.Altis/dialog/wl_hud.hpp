...
class WastelandHud {
	...
	class controlsBackground {
		...
		class WastelandHud_Status:w_RscText
		{
			...
			type = CT_STRUCTURED_TEXT;
			size = 0.040;
			x = safeZoneX + (safeZoneW * (1 - (1/ SafeZoneW)));
			y = safeZoneY + (safeZoneH * (1 - (0.4 / SafeZoneH)));
			w = 1; h = 0.5;
			colorText[] = {1,1,1,1};
			lineSpacing = 3;
			colorBackground[] = {0,0,0,0};
		...
		class WastelandHud_dwLogo:w_RscText
		{
			idc = hud_activity_icon_idc;
			type = CT_STATIC;
			style = 2096;  // Same Size w and h of Picture
			size = 0.040;
			x = safeZoneX;
			y = safeZoneY;
			w = (safeZoneW * 0.12); 
			h = (safeZoneH * (safeZoneW / safeZoneH) * 0.12);
			colorText[] = {1,1,1,1};
			lineSpacing = 3;
			colorBackground[] = {0,0,0,0};
