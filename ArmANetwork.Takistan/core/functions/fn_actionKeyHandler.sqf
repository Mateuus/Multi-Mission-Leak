...
if ((playerSide == blufor) && {(EQUAL(LIFE_SETTINGS(getNumber,"revive_cops"),1))} && (player getVariable "eg_canRevive") OR (playerSide == independent) && {"Medikit" in (items player)}) then {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	if((_curTarget GVAR ["restrained",false]) && !dialog && playerSide == west) then {
		...
		if((_curTarget GVAR ["restrained",false]) && ((_curTarget getVariable "restrainedtype") == "civ") && !dialog && playerSide == civilian && !(player getVariable["restrained",false])) then {
