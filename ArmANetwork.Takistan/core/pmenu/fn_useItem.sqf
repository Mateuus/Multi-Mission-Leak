...
switch (true) do {
	...
	case (EQUAL(_item,"vodka")): {
		if(playerSide in [west,independent]) exitWith {hint "Get outta here!";};
		if(life_drink == 1) exitWith {hint "You cannot drink any more at this time. Slow down!";};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			if(life_drink < 1) then
			{
				if(isNil "life_drink") then {life_drink = 0;};
				if(life_drink >= 0.92) then
				{
					life_drink = 1;
				}
				else
	...
	case (EQUAL(_item,"whiskey")): {
		if(playerSide in [west,independent]) exitWith {hint "Get outta here you alcoholic!";};
		if(life_drink == 1) exitWith {hint "You cannot drink any more at this time. Slow down!";};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			if(life_drink < 1) then
	...
	case (EQUAL(_item,"demolitioncharge")):
	{
