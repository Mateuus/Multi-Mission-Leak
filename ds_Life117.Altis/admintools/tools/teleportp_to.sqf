...
//Teleporting Others to Me Script
...
smenu =
{
	_pmenu = [["Teleport Other Player(s) to Me",true]];
...
if (pselect5 != "exit") then
{
	_name = pselect5;
	{
		if(name _x == _name) then
		{
			...
			_posplr = [((getPosASL player) select 0) - 3, ((getPosASL player) select 1) + 3, ((getPosASL player) select 2)];
			vehicle _x setPosASL _posplr;
		};
