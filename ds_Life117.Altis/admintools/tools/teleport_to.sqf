...
smenu =
{
	_pmenu = [["Teleport to Other Player(s)",true]];
...
if (pselect5 != "exit") then
{
	_name = pselect5;
	{
		if(name _x == _name) then
		{
			...
			_posplr = [((getPosASL _x) select 0) - 3, ((getPosASL _x) select 1) + 3, ((getPosASL _x) select 2)];
			vehicle player setPosASL _posplr;
		};
