...
[[player getVariable["name",name player],_payment],"DS_fnc_revivedMedic",_body,FALSE] spawn BIS_fnc_MP;
DS_doingStuff = false;
sleep 1;
player reveal _body;
...
if(((DS_infoArray select 10) == 12)&&((side _body) == west))then
	{
	[3] call DS_fnc_questCompleted;
	};
...
if((DS_infoArray select 10) == 19)then
	...
	if((DS_infoArray select 11) > 3)then
		{
		[3] call DS_fnc_questCompleted;
		};
	};
