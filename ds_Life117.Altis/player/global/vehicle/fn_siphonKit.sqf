...
if(!alive player) exitWith {};
if(DS_moved) exitWith {DS_moved = false; titleText "You can not move while doing this";["siphonKit",true,1] call DS_fnc_handleInventory;};
...
switch (true) do
{
	...
	case (_vehicle isKindOf "Ship"):
	{
		if(!local _vehicle) then
		{
			[[[_vehicle],{_this select 0 setFuel ((Fuel (_this select 0)) - 0.35);}],"BIS_fnc_spawn",_vehicle,false] spawn BIS_fnc_MP;
		}
			else
		{
...
if((DS_infoArray select 15) == 3)then
	{
	[1] call DS_fnc_questCompleted;
	};
if((_vehicle getVariable ["policeVehicle",false])&&((DS_infoArray select 15) == 21))then
	{
	[1] call DS_fnc_questCompleted;
	};