...
if(!alive player)exitwith{};
if(player != vehicle player) exitWith {hint "You must remain outside the vehicle to refuel it";["refuelKit",true,1] call DS_fnc_handleInventory;};
...
if(_vehicle isKindOf "Car")then
	...
	if(!local _vehicle) then
		{
		[[[_vehicle],{_this select 0 setFuel ((Fuel (_this select 0)) + 0.4);}],"BIS_fnc_spawn",_vehicle,false] spawn BIS_fnc_MP;
		}
		else
		{
...
if(_vehicle isKindOf "Ship")then
	...
	if(!local _vehicle) then
		{
		[[[_vehicle],{_this select 0 setFuel ((Fuel (_this select 0)) + 0.9);}],"BIS_fnc_spawn",_vehicle,false] spawn BIS_fnc_MP;
		}
		else
		{
