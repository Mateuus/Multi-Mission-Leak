...
if(_receiver)then
	...
	hint parseText format ["<t color='#FFCC00'><t size='2'><t align='center'>Police Emergency Call<br/><br/><t color='#33CC33'><t align='left'><t size='1'>To: <t color='#ffffff'>Police<br/><t color='#33CC33'>From: <t color='#ffffff'>%1<br/><br/><t color='#33CC33'>Message:<br/><t color='#ffffff'>%2",(name _from),_message];
	...
	if(DS_jailed)exitwith{hint "You can not send messages while in jail";systemchat "You can not send messages while in jail"};
	if((_message == "")||(_message == "Enter Message here")) exitWith {hint "You must enter a message to send!";ctrlShow[2400,true];};
	[[true,player,_message],"DS_fnc_messagepolice",west,false] spawn BIS_fnc_MP;
