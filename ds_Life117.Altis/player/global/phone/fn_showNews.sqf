...
if(count DS_newsLog == 0) then 
	{
	_log lbAdd "No News To Display";
...
//Fill the message log
		...
		while{_counter != 0}do
			{
			_log lbAdd format["%1",(DS_newsLog select ((_counter)-1))];
			_log lbSetData [(lbSize _log)-1,(DS_newsLog select ((_counter)-1))];
			_counter = _counter - 1;
			};
	};
//Set up Combo
