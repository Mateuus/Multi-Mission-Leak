...
*/
	...
	private ["_hunger","_thirst","_overLoaded","_nightStalker","_backPack","_sickness"];
	_hunger =
	{
		if(((headgear player) in ["H_PilotHelmetFighter_B"])&&(!((vehicle player) isKindOf "Air")))then
			{
		...
		while{true}do
			...
			if(!_battery)then
				...
				if(((primaryWeaponItems player select 2) == "optic_Nightstalker")&&(DS_item_battery == 0))then
					...
					hint "Your night stalker has been destroyed because you tried using it without a battery";
					systemchat "Your night stalker has been destroyed because you tried using it without a battery";
					player say3D "fail";
					};
				};
