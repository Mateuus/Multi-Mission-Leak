...
*/
private ["_cop","_gangLeader","_gangName"];
...
[]spawn	
	...
	while{true}do
		...
		if((!([getPos player,45,west] call DS_fnc_nearbyPlayers))&&(!([getPos player,45,independent] call DS_fnc_nearbyPlayers))&&(player getVariable["cuffed",FALSE])&&(vehicle player == player))exitWith
			{
