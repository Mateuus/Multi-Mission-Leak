...
if((_typeA in ["weedu","heroinu","cocaineu"])&&((DS_infoArray select 15) >  24))then
	{
	_sleep = (_sleep*0.7);
	};
switch(_typeA)do
...
if(vehicle player != player) exitWith {hint "You can not process while inside a vehicle."; 5 cutText ["","PLAIN"]};
...
if((_typeA in ["weedu","heroinu","cocaineu"])&&((DS_infoArray select 15) == 35))then
	{
	_tempNum = (DS_infoArray select 16) + _amount;
	DS_infoArray set [16,_tempNum];
