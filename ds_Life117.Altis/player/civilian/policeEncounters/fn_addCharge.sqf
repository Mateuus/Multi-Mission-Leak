/*
	File: fn_addCharge.sqf
	Author: Shawn "Huntah" Macgillivray
	
	Description:
	Adds bounty information to a player
*/
...
if(isNil {_charge})exitwith{};//Don't do that
//[["481"],"life_fnc_addCharge",_civ,false] spawn life_fnc_MP;
//["483"] spawn life_fnc_addCharge;
switch(_charge) do
{
...
if(((DS_questLog select 4) == 18)&&((life_wanted_stats select 0) > 50000))then
			{
			[0,0,2] spawn DS_fnc_questCompleted;
			};

//Add name to server wanted list?
if((count DS_wanted_stats) > 1)then
	{
	//Double check they arn't already there for some reason
	_tempArray = [];
	...
	publicVariable "life_wanted_people";
	};
