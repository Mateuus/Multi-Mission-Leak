...
if(_type == 0)then
	...
	if((playerside == west)&&((DS_infoArray select 21) > 10))then{_bounty = (_bounty*1.5)};
	...
	[[(_bounty/2),true],"HUNT_fnc_policeBank",false,false] spawn DS_fnc_MP;
	};
...
if(_type == 1)then
	...
	hint format ["You have received a bounty of $%1 for killing %2",([_bounty] call DS_fnc_numberText),_civ];
	systemchat format ["You have received a bounty of $%1 for killing %2",([_bounty] call DS_fnc_numberText),_civ];
	[_bounty,true,true] call DS_fnc_handleMoney;
	[50,true] spawn DS_fnc_moralAlter;
	[[(_bounty/8),true],"HUNT_fnc_policeBank",false,false] spawn DS_fnc_MP;
	};
