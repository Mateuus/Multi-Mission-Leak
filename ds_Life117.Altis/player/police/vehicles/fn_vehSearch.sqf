...
*/
private ["_vehicle","_convoy","_bootInfo","_value","_tempValue","_class","_amount","_action","_bulletProof"];
...
if(_value == 0)then
	...
	{
	[[0,format["A searched vehicle revealed illegal items with an estimated street value of $%1.",[_value] call DS_fnc_numberText]],"DS_fnc_globalMessage",true,false] spawn DS_fnc_MP;
	...
	[[(round(_value/2)),true],"HUNT_fnc_policeBank",false,false] spawn DS_fnc_MP;
	};

sleep 5;
if((!(_vehicle getVariable ["policeVehicle",false]))&&((DS_infoArray select 21) == 5))then
	{
