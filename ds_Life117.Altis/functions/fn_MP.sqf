...
with missionnamespace do {
	...
	if(isServer && isDedicated) then {
		DS_fnc_MP_Packet = [0,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];
	} else {
		DS_fnc_MP_Packet = [0,_params,_functionName,_target,_isPersistent,_isCall,profileName,getPlayerUID player];
	};
	publicvariableserver "DS_fnc_MP_Packet";
