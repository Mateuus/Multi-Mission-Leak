...
if((playerSide == west && player GVAR ["isEscorting",false])) exitWith {
	[] call life_fnc_copInteractionMenu;
};

if((playerSide == civilian && player GVAR ["isEscorting",false])) exitWith {
...
if((playerSide != independent && _curObject isKindOf "B_supplyCrate_F" OR _curObject isKindOf "Box_IND_Grenades_F") && {player distance _curObject < 3} ) exitWith {
	if(alive _curObject) then {
		[_curObject] call life_fnc_containerMenu;
	};
};

if(playerSide != independent && _curObject isKindOf "House_F" && {player distance _curObject < 12} OR ((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _curObject OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curObject)) exitWith {
...
if(isPlayer _curObject && _curObject isKindOf "Man") then {
	...
	if((_curObject GVAR ["tied",false]) && !dialog && playerSide == civilian) then {
		[_curObject] call life_fnc_civInteractionMenu;
	};

	if(playerSide == civilian && !(_curObject GVAR ["tied",false]) && (_curObject GVAR ["restrained",false])) then
	{
		hint "Ta osoba jest skuta przez Policję.";
	};
