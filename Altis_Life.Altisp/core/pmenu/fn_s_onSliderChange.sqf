...
switch (_mode) do {
	...
	case 3: {
		...
		if (!isNil "_classname" || _classname =="") then {
			...
			ctrlSetText[20324,format["Paliwo: %1 litry",round(_value) - (floor(_fuel * _fueltank))]];
			ctrlSetText [20323,format ["Razem: $%1",round (life_fuelPrices * (round(_value) -(floor(_fuel * _fueltank)))) ]];
		} else {
