...
if((player GVAR["restrained",false])) exitWith {life_action_inUse = false;_hideout SVAR ["inCapture",false,true];};
...
titleText["Kryjówka została przejęta.","PLAIN"];
_flagTexture = [
...
[[0,1],"STR_GNOTF_CaptureSuccess",true,[name player,(group player) GVAR "gang_name"]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
 
_markername = str(getPos _hideout);
_gangname2 = formatText["Przejęta przez: %1",(group player) GVAR "gang_name"];
