...
if(localize _displayName find "Specjalizacja" >= 0) then {
	[8,_varname] call SOCK_fnc_updatePartial;
} else {
	if(localize _displayName find "Pozwolenie na broń LVL" >= 0) then {
		[9,_varname] call SOCK_fnc_updatePartial;
	} else {
		if(localize _displayName find "Transport" >= 0) then {
			[10,_varname] call SOCK_fnc_updatePartial;
		} else {
