...
if (count crew _vehicle isEqualTo 0) then {
    ...
    _message = format["Crush Log: %1 crushed %2's %3", profileName,((_vehicleData select 0) select 1),_vehicleName];
    diag_log format["POLICE: %1",_message];
    if (_vehicle in life_vehicles) then {
        hint format[localize "STR_NOTF_OwnCrushed",[_value] call life_fnc_numberText,_type];
        BANK = BANK - _value;
    } else {
