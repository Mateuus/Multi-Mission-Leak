...
switch (playerSide) do {
	...
	if(license_civ_don4) then{
			_buyMultiplier = LIFE_SETTINGS(getNumber,"vehicle_purchase_multiplier_donor4");
			_rentMultiplier = LIFE_SETTINGS(getNumber,"vehicle_rental_multiplier_donor4");
	} else {
		if((license_civ_don1) || (license_civ_don2) || (license_civ_don3)) then{  
			_buyMultiplier = LIFE_SETTINGS(getNumber,"vehicle_purchase_multiplier_donor");
			_rentMultiplier = LIFE_SETTINGS(getNumber,"vehicle_rental_multiplier_donor");
		} else {
...
{  
     if (call compile _x) then {_exit = false;}; 
} forEach _licenses;  
  
if (_exit) exitWith {hint format ["Brak wymaganej licencji"];};
