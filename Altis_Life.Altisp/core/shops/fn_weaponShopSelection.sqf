...
if((GVAR_UINS ["Weapon_Shop_Filter",0]) == 1) then {
	...
	_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Cena: <t color='#8cff9b'>$%1</t></t>",[(_price)] call life_fnc_numberText];
	_control lbSetValue[_index,_price];
} else {
	_price = _control lbValue _index;
	_item = CONTROL_DATAI(_control,_index);
	if(_price > CASH) then {
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Cena: <t color='#ff0000'>$%1</t><br/>Brakuje: <t color='#8cff9b'>$%2</t></t>",[(_price)] call life_fnc_numberText,[(_price - CASH)] call life_fnc_numberText];
	} else {
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Cena: <t color='#8cff9b'>$%1</t></t>",[(_price)] call life_fnc_numberText];
	};
