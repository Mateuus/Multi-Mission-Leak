...
switch (_mode) do {
	case 0: {
		_bank = [(_group getVariable ["gang_bank",0])] call ES_fnc_numberSafe;
		_maxMembers = _group getVariable ["gang_maxMembers",8];
		_members = [(_group getVariable "gang_members")] call ES_fnc_mresArray;
	...
	case 1: {
		_query = format["UPDATE gangs SET bank='%1' WHERE id='%2'",([(_group getVariable ["gang_bank",0])] call ES_fnc_numberSafe),_groupID];
	};
	...
	case 4: {
		...
		_membersFinal = [(_group getVariable "gang_members")] call ES_fnc_mresArray;
