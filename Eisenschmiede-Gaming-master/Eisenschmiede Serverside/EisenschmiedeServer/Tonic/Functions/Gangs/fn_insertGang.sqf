...
_gangName = [_gangName] call ES_fnc_mresString;
...
if(!(EQUAL(count _queryResult,0))) exitWith {
	[1,"There is already a gang created with that name please pick another name."] remoteExecCall ["ES_fnc_broadcast",_ownerID];
	ES_action_gangInUse = nil;
	_ownerID publicVariableClient "ES_action_gangInUse";
};
...
if(!(EQUAL(count _queryResult,0))) exitWith {
	[1,"You are currently already active in a gang, please leave the gang first."] remoteExecCall ["ES_fnc_broadcast",_ownerID];
	ES_action_gangInUse = nil;
	_ownerID publicVariableClient "ES_action_gangInUse";
...
_queryResult = [_query,2] call ES_fnc_asyncCall;
_gangMembers = [[_uid]] call ES_fnc_mresArray;
...
[_group] remoteExecCall ["ES_fnc_gangCreated",_ownerID];
