...
waitUntil {_state == _vehicle getVariable ["blinke",0]};
...
_brightness = 0;
while{ (alive _vehicle) && _vehicle getVariable ["blinke",-1] == _state } do  
{
	...
	if (vehicle player == _vehicle) then { PlaySound "blink"; };
	sleep 0.8;  
};  
