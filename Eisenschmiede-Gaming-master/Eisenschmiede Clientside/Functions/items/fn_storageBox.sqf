...
if(!(_house in ES_vehicles)) exitWith {hint localize "STR_ISTR_Box_NotinHouse"};
...
_positions = [_house] call ES_fnc_getBuildingPositions;
_pos = [0,0,0];
{
	_slots = _house getVariable ["slots",[]];
...
if(!([false,_boxType,1] call ES_fnc_handleInv)) exitWith {};
...
[_house] remoteExecCall ["ES_fnc_updateHouseContainers",2];
