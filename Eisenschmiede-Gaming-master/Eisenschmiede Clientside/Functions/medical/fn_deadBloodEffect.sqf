ES_oldBleedRemaining = _this select 0;

ES_bloodActionPlaying = true;
[]spawn {
    ...
    //lower
    if((ES_oldBleedRemaining > 5) && (ES_oldBleedRemaining < 25)) then
    {
    ...
    //lower, 50% middle
    if ((ES_oldBleedRemaining >= 25) && (ES_oldBleedRemaining < 40)) then
    {
        _texLower ctrlsetfade 0.2;	 //0 --changed
    ...
    //lower, middle
    if ((ES_oldBleedRemaining >= 40) && (ES_oldBleedRemaining < 55)) then
    {
    ...
    if ((ES_oldBleedRemaining >= 55) && (ES_oldBleedRemaining < 70)) then
    {
        _texLower ctrlsetfade 0.2;	 
        _texMiddle ctrlsetfade 0.7;  
    ...
    //lower, middle, upper
    if (ES_oldBleedRemaining >= 70) then
    {
        _texLower ctrlsetfade 0.2;	 
        _texMiddle ctrlsetfade 0.7;  
