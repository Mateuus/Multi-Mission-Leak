...
if(!isClass(missionConfigFile >> "VirtualShops" >> ES_shop_type)) exitWith {closeDialog 0; hint "Config does not exist?";}; //Make sure the entry exists..
ctrlSetText[2403,localize (M_CONFIG(getText,"VirtualShops",ES_shop_type,"name"))];
_shopItems = M_CONFIG(getArray,"VirtualShops",ES_shop_type,"items");

{
	...
	if(!(EQUAL(_price,-1))) then {
		_item_list lbAdd format["%1  ($%2)",(localize _displayName),[_price] call ES_fnc_numberText];
