...
switch (_index) do {
	case 0: {
		_config = [_shop] call ES_fnc_weaponShopCfg; {
			_itemInfo = [_x select 0] call ES_fnc_fetchCfgDetails;
	...
	case 1: {
		...
		((findDisplay 38400) displayCtrl 38405) ctrlSetText localize "STR_Global_Sell";
		{
			if(!(_x in _listedItems) && _x != "") then {
				_itemInfo = [_x] call ES_fnc_fetchCfgDetails;
...
switch (_index) do {
	case 0: {
		_config = M_CONFIG(getArray,"WeaponShops",_shop,"items"); {
			_itemInfo = [SEL(_x,0)] call ES_fnc_fetchCfgDetails;
