...
ES_clothing_filter = _selection;

switch (_selection) do {
	case 4: {
		ES_shop_cam camSetTarget (player modelToWorld [0,-.15,1.3]);
		ES_shop_cam camSetPos (player modelToWorld [1,-4,2]);
		ES_shop_cam camCommit 1;
	};
	...
	case 3: {
		ES_shop_cam camSetTarget (player modelToWorld [0,0,1.4]);
		ES_shop_cam camSetPos (player modelToWorld [-.1,2,1.4]);
		ES_shop_cam camCommit 1;
	};
	
	default {
		ES_shop_cam camSetTarget (player modelToWorld [0,0,1.6]);
		ES_shop_cam camSetPos (player modelToWorld [-.5,1,1.6]);
		ES_shop_cam camCommit 1;
	};
};
...
_array = switch(_selection) do {
	case 0: {M_CONFIG(getArray,"Clothing",ES_clothing_store,"uniforms");};
	case 1: {M_CONFIG(getArray,"Clothing",ES_clothing_store,"headgear");};
	case 2: {M_CONFIG(getArray,"Clothing",ES_clothing_store,"goggles");};
	case 3: {M_CONFIG(getArray,"Clothing",ES_clothing_store,"vests");};
	case 4: {M_CONFIG(getArray,"Clothing",ES_clothing_store,"backpacks");};
};

{
	...
	if(!(EQUAL(_className,"NONE"))) then {
		_details = [_className] call ES_fnc_fetchCfgDetails;
		_pic = SEL(_details,2);
	};
