...
if(!dialog) then
{
	if(!(createDialog "ES_impound_menu")) exitWith {};
	ES_impound_yard = _this select 3;
};
...
for "_i" from 0 to (count ES_vehicles)-1 do
{
	_veh = ES_vehicles select _i;
	if(_veh distance impound_obj < 50) then
	{
		_price = [_veh] call ES_fnc_impoundPrice;
		switch (true) do
		{
		...
		_color = [(typeOf _veh),(_veh getVariable "ES_VEH_color")] call ES_fnc_vehicleColorStr;
		...
		_cars lbAdd format["%1 %2 - Cost: $%3",_name,_text,[_price] call ES_fnc_numberText];
