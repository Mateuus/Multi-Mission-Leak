...
if(isNull ES_trunk_vehicle OR !alive ES_trunk_vehicle) exitWith {hint localize "STR_MISC_VehDoesntExist"};
...
_index = [_ctrl,((ES_trunk_vehicle getVariable "Trunk") select 0)] call ES_fnc_index;
_data = (ES_trunk_vehicle getVariable "Trunk") select 0;
_old = ES_trunk_vehicle getVariable "Trunk";
...
_weight = ([_ctrl] call ES_fnc_itemWeight) * _num;
if(_ctrl == "money") then {
	if(_num == _value) then {
		_data set[_index,-1];
		_data = _data - [-1];
	...
	ES_trunk_vehicle setVariable["Trunk",[_data,(_old select 1) - _weight],true];
	[ES_trunk_vehicle] call ES_fnc_vehInventory;
} else {
	if([true,_ctrl,_num] call ES_fnc_handleInv) then {
		if(_num == _value) then {
			_data set[_index,-1];
			_data = _data - [-1];
		} else {
