...
if(dialog) exitWith {};
if(ES_is_processing) exitwith {};
_fuelstations = nearestobjects [player, ["Land_fs_feed_F"],10];
if (count _fuelstations == 0 ) exitwith {ES_action_inUse = false;};
if(!createDialog "ES_FuelStat") exitWith {};
_fuelCost = fuel_cost;
[] spawn {waituntil {!dialog}; ES_action_inUse =false;};
ctrlSetText [20301,"Fuel Station"];
if (isnil "ES_fuelPrices") then {
	ES_fuelPrices = _fuelCost;
};

//Fetch the shop config.
_vehicleList = nearestObjects [player, ["Car","air"], 10];
if (count _vehicleList < 1) exitwith {hint localize "STR_NOTF_VehicleNear";ES_action_inUse = true;};
vehicleFuelList =[];
{
	_fuel = fuel _x;
...
ctrlsettext [20322,format [localize "STR_Price_Fuel_Pump",ES_fuelPrices]];
ctrlsettext [20323,format ["Total : %1$",ES_fuelPrices * 0.1]];
//Loop through
{
