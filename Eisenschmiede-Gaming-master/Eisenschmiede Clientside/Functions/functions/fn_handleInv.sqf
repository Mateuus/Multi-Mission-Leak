#include "..\script_macros.hpp"
/*
	File: fn_handleInv.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Do I really need one?
*/
private["_math","_item","_num","_return","_var","_weight","_value","_diff"];
_math = [_this,0,false,[false]] call BIS_fnc_param; //true = HDM; false = HDF;
_item = [_this,1,"",[""]] call BIS_fnc_param; //The item we are using to add or remove.
_num = [_this,2,0,[0]] call BIS_fnc_param; //Number of items to add or remove.
if(EQUAL(_item,"") OR EQUAL(_num,0)) exitWith {false};

_var = ITEM_VARNAME(_item);

if(_math) then {
	_diff = [_item,_num,ES_carryWeight,ES_maxWeight] call ES_fnc_calWeightDiff;
	_num = _diff;
	if(_num <= 0) exitWith {false};
};
_weight = ([_item] call ES_fnc_itemWeight) * _num;
_value = ITEM_VALUE(_item);

if(_math) then {
	//Lets LELELELLELEELELE!
	if((ES_carryWeight + _weight) <= ES_maxWeight) then {
		missionNamespace setVariable [_var,(_value + _num)];
		
		if((missionNamespace getVariable _var) > _value) then {
			ES_carryWeight = ES_carryWeight + _weight;
			_return = true;
		} else {
			_return = false;
		};
	} else {_return = false;};
} else {
	//Lets LELLELELE!
	if((_value - _num) < 0) then { _return = false;} else {
		missionNamespace setVariable[_var,(_value - _num)];
		
		if((missionNamespace getVariable _var) < _value) then {
			ES_carryWeight = ES_carryWeight - _weight;
			_return = true;
		} else {_return = false;};
	};
};

_return;