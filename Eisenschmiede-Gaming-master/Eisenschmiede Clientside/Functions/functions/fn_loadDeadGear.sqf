...
{
	_handle = [_x,true,false,false,false] spawn ES_fnc_handleItem;
	waitUntil {scriptDone _handle};
} foreach _magazines;

if(!(EQUAL(_primary,""))) then {[_primary,true,false,false,false] spawn ES_fnc_handleItem;};
if(!(EQUAL(_launcher,""))) then {[_launcher,true,false,false,false] spawn ES_fnc_handleItem;};
if(!(EQUAL(_handgun,""))) then {[_handgun,true,false,false,false] spawn ES_fnc_handleItem;};

{_handle = [_x,true,false,false,false] spawn ES_fnc_handleItem; waitUntil {scriptDone _handle};} foreach _items;
{[_x,true,false,false,true] call ES_fnc_handleItem;} foreach (_uitems);
{[_x,true,false,false,true] call ES_fnc_handleItem;} foreach (_vitems);
{[_x,true,true,false,false] call ES_fnc_handleItem;} foreach (_bitems);
{[_x,true,false,true,false] call ES_fnc_handleItem;} foreach (_primitems);
{[_x,true,false,true,false] call ES_fnc_handleItem;} foreach (_secitems);
{[_x,true,false,true,false] call ES_fnc_handleItem;} foreach (_handgunitems);  
