...
_details = [_item] call ES_fnc_fetchCfgDetails;
	...
	switch((_details select 6)) do {
		...
		case "CfgVehicles": {
			...
			if(!isNil {_items}) then { 
				{[_x,true,true,false,true] call ES_fnc_handleItem; } foreach _items;
			};
		};
				...
				switch(SEL(_details,5)) do {
						...
						if(_ispack) then {
							...
							if(_override) then{
								...
								if(EQUAL(vest player,_item)) then {
									...
									if(!isNil {_items}) then {
										{[_x,true,false,false,true] spawn ES_fnc_handleItem;} foreach _items;
									};
								};
							};
						};
					...
					case 201: {
						if(_ispack) then {
							...
							private "_type";
							_type = [_item,201] call ES_fnc_accType;
							if(_ongun) then {
								switch (_type) do {
					...
					case 301: {
						if(_ispack) then {
							...
							private "_type";
							_type = [_item,301] call ES_fnc_accType;
					...
					case 101:{
						if(_ispack) then {
							...
							private "_type";
							_type = [_item,101] call ES_fnc_accType;
