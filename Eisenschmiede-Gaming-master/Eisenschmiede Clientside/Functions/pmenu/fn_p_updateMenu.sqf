...
_mstatus ctrlSetStructuredText parseText format["<img size='1.3' image='Images\Icons\bank.paa'/> <t size='0.8px'>$%1</t><br/><img size='1.2' image='Images\Icons\money.paa'/> <t size='0.8'>$%2</t>",[ES_atmbank] call ES_fnc_numberText,[ES_cash] call ES_fnc_numberText];
ctrlSetText[2009,format["Weight: %1 / %2", ES_carryWeight, ES_maxWeight]];
...

{
	...
	if(LICENSE_VALUE(configName _x,_side)) then {
		...
		_lic lbSetData [(lbSize _lic)-1,configName _x];
	};
