[] spawn  {
	...
	_fnc_food = 
	{
		if(ES_hunger < 2) then {player setDamage 1; hint localize "STR_NOTF_EatMSG_Death";}
		else
		{
	...
	_fnc_water = 
	{
		if(ES_thirst < 2) then {player setDamage 1; hint localize "STR_NOTF_DrinkMSG_Death";}
		else
		{
...
[] spawn
{
	while {true} do
	{
		waitUntil {(ES_drink > 0)};
		while{(ES_drink > 0)} do {
		
			if(ES_drink > 0.08) then {
...
[] spawn
{
	...
	while{true} do
	{
		...
		_load = round(_cfg / 8);
		ES_maxWeight = ES_maxWeightT + _load;
