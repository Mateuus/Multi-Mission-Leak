...
if(EQUAL(LIFE_SETTINGS(getNumber,"Pump_service"),1)) then{
	[] spawn life_fnc_Setup_Sation_Service;
};

if (life_HC_isActive) then {
	[getPlayerUID player,player getVariable["realname",name player]] remoteExecCall ["HC_fnc_wantedProfUpdate",HC_Life];
} else {
	[getPlayerUID player,player getVariable["realname",name player]] remoteExecCall ["life_fnc_wantedProfUpdate",RSERV];
};
