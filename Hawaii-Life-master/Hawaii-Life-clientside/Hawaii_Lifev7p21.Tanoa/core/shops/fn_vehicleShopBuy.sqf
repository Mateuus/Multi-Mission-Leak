...
*/
private["_mode","_spawnPoints","_className","_basePrice","_colorIndex","_spawnPoint","_vehicle","_shopSide","_licenses","_licensesName","_exit","_neon"];
_mode = SEL(_this,0);
_exit = false;
_neon = 0;
...
{
	if(!(EQUAL(_x,""))) then {
		if(!(LICENSE_VALUE(_x,_shopSide))) then {
			ADD(_licensesName,localize M_CONFIG(getText,"Licenses",_x,"displayName") + "<br/>");
			_exit = true;
		};
	};
} foreach _licenses;
if(_exit) exitWith {closeDialog 0;hint parseText format[(localize "STR_Shop_Veh_NoLicense")+ "<br/><br/>%1",_licensesName];};
...
if((SEL(life_veh_shop,2) isEqualTo "neon")) then {_vehicle setObjectMaterialGlobal [0, "textures\vr_white.rvmat"];_neon = 1};
...
switch(playerSide) do {
	...
	case civilian: {
		if(EQUAL(SEL(life_veh_shop,2),"civ")) then {
...
if(_mode) then {
	if(!(_className in ["B_G_Offroad_01_armed_F","B_MRAP_01_hmg_F"])) then {
		if (life_HC_isActive) then {
			[(getPlayerUID player),playerSide,_vehicle,_colorIndex,_neon] remoteExecCall ["HC_fnc_vehicleCreate",HC_Life];
		} else {
			[(getPlayerUID player),playerSide,_vehicle,_colorIndex,_neon] remoteExecCall ["TON_fnc_vehicleCreate",RSERV];
		};
	};
};
