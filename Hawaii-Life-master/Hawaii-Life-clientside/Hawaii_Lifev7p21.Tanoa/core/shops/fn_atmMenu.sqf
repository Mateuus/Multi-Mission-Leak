...
lbClear _units;
CONTROL(2700,2701) ctrlSetStructuredText parseText format["<t size='1.3px'>$ %2</t><br/><br/><t size='1.3px'>$ %1</t>",[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];

{
	_name = _x GVAR ["realname",name _x];
	if(alive _x) then {
		if(!(EQUAL(_name,profileName))) then {
			switch (side _x) do {
				case west: {_type = "Cop"};
