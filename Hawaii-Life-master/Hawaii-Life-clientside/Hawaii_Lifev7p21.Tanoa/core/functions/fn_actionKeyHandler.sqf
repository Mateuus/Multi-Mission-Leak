...
if(_curTarget isKindOf "B_supplyCrate_F" OR _curTarget isKindOf "Box_IND_Grenades_F") then {
...
if(_curTarget isKindOf "House_F") then {
	if (player distance _curTarget < 12) exitWith {
...
if(!alive _curTarget) then {
	if(_curTarget isKindOf "Man") then {
		if(!(_curTarget GVAR["Revive",false])) exitWith {
			//Hotfix code by ins0
			...
			if (playerSide isEqualTo civilian) then {
				if(cursorTarget GVAR "infected") then {
					[_curTarget] call life_fnc_getHumanFlesh;
				};
			};
		};
	};
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	if(_isVehicle) then {
		...
		if((typeOf _curTarget) in _animalTypes) then {
			if(EQUAL((typeOf _curTarget),"Turtle_F")) then {
				if (!alive _curTarget) then {
					private "_handle";
			...
			if((typeOf _curTarget) in _miscItems) then {
				if (life_HC_isActive) then {
					[_curTarget,player,false] remoteExecCall ["HC_fnc_pickupAction",HC_Life];
				} else {
					[_curTarget,player,false] remoteExecCall ["TON_fnc_pickupAction",RSERV];
				};
			} else {
				//It wasn't a misc item so is it money?
				if(EQUAL((typeOf _curTarget),_money)) then {
					if (!(_curTarget GVAR ["inUse",false])) then {
						if (life_HC_isActive) then {
							[_curTarget,player,true] remoteExecCall ["HC_fnc_pickupAction",HC_Life];
						} else {
