...
*/
private["_container","_number","_type","_house","_containers","_houseCfg","_hausid"];
		...
		if(!(_house in life_vehicles) OR isNil {_house GVAR "house_owner"}) then {
			...
			if(count _containers >= (_houseCfg select 1)) then {
				...
				hint localize "STR_ISTR_Box_HouseFull";
					...
					_hausid = _house getVariable["house_id",0];
					_container setVariable["house_id",_hausid,true];
