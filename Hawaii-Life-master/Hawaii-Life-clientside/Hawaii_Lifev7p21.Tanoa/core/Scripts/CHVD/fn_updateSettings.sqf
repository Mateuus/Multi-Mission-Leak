...
if (_inUAV) then {
	switch (true) do {
		case (getConnectedUAV player isKindOf "LandVehicle" || getConnectedUAV player isKindOf "Ship"): {
			if (_updateType isEqualTo 1 || _updateType isEqualTo 0 || _updateType isEqualTo 3) then {
				setViewDistance CHVD_car;
			};
			if (_updateType isEqualTo 2 || _updateType isEqualTo 0 || _updateType isEqualTo 3) then {
