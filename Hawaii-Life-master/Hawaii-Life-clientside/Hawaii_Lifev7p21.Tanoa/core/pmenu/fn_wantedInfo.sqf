...
if(count _data isEqualTo 0) exitWith {hint "Failed to fetch crimes";};
lbClear _list;
...

{
	...
	if(!(_crime in _mylist)) then
	{
		...
		_list lbAdd format[localize "STR_Wanted_Count",{_x isEqualTo _crime} count _crimes,localize _crime];
	};
} foreach _crimes;
