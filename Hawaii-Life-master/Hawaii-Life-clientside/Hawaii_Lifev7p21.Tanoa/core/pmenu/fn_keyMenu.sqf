...
for "_i" from 0 to (count life_vehicles)-1 do {
	...
	if(!isNull _veh) then {
		if(alive _veh) then {
			...
			_vehicles lbAdd format["%1 %3 [%2m]",_name,round(player distance _veh),_text];lbSetColor[2701,(lbSize _vehicles)-1,[0,0,0,1]];
...
if(((lbSize _vehicles)-1) isEqualTo -1) then {
	...
	lbSetColor[2701,(lbSize _vehicles)-1,[0,0,0,1]];
};
