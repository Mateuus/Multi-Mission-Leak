class handy_apps1 {
	...
	class controls {
		...
		class ButtonMusik : Life_RscButtonMenu {
			idc = -1;
			tooltip = "$STR_PM_music";
			onButtonClick = "if(isClass(configFile >> ""CfgPatches"" >> ""a3_nradio"")) then {[] call NRD_fnc_start;} else {[0,true] spawn life_fnc_InfoMenu;};";
			size = "0.022 * safeZoneH";
			x = 0.7607 * safezoneW + safezoneX;
...
class handy_apps2 {
	...
	class controls {
		...
		class ButtonJackpot : Life_RscButtonMenu {
			idc = -1;
			onButtonClick = "createDialog ""life_jackpot"";";
			size = "0.022 * safeZoneH";
			x = 0.7607 * safezoneW + safezoneX;
			y = 0.35425 * safezoneH + safezoneY;
			w = 0.0495 * safezoneW;
