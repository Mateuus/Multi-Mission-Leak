...
*/ 
private["_house","_houseID","_ownerID","_housePos","_query","_containers"]; 
...
if(_houseID isEqualTo -1) then { 
 ...
 _query = format["UPDATE houses SET owned='0', pos='[]',security = 0 WHERE pid='%1' AND pos='%2' AND owned='1'",_ownerID,_housePos]; 
 ...
 _query = format["UPDATE houses SET owned='0', pos='[]',security = 0 WHERE id='%1'",_houseID]; 
}; 
...
_containers = nearestObjects[(getPosATL _house),["Box_IND_Grenades_F","B_supplyCrate_F"],25]; 
