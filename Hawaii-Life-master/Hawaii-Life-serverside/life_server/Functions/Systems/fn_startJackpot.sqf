...
// [5,format["<t size= '1.6' color='#f19727'>Hawaii-Jackpot<br/><br/></t><t size= '1.2'>%1hat den Hawaii-Jackpot gestartet mit<br/>$%2<br/>Aktueller Wert:<br/>$%2</t>",_name,[life_gamble_amt] call life_fnc_numberText],false] remoteExecCall ["life_fnc_broadcast",RCLIENT]; 
...
if (_winningIndex >= 0) then { 
 ...
 _placedMoney = _winnerInfo select 4; 
 _totalMoney = life_gamble_amt; 
}; 
if (isNil {_player} || (count life_gamble_teil) isEqualTo 1) then { 
 { 
  _name = _x select 1; 
  _player = _x select 5; 
 ...
 //[0,format["Der gewinner des Hawaii-Jackpots über $%1 ist %2 mit einer Chance von %3%4",[life_gamble_amt * 0.9] call life_fnc_numberText,_name,round((_placedMoney/_totalMoney)*10000)/100,"%"],false] remoteExecCall ["life_fnc_broadcast",RCLIENT]; 
