...
*/ 
private["_animalList","_dist","_radius","_radius2","_animals","_zoneName","_unitsNear","_animalsActive"]; 
...
for "_i" from 0 to 1 step 0 do { 
 ...
 if(_unitsNear) then { 
  if(!_animalsActive) then { 
   _animalsActive = true; 
   for "_i" from 1 to _maxAnimals do { 
    _animalClass = _animalList select floor random count _animalList; 
    _position = [((_zone select 0) - _radius2 + random (_radius2 * 2)), ((_zone select 1) - _radius2 + random (_radius2 * 2)),0]; 
