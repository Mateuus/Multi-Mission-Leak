...
Author:
 Ducky, Repentz
Description:
 Cleans up all vehicles on the map if the players have not been in them for x time
*/

private ["_vehicle","_impound","_vInfo","_vInfo","_plate","_uid","_query","_sql","_trunk","_vehItems","_vehMags","_vehWeapons","_vehBackpacks","_cargo","_saveItems","_resourceItems","_fuel","_damage","_itemList","_totalweight","_weight","_thread"];
_vehicle = [_this,0,objNull,[objNull]] call BIS_fnc_param;
_impound = [_this,1,false,[true]] call BIS_fnc_param;
_resourceItems = LIFE_SETTINGS(getArray,"save_vehicle_items");

if (isNull _vehicle) exitWith {}; //Bad data passed.
