...
switch(_type) do{ 
 ...
 if(_msg == "") exitWith {["You must enter a message to be sent!",false,"slow"] call life_fnc_notificationSystem;
 ctrlShow[88885, true];
};
...
[ObjNull,_msg,player,3] remoteExec ["TON_fnc_handleMessages",2];
 ...
 //adminToPerson 
 case 5: { if((call life_adminlevel) < 1) exitWith {["You are not an admin!",false,"slow"] call life_fnc_notificationSystem;
};
 if(isNULL life_smartphoneTarget) exitWith {hint format["No person selected!"];
 };
 if(_msg == "") exitWith {["You must enter a message to be sent!",false,"slow"] call life_fnc_notificationSystem;
...
[life_smartphoneTarget,_msg,player,4] remoteExec ["TON_fnc_handleMessages",2];
 ...
 //adminMsgAll 
 case 7: { if((FETCH_CONST(life_adminlevel) < 1)) exitWith {["You are not an admin!",false,"slow"] call life_fnc_notificationSystem;
};
 if(_msg == "") exitWith {["You must enter a message to be sent!",false,"slow"] call life_fnc_notificationSystem;
 };
