...
if (!isNil {_vehicle getVariable "mining"}) exitWith {
    [localize "STR_NOTF_DeviceIsMining",true,"slow"] call life_fnc_notificationSystem;
};
...
if ((_weight select 1) >= (_weight select 0)) exitWith {
    [localize "STR_NOTF_DeviceFull",true,"slow"] call life_fnc_notificationSystem;
...
if (_zone isEqualTo "") exitWith {
    [localize "STR_NOTF_notNearResource",true,"slow"] call life_fnc_notificationSystem;
    life_action_inUse = false;
};
