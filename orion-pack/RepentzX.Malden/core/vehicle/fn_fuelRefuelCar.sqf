...
if (isNil "_classname" || _classname isEqualTo "") exitWith {
    [localize "STR_Select_Vehicle_Pump",true,"slow"] call life_fnc_notificationSystem;
    vehiclefuelList = [];
...
if (_car distance player > 10 && !(isNull objectParent player)) exitWith {
    [localize "STR_Distance_Vehicle_Pump",true,"slow"] call life_fnc_notificationSystem;
    vehiclefuelList = [];
    life_action_inUse = false;
    closeDialog 0;
};

if ((civRDMed - (_fueltoput * life_fuelPrices))> 0)then {
    ...
    for "_i" from 0 to 1 step 0 do {
        ...
        if !((civRDMed - round(0.01 * _totalcost))> 0) exitWith {};
        civRDMed = civRDMed - round((0.01 * _totalcost));
        _tp = _tp +1;
        if (_tp == 9) then {
            _tp = 0;
