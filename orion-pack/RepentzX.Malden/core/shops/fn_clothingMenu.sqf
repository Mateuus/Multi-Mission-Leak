...
if !(_exit) exitWith {[localize "STR_Shop_Veh_NoLicense",true,"slow"] call life_fnc_notificationSystem;};
...
//Cop / Civ Pre Check
if (_shop in ["bruce","dive","reb","kart"] && {!(playerSide isEqualTo civilian)}) exitWith {[localize "STR_Shop_NotaCiv",true,"slow"] call life_fnc_notificationSystem; closeDialog 0;};
if (_shop == "cop" && {!(playerSide isEqualTo west)}) exitWith {[localize "STR_Shop_NotaCop",true,"slow"] call life_fnc_notificationSystem; closeDialog 0;};
if (_shop == "dive" && {!license_civ_dive}) exitWith {[localize "STR_Shop_NotaDive",true,"slow"] call life_fnc_notificationSystem; closeDialog 0;};
