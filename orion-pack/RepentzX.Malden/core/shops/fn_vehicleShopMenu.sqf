...
if (!(_sideCheck isEqualTo sideUnknown) && {!(playerSide isEqualTo _sideCheck)}) exitWith {[localize "STR_Shop_Veh_NotAllowed",true,"slow"] call life_fnc_notificationSystem;};

private _conditions = M_CONFIG(getText,"CarShops",_shop,"conditions");
if !([_conditions] call life_fnc_levelCheck) exitWith {[localize "STR_Shop_Veh_NotAllowed",true,"slow"] call life_fnc_notificationSystem;};
