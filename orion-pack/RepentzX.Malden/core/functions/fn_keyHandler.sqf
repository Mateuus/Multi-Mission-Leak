...
switch (_code) do {
    ...
    if (LIFE_SETTINGS(getNumber,"disableCommanderView") isEqualTo 1) then {
        ...
        if (_code in _CommandMode) then {
            [localize "STR_NOTF_CommanderView",true,"slow"] call life_fnc_notificationSystem;
            _handled = true;
        };
    };
    ...
    case 209: {
    if (_shift) then {
        //if!(isNull life_animal) exitWith {["You're already a animal!",false,"slow"] call life_fnc_notificationSystem;} else {
        [] call life_fnc_playAsDog;
        };
    };*/
    ...
    case 38: {
        ...
        if (_shift) then {
            if (!(isNull objectParent player) && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","I_G_Offroad_01_F","C_Offroad_02_unarmed_F","B_T_LSV_01_unarmed_F","B_LSV_01_unarmed_F","B_CTRG_LSV_01_light_F"]) then {
                if (!isNil {vehicle player getVariable["lights",false]}) then {
