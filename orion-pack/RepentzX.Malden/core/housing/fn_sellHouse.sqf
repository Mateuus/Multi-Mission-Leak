...
if (_action) then {
    ...
    civRDMed = civRDMed + (round((_houseCfg select 0)/2));
    [1] call SOCK_fnc_updatePartial;
    _index = life_vehicles find _house;

    if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
        if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
            advanced_log = format [localize "STR_DL_AL_soldHouse_BEF",(round((_houseCfg select 0)/2)),[civRDMed] call life_fnc_numberText];
        } else {
            advanced_log = format [localize "STR_DL_AL_soldHouse",profileName,(getPlayerUID player),(round((_houseCfg select 0)/2)),[civRDMed] call life_fnc_numberText];
            };
        publicVariableServer "advanced_log";
    };
    [format["%1 sold their house for $%2.",profileName,(round((_houseCfg select 0)/2))], "MoneyLogs"] remoteExecCall ["A3Log", 2];
