...
try {
	if (civRDM < _purchasePrice) then {
		if (civRDMed < _purchasePrice) then {
			hint parseText format [localize "STR_Shop_Veh_NotEnough",[_purchasePrice - civRDMed] call life_fnc_numberText];
			throw "No money";
		};
	};
			
	if ((civRDMed > _purchasePrice) && (civRDM < _purchasePrice)) then {
		if (LIFE_SETTINGS(getNumber,"debit_tax") isEqualTo 1) then {
			...
			civRDMed = civRDMed - _purchasePrice;
			[1] call SOCK_fnc_updatePartial;
...
if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        advanced_log = format [localize "STR_DL_AL_boughtVehicle_BEF",_className,[_purchasePrice] call life_fnc_numberText,[civRDM] call life_fnc_numberText,[civRDMed] call life_fnc_numberText];
    } else {
        advanced_log = format [localize "STR_DL_AL_boughtVehicle",profileName,(getPlayerUID player),_className,[_purchasePrice] call life_fnc_numberText,[civRDM] call life_fnc_numberText,[civRDMed] call life_fnc_numberText];
    };
