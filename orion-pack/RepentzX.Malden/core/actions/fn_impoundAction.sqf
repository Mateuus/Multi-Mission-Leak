...
if (_vehicle getVariable "NPC") exitWith {[localize "STR_NPC_Protected",true,"slow"] call life_fnc_notificationSystem;
...
for "_i" from 0 to 1 step 0 do {
    sleep 0.3;
    _cP = _cP + (0.01 * (missionNamespace getVariable ["mav_ttm_var_impoundMultiplier", 1]));
...
if (player distance _vehicle > 10) exitWith {[localize "STR_NOTF_ImpoundingCancelled",true,"slow"] call life_fnc_notificationSystem; life_action_inUse = false;};
