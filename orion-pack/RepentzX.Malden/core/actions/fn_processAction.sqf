...
if (_exit) exitWith {life_is_processing = false; [localize "STR_NOTF_NotEnoughItemProcess",true,"slow"] call life_fnc_notificationSystem; life_action_inUse = false;};
...
if (_exit) exitWith {[localize "STR_Process_Weight",true,"slow"] call life_fnc_notificationSystem; life_is_processing = false; life_action_inUse = false;};
...
if (_hasLicense) then {
    ...
    if (player distance _vendor > 10) exitWith {[localize "STR_Process_Stay",true,"slow"] call life_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
	...
	[format["%1 Has Processed %2", profileName, _newItem], "MoneyMaking"] remoteExecCall ["A3Log", 2];
    for "_i" from 0 to (((_newItem select 0) select 1)*(_minimumConversions)) do {
    ["ItemProcessed"] spawn mav_ttm_fnc_addExp;
};
} else {
    if (civRDM < _cost) exitWith {hint format [localize "STR_Process_License",[_cost] call life_fnc_numberText]; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
