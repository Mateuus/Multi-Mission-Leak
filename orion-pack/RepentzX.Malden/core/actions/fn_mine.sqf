...
While {true} do {
    if (vehicle player != player) exitWith {[localize "STR_ISTR_Pick_MineVeh",true,"slow"] call life_fnc_notificationSystem;};
    if (life_interrupted) exitWith {life_interrupted = false;}; // Damit ist die Zeile hier gemeint 
    if (life_action_inUse) exitWith {};
    if !(isNull objectParent player) exitWith {};
    if (player getVariable "restrained") exitWith {
        [localize "STR_NOTF_isrestrained",true,"slow"] call life_fnc_notificationSystem;

    };
    _exit = false;
    if (player getVariable "playerSurrender") exitWith {
        [localize "STR_NOTF_surrender",true,"slow"] call life_fnc_notificationSystem;
    };
...
Life_action_inUse = false; // The loop ends over this variable!