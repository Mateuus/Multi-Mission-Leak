...
if (!(_ctrl isEqualTo "goldenbarz3")) exitWith {hint localize "STR_Cop_OnlyGold"};
if (_num > life_inv_goldenbarz3) exitWith {hint format [localize "STR_Cop_NotEnoughGold",_num];};

//Store it.
if (!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {[localize "STR_Cop_CantRemove",true,"slow"] call life_fnc_notificationSystem;};
