...
if (_safeInfo < 1) exitWith {closeDialog 0; [localize "STR_Civ_VaultEmpty",true,"slow"] call life_fnc_notificationSystem;};

_str = M_CONFIG(getText,"VirtualItems","goldenbarz3","displayName");
_shrt = M_CONFIG(getText,"VirtualItems","goldenbarz3","variable");
_tInv lbAdd format ["[%1] - %2",_safeInfo,(localize _str)];
_tInv lbSetData [(lbSize _tInv)-1,_shrt];
_icon = M_CONFIG(getText,"VirtualItems","goldenbarz3","icon");
