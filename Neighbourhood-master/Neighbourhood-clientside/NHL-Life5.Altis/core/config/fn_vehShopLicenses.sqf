...
switch (life_veh_shop select 0) do
{
	...
	case "tuning": {_ret = license_civ_driver;};
	...
	case "bm_car": {_ret = license_civ_bm;};
	case "opsec": {_ret = license_civ_opsec;};
	case "whinc": {_ret = license_civ_whinc;};	//wh inc
	case "mrsf": {_ret = license_cop_mrsf;};
	case "cc": {_ret = license_civ_cc;};
