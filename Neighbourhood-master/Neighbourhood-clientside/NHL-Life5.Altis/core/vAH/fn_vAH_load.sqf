...
if(life_erfahrung <=100) exitWith {hint "Du kannst das Auktionshaus erst mit Level 1 benutzen!", closeDialog 0;};
...
if (count all_ah_items == 0) then {
		_myListbox lbAdd format ["Aktuell sind keine Gegenstände inseriert"];
	...
	} else {
{
	...
	_time = round (_time / 3);
	if (_status == 0)then {
	_myListbox lbAdd format ["%5 mal %3 für $%2 , läuft in %4 Std. aus",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText,[_amount] call life_fnc_numberText];
...
while {true} do
{
	...
	if (isNull (findDisplay 15100)) exitWith {};
	sleep 1;
	((findDisplay 15100) displayCtrl 15160) ctrlSetText "Texturen\signs\ah.jpg";
	if (isNull (findDisplay 15100)) exitWith {};
	sleep 0.1;
	((findDisplay 15100) displayCtrl 15160) ctrlSetText "Texturen\signs\ah.jpg";
	if (isNull (findDisplay 15100)) exitWith {};
