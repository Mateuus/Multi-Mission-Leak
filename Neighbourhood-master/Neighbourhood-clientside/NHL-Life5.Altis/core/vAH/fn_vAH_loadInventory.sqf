...
if (_uniform!="") then {
 ...
 {   _val = missionNameSpace getVariable _x;   if (_val > 0) then {_name = [_x,1] call life_fnc_varHandle; playerInventoryArray pushBack [_name,_val,3,"ymenu"]};  } foreach life_inv_items; 
...
if (count playerInventoryArray == 1) then {
		_myInvListbox lbAdd format ["Du hast keine Items im Inventar/Auktionshaus!"];
	} else {
{
	...
	switch (_status) do {
		...
		case 3: {_stat = ": In meinem Inventory"}; //MY INV
	};
	...
	_index = _index + 1;
	if (_item == "yolo") then {
			_myInvListbox lbAdd format ["--------------------AUKTIONSHAUS ITEMS-----------------------"];
	...
	switch (_type) do {
		case "imenu": {
			...
			_time = round (_time / 3);
			if (_time <= 0) then {_price = 0; if (_status == 1) then {_toDel = round(_time + 216); _timeDisplay = format[": Wird in %1 Std. gelöscht",_toDel]} else {_timeDisplay = "";};} else {_timeDisplay = format[": Expires in %1 Hrs",_time]};
			if (_price == 0) then {_priceDisplay = ""} else {_priceDisplay = format[": Auktionspreis $%1",[_price]call life_fnc_numberText];};
			};
		case "ymenu": {
...
while {true} do
{
	...
	if (isNull (findDisplay 15600)) exitWith {};
	sleep 1;
	((findDisplay 15600) displayCtrl 15660) ctrlSetText "Texturen\signs\ah.jpg";
	if (isNull (findDisplay 15600)) exitWith {};
	sleep 0.1;
	((findDisplay 15600) displayCtrl 15660) ctrlSetText "Texturen\signs\ah.jpg";
	if (isNull (findDisplay 15600)) exitWith {};
