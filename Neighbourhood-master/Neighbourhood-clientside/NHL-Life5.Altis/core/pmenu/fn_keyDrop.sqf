...
_sel = lbCurSel _list;
if(_sel == -1) exitWith {hint "Es wurde nichts Ausgewählt";};
_index = (parseNumber(_list lbData _sel));
_vehicle = life_vehicles select _index;
if(_vehicle isKindOf "House_F") exitWith {hint "Du kannst doch deinen Haustürschlüssel nicht einfach wegwerfen!"};
