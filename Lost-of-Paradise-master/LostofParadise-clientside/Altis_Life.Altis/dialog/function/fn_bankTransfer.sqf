...
if(_val > life_atm5cash) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_atm5cash) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

life_atm5cash = life_atm5cash - (_val + _tax);

[[_val,profileName],"TON_fnc_clientWireTransfer",_unit,false] spawn life_fnc_MP;
[[player, _unit, _val, 0, life_atm5cash, life_6cash],"TON_fnc_handleDBLog",false] spawn life_fnc_MP; //By Nukefliege
