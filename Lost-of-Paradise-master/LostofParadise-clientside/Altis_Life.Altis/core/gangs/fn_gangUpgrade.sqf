...
if(_action) then {
	if(life_atm5cash < _upgradePrice) exitWith {
		hint parseText format[
			...
			[life_atm5cash] call life_fnc_numberText,
			[_upgradePrice - life_atm5cash] call life_fnc_numberText
		];
	};
	__SUB__(life_atm5cash,_upgradePrice);
