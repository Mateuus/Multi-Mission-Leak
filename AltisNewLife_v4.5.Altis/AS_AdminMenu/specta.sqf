...
cutText ["Initialisation du mode spectateur...","BLACK IN", 2];
...
[] spawn {sleep(3);if(dialog) then {cutText["\n\n\n\n\nPress F1 pour de l'aide","PLAIN DOWN", 0.75]}};
...
KEGs_fnc_MovementCameraLoop =
	{
		...
		While { VM_SpectatorCamerasEnabled  } do
		{
			...
			//while { uiSleep 0.05; ( ( !(isPlayer KEGs_target) || ((isPlayer KEGs_target) && { (alive KEGs_target) } )) && { KEGsNextRun } ); } do
			...
			if ( (isNull KEGs_target) ) then
			{
				...
				waitUntil { uiSleep 0.5; if ( (isNull KEGs_target) && ( alive player ) ) then { KEGs_target = player}; ( !(isNull KEGs_target) ); };
