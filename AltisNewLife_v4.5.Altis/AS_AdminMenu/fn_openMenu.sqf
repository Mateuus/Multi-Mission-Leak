...
if (AS_level < 1) exitWith {hint "Vous n'etes pas autorisé a faire ca !";};
...
} forEach playableUnits;
{
	if (vehicle _x == _x) then
	{
		if (currentWeapon _x != "") then {
			_playersLB lbAdd format ["%1 [ARME : %2] ",name _x,(([currentWeapon _x] call VAS2_fnc_fetchCfgDetails) select 1)];
		...
		_playersLB lbAdd format ["%1 [DANS : %2]",name _x,vehicle _x];
		if (uniform _x != "") then {
			_playersLB lbSetPicture [(lbSize _playersLB)-1, (([uniform _x] call VAS2_fnc_fetchCfgDetails) select 2)];
			_playersLB lbSetPictureColor [(lbSize _playersLB)-1, [1, 1, 1, 1]];
		};
