...
uiSleep 0.1;
		...
		_uniqueName=format ["<t color=""#E4FB44"">" + ("DIRECT : %1") + "</t>",name _x];
		_monitor addAction [_uniqueName, "call fnc_PROCESS_LIVE",[_x,_debug],0,false,true,"","_target distance _this < 3.5"];
				
			_uniqueName=format ["<t color=""#84EFF2"">" + ("VUE SATELLITE : %1") + "</t>",name _x];
	...
	if (_debug) then {_sidechatHint = format["Total de camera : %1", _cameraCount];PAPABEAR sideChat _sidechatHint;GlobalSideChat = _sidechatHint; publicVariable "GlobalSideChat";};
...
private ["_name","_cam","_debug","_class","_newClass","_arguments"];
	...
	_monitor setObjectTextureglobal [0,"images\event\standby.jpg"];
	SLEEP 0.6;
...
if (_debug) then {_sidechatHint = format["Camera active : %1",count _conCurrentCams];PAPABEAR sideChat _sidechatHint;GlobalSideChat = _sidechatHint; publicVariable "GlobalSideChat";};
...
if (_debug) then {_sidechatHint = format["Eteindre la camera : %1",count _conCurrentCams];PAPABEAR sideChat _sidechatHint;GlobalSideChat = _sidechatHint; publicVariable "GlobalSideChat";};
					};
			};
fnc_RENDER = {
	_monitor=(_this select 0);
