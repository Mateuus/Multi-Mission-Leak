...
if(!(_house in life_vehicles)) exitWith {hint "Vous devez être à l'intérieur de votre maison pour placer cet objet"};
_containers = _house getVariable["containers",[]];
_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(count _houseCfg == 0) exitWith {}; //What the fuck happened?
if(count _containers >= (_houseCfg select 1)) exitWith {hint "Vous ne pouvez pas placer plus de stockage a l'interieur de cette maison"};
...
if(_pos isEqualTo [0,0,0]) exitWith {hint "Vous n'avez plus de place de stockage dans la maison"};
