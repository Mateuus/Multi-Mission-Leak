...
if(_shooter isKindOf "Man" && alive player) then
{
	if(!life_istazed) then
	{
		...
		_attach = if(RIFLE != "") then {primaryWeaponItems _unit} else {[]};
		...
		//[[0,format["%1 a été tazzé par %2", _unit getVariable["realname",name _unit], _shooter getVariable["realname",name _shooter]]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
