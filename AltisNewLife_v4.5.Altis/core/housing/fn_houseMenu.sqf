...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget getVariable "house_owner"}) then {
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		_Btn4 ctrlSetText "Systeme de securite";
		_Btn4 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_secureHouse; closeDialog 0;";
		_Btn4 ctrlShow true;
		if(life_inv_sechouse > 0) then {_Btn4 ctrlEnable true;} else {_Btn4 ctrlEnable false;};
