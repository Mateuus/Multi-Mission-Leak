...
if(life_sidechat) then {
	...
	_side ctrlSetText "OUI";
} else {
	_side ctrlSetTextColor [1,0,0,1];
	_side ctrlSetText "NON";
};

if(!(life_fps)) then {
	_fps ctrlSetTextColor [1,0,0,1];
	_fps buttonSetAction "life_fps = true; [] call life_fnc_fps; [] call life_fnc_settingsMenu;";
	_fps ctrlSetText "NON";
} else {
	_fps ctrlSetTextColor [0,1,0,1];
	_fps buttonSetAction "life_fps = false; [] call life_fnc_fps; [] call life_fnc_settingsMenu;";
	_fps ctrlSetText "OUI";
};