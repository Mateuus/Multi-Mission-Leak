...
_config = [_itemFilter] call life_fnc_craftCfg;
{
	if(_itemFilter == "item") then{
		//_str = M_CONFIG(getText,"ANL_VItems",_x select 0,"displayName");	
		...
		_icon = M_CONFIG(getText,"ANL_VItems",_x select 0,"icon");		
...
_config = [_itemFilter] call life_fnc_craftCfg;
{
	...
	if(_item == _x select 0)then
	{
		...
		for [{_i = 0},{_i < _invSize - 1},{_i = _i + 2}] do {
			//_str = M_CONFIG(getText,"ANL_VItems",_matsNeed select _i,"displayName");
