...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText["Vous avez été relaché, votre peine a été purgée","PLAIN"];
		};
	};
};

if((player getVariable["surrender",FALSE])) then { player SVAR["surrender",FALSE,TRUE]; player switchMove ""; };
titleText[format["Vous avez été arreté par %1",_cop getVariable["realname",name _cop]],"PLAIN"];
