...
if((_this select 3) == "reb" && !liciv_rebel) exitWith {hint "Vous devez passer votre formation rebelle"; closeDialog 0;};
if((_this select 3) in ["dive"] && !liciv_dive) exitWith {hint "Désolé, vous devez achetez la licence de plongée."; closeDialog 0;};
};

if((_this select 3) in ["cop"] && playerSide != west ) exitWith {hint "Vous devez être de la gendarmerie pour utiliser ce magasin !"; closeDialog 0;};
if((_this select 3) in ["medic"] && playerSide != independent ) exitWith {hint "Vous devez être medecin pour utiliser ce magasin !"; closeDialog 0;};
