...
if((time - life_action_delay) < 2) exitWith {hint "Vous avez trop mal au dos pour vous baisser si rapidement !";};
...
if(playerSide == west && (EQUAL(_illegal,1))) exitWith {
	titleText[format[localize "STR_NOTF_PickedEvidence",localize _itemName,[round(ITEM_SELLPRICE(SEL(_itemInfo,0)) / 2)] call life_fnc_numberText],"PLAIN"];
	ADD(compte_banque,round(ITEM_SELLPRICE(SEL(_itemInfo,0)) / 2));
	deleteVehicle _this;
