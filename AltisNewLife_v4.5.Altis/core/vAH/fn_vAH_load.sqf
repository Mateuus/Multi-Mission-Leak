...
if (count all_ah_items == 0) then {
		_myListbox lbAdd format ["Il n'y a pas encore d'objet dans le marché noir !"];
	...
	} else {
{
	...
	_time = round (_time / 3);
	if (_status == 0)then {
	_myListbox lbAdd format ["%5 lots de %3 pour %2€ par %1, expire dans %4 heures",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText,[_amount] call life_fnc_numberText];
...
hint "Offres chargees avec succes";
...
while {true} do
{
	...
	if (isNull (findDisplay 15100)) exitWith {};
	uiSleep 1;
	((findDisplay 15100) displayCtrl 15160) ctrlSetText "";
	if (isNull (findDisplay 15100)) exitWith {};
	uiSleep 0.1;
	((findDisplay 15100) displayCtrl 15160) ctrlSetText "";
	if (isNull (findDisplay 15100)) exitWith {};
