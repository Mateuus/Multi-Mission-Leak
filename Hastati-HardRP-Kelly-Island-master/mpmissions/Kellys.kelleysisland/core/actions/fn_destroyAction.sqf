...
[0,"STR_NOTF_VehBeingDestroyed",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
life_action_inUse = true;

_upp = localize "STR_NOTF_VehDestroying";
...
if(player distance _vehicle > 10) exitWith {hint localize "STR_NOTF_VehDestroyCancelled"; life_action_inUse = false;};
...
if(EQUAL(count crew _vehicle,0)) then {
	...
	[_vehicle] remoteExecCall ["TON_fnc_vehicleDead",RSERV];
	waitUntil {!life_impound_inuse};
	hint format[localize "STR_NOTF_VehDestroyed",_type,_price];
	[0,"STR_NOTF_HasDestroyed",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
	ADD(BANK,_price);
} else {
	hint localize "STR_NOTF_VehDestroyCancelled";
};
