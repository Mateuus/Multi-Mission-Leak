...
_uid = _data select 1;
{ if (getPlayerUID _x == _uid) then { _unit = _x; if (_x getVariable ["restrained",false]) then { _exit = true; }; }; } forEach playableUnits;
...
{
	[[format["%1 wants to pardon you, do you accept?",name player], player],"life_fnc_confirmGovPardon",_unit,false] spawn BIS_fnc_MP;
...
[[_uid],"life_fnc_wantedPardon",false,false] spawn BIS_fnc_MP;
[[format["%1 has been pardoned by %2",_data select 0, name player]],"life_fnc_localChatMessage",true,false] spawn BIS_fnc_MP;