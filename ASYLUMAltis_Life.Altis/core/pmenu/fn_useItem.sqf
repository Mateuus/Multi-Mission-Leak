...
switch (true) do
{
	...
	case (_item in ["beer","moonshine","whiskeyc_1","whiskeyc_2","whiskeyc_3","whiskeyr_1","whiskeyr_2","whiskeyr_3","scotch_1","scotch_2","scotch_3"]):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			life_thirst = 100;
	...
	case (_item == "campfire"):
	{
		...
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			...
			_fire addAction["Cook Raw Meat",life_fnc_cookMeat,cursorObject,-1,false,false,"",'!isNull cursorObject && (player distance cursorObject) < 4 && (typeOf cursorObject) == "Campfire_burning_F" && (life_inv_chicken > 0 || life_inv_dog > 0 || life_inv_sheep > 0 || life_inv_goat > 0) '];
			_fire addAction["Clean Up Campfire",{deleteVehicle cursorObject},cursorObject,-1,false,false,"",'!isNull cursorObject && (player distance cursorObject) < 4 && (typeOf cursorObject) == "Campfire_burning_F" '];
		};
	};
	...
	case (_item in ["berry","apple","salema","ornate","mackerel","tuna","mullet","catshark","turtle","turtlesoup","donuts","tbacon","peach","redburger","chickenp","dogp","sheepp","goatp","rabbitp","snakep","toasty","sandwich"]):
	{
