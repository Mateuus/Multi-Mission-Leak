	...
	{
		{
			if(life_worldspace distance getMarkerPos _x < 150) exitWith {
				...
				[[[1,2],"A potentially hostile player has entered the world nearby!"],"life_fnc_broadcast",_players,false] spawn BIS_fnc_MP;
			};
...
[] spawn
{
	...
	while {true} do
	{
		...
		if ((_prev in ["srifle_DMR_01_F","srifle_EBR_F","srifle_DMR_03_khaki_F","srifle_DMR_03_multicam_F","srifle_DMR_03_tan_F","srifle_DMR_03_woodland_F","srifle_DMR_06_olive_F","srifle_DMR_06_camo_F","hlc_rifle_akm","CUP_srifle_AWM_des","CUP_srifle_AWM_wdl"]) && !(31 in life_talents)) then { player removeWeapon (primaryWeapon player); };
		if (life_is_arrested && _prev != "") then { player removeWeapon (primaryWeapon player); };
...
[] spawn
{
	while {true} do
	{
		waitUntil {sleep 300; life_wanted > 0};
		if (round (random 1) == 0) then
		{
			...
			{
				[["0",name player,6],"life_fnc_clientMessage",true,false] spawn BIS_fnc_MP;
			}
			else
			{
				...
				{
					[["1",name player,6],"life_fnc_clientMessage",true,false] spawn BIS_fnc_MP;
				};
			};
		};
	};
...
[] spawn
{
	...
	while {true} do
	{
		waitUntil {sleep 60; _distance = if (worldName == "Australia") then { 500 } else { 150 }; _lastRobbed = 0; _cops = 0; { if (side _x == west) then { _cops = _cops + 1; }; } forEach playableUnits; if (!isNil "life_bank_lastRobbed") then { _lastRobbed = life_bank_lastRobbed; }; (!life_bank_inProgress && (player distance (getMarkerPos "fed_reserve") < _distance) && (_lastRobbed < (time - 600)) && _cops > 5)};
		...
		if (_detected && (life_inv_drill > 0 || life_inv_goldbar > 0 || life_inv_boltCutter > 0 || life_inv_demoCharge > 0)) then
		{
			...
			if (!_concealed) then
			{
				...
				[[player],"ASY_fnc_fedAlarm",false,false] spawn BIS_fnc_MP;
			};
		};
	};
};
...
//[[24, player, format["Initialized as civilian"]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;