...
_label = format["<t font='puristaMedium' shadow='1' size='2'>%1</t><br/>%2 Prestige. ", _talent select 2, [_talent select 1] call life_fnc_numberText];
...
if ((_talent select 0) in life_coptalents) then { _label = format["%1<t color='#00FF00'>You have learned this talent.</t><br/><br/>", _label]; }
else
{
	if (count _req > 0) then
	{
		if (!((_req select 3) in life_coptalents)) then { _label = format["%1<t color='#FF0000'>You must learn %2 before you can learn this.</t><br/><br/>", _label, _req select 2]; }
...
_learn ctrlEnable (((_talent select 3) == 0 || (_talent select 3) in life_coptalents) &&  !((_talent select 0) in life_coptalents));