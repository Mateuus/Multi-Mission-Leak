...
for "_i" from 0 to ((count life_prestigeInfo) - 1) do
{
	_x = life_prestigeInfo select _i;
	if ((_x select 3) == _parent) then
	{
		...
		if (!(_id in life_coptalents)) then { _list lbSetColor [(lbSize _list)-1,[1,1,1,0.5]]; };
		if (!((_x select 3) in life_coptalents) && (_x select 3) > 0) then { _list lbSetColor [(lbSize _list)-1,[1,0,0,0.9]]; };
