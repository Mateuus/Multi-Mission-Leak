...
[] spawn  {
	while{true} do
	{
		...
		if(!(player getVariable ["restrained", false] && !(player getVariable ["isCivRestrained", false]))) then {
			_change = 15;
			if (59 in life_talents) then { _change = 11; };
			if (60 in life_talents) then { _change = 7; };
...
[] spawn
{
	while{true} do
	{
		...
		waitUntil {(_oldDamage != damage player) || _canRevive != (player getVariable["can_revive", 0]) || (player getVariable["can_revive",-1000]) > serverTime};
