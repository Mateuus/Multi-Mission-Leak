...
if(_continue) then {
	...
	[[player],"life_fnc_revived",_target,FALSE] spawn BIS_fnc_MP;
	...
	if (_target isKindOf "Man") then
	{
		...
		{
			["atm","add", life_revive_fee] call life_fnc_uC;
			titleText[format["You have revived %1 and received $%2 for your services.",_targetName,[life_revive_fee] call life_fnc_numberText],"PLAIN"];
