...
[[player,true],"ASY_fnc_addToEvent",false,false] spawn BIS_fnc_MP;
...
while {true} do
{
	...
	if (count life_active_event > 0) then
	{
		...
		if (player distance life_event_startpos > 30 && (_end - time) > 10) then { _exit = true; hint "You've gone too far from the event entry location and have been removed from the event."; [[player,false],"ASY_fnc_addToEvent",false,false] spawn BIS_fnc_MP; };
	};
	if (_exit) exitWith {};
};