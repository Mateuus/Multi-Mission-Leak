...
{
	[[_target,true],"life_fnc_setRestrained",false,false] spawn BIS_fnc_MP;
};
...
[[_target,"AmovPercMstpSnonWnonDnon","playNow"],"life_fnc_animSync",true,false] spawn BIS_fnc_MP;
...
while {_continue} do
{
	...
	waitUntil {
		_timeUp = (!_wasCop || (vehicle player == player && _wasCop && ({side _x == west && player distance _x < 500} count playableUnits == 0))) && time - _startTime > _maxRestrainTime &&  !(_target getVariable ["Escorting",false]) && !(_target getVariable ["transporting",false]);
...
if({side _x == west && player distance _x < 400} count playableUnits == 0) then {[[player,false],"life_fnc_setRestrained",false,false] spawn BIS_fnc_MP;};
[[_target,"AmovPercMstpSnonWnonDnon","playNow"],"life_fnc_animSync",true,false] spawn BIS_fnc_MP;
