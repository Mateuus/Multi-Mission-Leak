...
//life_return_value = "";
//[[_itemName, player],"ASY_fnc_verifyPickup",false,false] call BIS_fnc_MP;
...
if(playerSide == west && _illegal != -1) exitWith
{
	...
	//[[31, player, format["Picked up %1 and placed in evidence for %2 reward", _itemName, (life_illegal_items select _illegal)]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
...
if(_diff != _itemInfo select 1) then
{
	if(([true,_itemInfo select 0,_diff] call life_fnc_handleInv)) then
	{
		...
		//[[32, player, format["Picked up %1 %2", _diff, _itemName]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	};
}
	else
{
	if(([true,_itemInfo select 0,_itemInfo select 1] call life_fnc_handleInv)) then
	{
		...
		//sleep 0.5;
		titleText[format["You picked up %1 %2",_diff,_itemName],"PLAIN"];
		//[[33, player, format["Picked up %1 %2", _diff, _itemName]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	};
};

_logItems = ["dirty_money","diamondf","storage1","storage2","money","meth","speedbomb"];
if ((_itemInfo select 0) in _logItems) then { [[33, player, format["Picked up %1 %2", _diff, _itemName]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP; };