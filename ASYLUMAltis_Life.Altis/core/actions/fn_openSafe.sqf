...
if (!_explosive) then
{
	...
	while{true} do
	{
		if(animationState player != "acts_carfixingwheel") then {
			...
			[[player,"Acts_carFixingWheel"],"life_fnc_animSync",true,false] spawn BIS_fnc_MP;
		};
		sleep 0.38;
		_cP = _cP + 0.01;
...
else
{
	...
	[[player, "bombarm",30],"life_fnc_playSound",true,false] spawn BIS_fnc_MP;
	sleep 5;
	_success = true;
};
...
if (_success) then
{
	...
	if (!life_bank_inProgress) then
	{
		...
		if ((floor random _chance) == 0) then {	[[player],"ASY_fnc_fedAlarm",false,false] spawn BIS_fnc_MP; };
	};
};