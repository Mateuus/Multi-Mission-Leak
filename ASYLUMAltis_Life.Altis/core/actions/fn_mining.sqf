...
_yield = switch(true) do {
	case ({player distance getMarkerPos format["mine_%1",_x] < 100} count [1,3,4,5,7] > 0): {1};
	case (player distance (getMarkerPos "mine_2") < 100): {0.7};
	case (player distance (getMarkerPos "mine_6") < 100): {2};
};
...
[] spawn
{
	...
	{
		if (_i == 0) then { [[player, "pickaxe",10],"life_fnc_playSound",true,false] spawn BIS_fnc_MP; _i = 1; }
		else { _i = 0; };
...
while{life_carryWeight < life_maxWeight - 3} do 
{
	...
	{
		case (!(49 in life_talents) && floor (random (4 * _yield)) < 1): {"rock" };
		case (floor (random (500 / _yield)) < 1): { "diamondf" };
		case (floor (random (6 / _yield)) < 1): { "diamond" };
		case ((52 in life_talents) && floor (random (3 / _yield)) < 1): {"diamond" };
		case (floor (random (4 / _yield)) < 1): { "ironore" };
		case ((51 in life_talents) && floor (random (3 / _yield)) < 1): {"ironore" };
		case (floor (random (3 / _yield)) < 1): { "copperore" };
		case (floor (random (4 / _yield)) < 1): { "sand" };
		default { "rock" };
	};
	life_last_gather = time;
	if (_random == "rock") then
	{
		sleep _delay;
