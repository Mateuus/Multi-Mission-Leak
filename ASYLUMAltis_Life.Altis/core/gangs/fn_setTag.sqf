...
switch(_this select 0) do {
	...
	case 2: {
		...
		[[3,life_gang,_tag],"life_fnc_setTag",false,false] call bis_fnc_MP;
		closeDialog 0;
		hint format["You have chosen your gang tag as [%1]",_tag];
	};
	case 3: {
		//For Paratus: maybe replace this with SQL. 
		//_query = format["UPDATE gangs SET gangTag=%1 WHERE id=%2"]......
		profileNameSpace setVariable[format["tag%1",_this select 1],_this select 2];
	};
	case 4: {
		//And this, called from forceJoinGroup because idk.
