...
_members = 0; { if (_x getVariable ["gang","0"] == life_gang && player distance _x < _maxDistance) then { _members = _members + 1; }} forEach playableUnits;
...
while{true} do
{
	...
	_members = 0; { if (_x getVariable ["gang","0"] == life_gang && player distance _x < 2000) then { _members = _members + 1; }} forEach playableUnits;
