...
private ["_send", "_playableUnits", "_display", "_units"];
...
if (_send) then
{
	...
	closeDialog 0;
	[[player,life_gang, player getVariable ["gangName","unknown"]],"life_fnc_invitedGang",_info,false] spawn BIS_fnc_MP;
}
else
{
	...
	_playableUnits = [];
	{
		if ((side _x) == civilian && (_x getVariable ["gangName", ""]) == "") then { _playableUnits set [count _playableUnits, name _x]; };
