...
0 cutFadeOut 9999999;
[[player,life_adminlevel],"ASY_fnc_requestQueue",false,false] spawn BIS_fnc_MP;
...
[[player,true],"life_fnc_simDisable",false,false] spawn BIS_fnc_MP;
...
[] spawn
{
	...
	while {true} do
	{
		...
		if ((_prev == "U_B_HeliPilotCoveralls" && !difficultyEnabledRTD) || (_prev == "U_O_GhillieSuit" && !(30 in life_talents)) || (_prev == "V_HarnessOGL_brn" && !(33 in life_talents)) || (_prev == "U_NikosAgedBody" && (getPlayerUID player != (life_configuration select 0)))) then { removeUniform player; };
		[] call life_fnc_equipGear;
		waitUntil {sleep 5;(_prev != (uniform player) || _diff != (str difficultyEnabledRTD))};
	};
};
...
[] spawn
{
	...
	while {true} do
	{
		...
		if (_prev == "H_PilotHelmetHeli_O" && !((vehicle player) isKindOf "Air")) then { removeHeadgear player; };
		waitUntil {sleep 5;_prev != (headgear player)};
	};
};
