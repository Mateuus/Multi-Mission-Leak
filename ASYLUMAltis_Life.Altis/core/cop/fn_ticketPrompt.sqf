...
_control ctrlSetStructuredText parseText format["<t align='center'><t size='.8px'>%1 has given you a ticket for $%2",name _cop,[life_ticket_val] call life_fnc_numberText];

[] spawn
{
	disableSerialization;
	waitUntil {life_ticket_paid OR life_ticket_attempted OR (isNull (findDisplay 2600))};
	if(isNull (findDisplay 2600) && !life_ticket_attempted) then
	{
		[[0,format["%1 refused to pay the ticket.",name player]],"life_fnc_broadcast",west,false] spawn BIS_fnc_MP;
		[[1,format["%1 refused to pay the ticket.",name player]],"life_fnc_broadcast",life_ticket_cop,false] spawn BIS_fnc_MP;
	};
};