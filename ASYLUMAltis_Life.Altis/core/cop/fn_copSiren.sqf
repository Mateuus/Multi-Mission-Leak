...
if(!_upstroke || (_upstroke && _forcedPhaser) || (_upstroke && _hasSiren && (_cycle || _changeMode))) then {
	...
	if(!_hasSiren) then {
		...
		if(!(_vehicle getVariable ["lights", false])) then {
			[_vehicle,"life_fnc_copLights",true,false] spawn BIS_fnc_MP;
		};
	};
	...
	if(_forcedPhaser) then {
		[[_vehicle, _sirenLogic, "SirenPhaser", _phaserLength, false, player], "life_fnc_playCopSiren", true, false, false] call BIS_fnc_MP;
	} else {
		// otherwise, play your normal first siren sound
		[[_vehicle, _sirenLogic, _sirenType, _sirenDuration, false, player], "life_fnc_playCopSiren", true, false, false] call BIS_fnc_MP;
		...
		if(_vehicle getVariable "sirenDual") then {
			...
			[[_vehicle, _sirenLogic, _dualType, _dualDuration, _sameDual, player], "life_fnc_playCopSiren", true, false, false] call BIS_fnc_MP;
		};
	};
