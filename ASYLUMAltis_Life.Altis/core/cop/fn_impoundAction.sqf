...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if (_seize) then 
	{
		[[0,format["%1 your %2 is being seized by the police.",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn BIS_fnc_MP;
		_upp = "Seizing Vehicle";
	} else {
		[[0,format["%1 your %2 is being impounded by the police.",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn BIS_fnc_MP;
	...
	//End Edit
	{
			...
			if (_seize) then 
			{
				...
				[[_vehicle],"ASY_fnc_vehicleDead",false,false] spawn BIS_fnc_MP;
				hint format["You have seized a %1\n\nYou have received $%2 for cleaning up the streets!",_type,_price];
			}
			else
			{
				life_impound_inuse = true;
				[[_vehicle,true,player],"ASY_fnc_vehicleStore",false,false] spawn BIS_fnc_MP;
