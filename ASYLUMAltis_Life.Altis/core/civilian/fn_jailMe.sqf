...
life_breakout = false;
[] call life_fnc_seizePlayerIllegalAction;
...
while {life_is_arrested} do
{
	...
	if(worldName != "Australia" && player distance (getMarkerPos _jailSpawn) > 53 && !(player getVariable ["defendant",false]) && !life_laser_inprogress) then // exitWith
	{
...
switch (true) do
{
	...
	case (life_breakout) :
	{
		...
		[[0,format["%1 has escaped from prison!",name player]],"life_fnc_broadcast",nil,false] spawn BIS_fnc_MP;
		if (worldName != "Australia") then { player setPos (getMarkerPos _jailRelease); };
		//[[getPlayerUID player],"life_fnc_wantedPardon",false,false] spawn BIS_fnc_MP;
		sleep 2;
	...
	case (alive player && !life_breakout && !_bail) :
	{
		...
		if (life_bail_amount >= life_executionThreshold && (life_configuration select 9)) then
		{
			...
			sleep 9;
			player setDamage 1;
			[["999999",4000,"add",player],"ASY_fnc_updateGangBank",false,false] spawn BIS_fnc_MP;
			life_respawned = true;
