...
if (!(_unit isKindOf "Man")) then
{
	...
	{
		[[_unit, _alarmSound, 50], "life_fnc_playSound", true, false] spawn BIS_fnc_MP;
		[[Str(life_wanted),name player,8],"life_fnc_clientMessage",west,false] spawn BIS_fnc_MP;
	};
};
...
for "_i" from 0 to 1 do
{
	...
	if(_i == 1) then {sleep 0.2;};
};

if(player distance _unit > 10) exitWith {life_action_in_use = false; titleText["You are not in range!","PLAIN"];};
...
if (_unit isKindOf "Man") then {
	...
	else
	{
		...
		[[0,format["%1 was seen trying to lockpick %2's handcuffs.",name player,name _unit]],"life_fnc_broadcast",west,false] spawn BIS_fnc_MP;
	};
} else {
	[[0,format["%1 was seen trying to lockpick a %2.",name player,getText(configFile >> "CfgVehicles" >> (typeOf _unit) >> "displayName")]],"life_fnc_broadcast",west,false] spawn BIS_fnc_MP;
	if(_dice <= _chance) then
	{
		...
		//[[63, player, format["Picked lock of %1", typeOf _unit]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	...
	else
	{
		...
		titleText["The lockpick broke.","PLAIN"];
		//[[62, player, format["Attempted to pick lock of %1 but failed", typeOf _unit]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	};
};