...
//Player Inventory Items
{
	...
	if(_x != "life_money") then
	{
		...
		if(_val > 0) then
		{
			...
			_pInv lbSetPicture [(lbSize _pInv)-1,format["icons\%1.paa",_shrt]];
		};
	}
		else
	{
		if(life_money > 0) then
		{
			...
			_pInv lbSetPicture [(lbSize _pInv)-1,format["icons\%1.paa","money"]];
		};
	};
...
//Trunk Inventory Items
{
	if((_x select 0) != "money") then
	{
		...
		if(_val > 0) then
		{
			...
			_tInv lbSetPicture [(lbSize _tInv)-1,format["icons\%1.paa",_x select 0]];
		};
	}
		else
	{
		...
		if(_val > 0) then
		{
			...
			_tInv lbSetPicture [(lbSize _tInv)-1,format["icons\%1.paa","money"]];
		};
	};
} foreach _data;