...
if(isNull _vehicle OR !(_vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR (typeOf _vehicle) in ["Land_TentA_F", "Land_TentDome_F"] OR (_vehicle getVariable["containerId", -1] > -1))) exitWith {systemChat "Bad Vehicle";}; //Either a null or invalid vehicle type.
