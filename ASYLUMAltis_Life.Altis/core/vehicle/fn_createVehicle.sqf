...
if (isServer) then { [_vehicle,{_this addEventHandler["handleDamage",{_this call life_fnc_handleVehicleDamage}]}] remoteExecCall ["BIS_fnc_call",_player]; }
else { _vehicle addEventHandler["handleDamage",{_this call life_fnc_handleVehicleDamage;}]; };
_vehicle lock 2;
...
if (_animate != "") then
{
	[[_vehicle,_animate,true],"life_fnc_vehicleAnimate",false,false] spawn BIS_fnc_MP;
	[_vehicle,_animate,true] spawn life_fnc_vehicleAnimate;
};

[[_vehicle],"ASY_fnc_igiInit",false,false] spawn BIS_fnc_MP;
...
[[_vehicle],"ASY_fnc_addVehicle2Chain",_player,false] spawn BIS_fnc_MP;
_vehicle;