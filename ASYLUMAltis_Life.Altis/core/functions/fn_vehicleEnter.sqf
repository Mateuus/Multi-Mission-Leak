...
if(_vehicle getVariable ["medicsOnly", false] && _position == "driver") then {
	if(side _unit != independent) then {
		_unit action ["GetOut", _vehicle];
		["Only medics are licensed to pilot medivac helicopters!", "hint", _unit] spawn BIS_fnc_MP;
	} else {
...
{
	[[_vehicle,[-0.010813,-0.506166,-0.7]],"life_fnc_centerMass",_unit,false] spawn BIS_fnc_MP;
};
//["in","life_fnc_fireMode",_unit, false] spawn BIS_fnc_MP;