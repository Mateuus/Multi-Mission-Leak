...
*/
private["_unit","_killer","_weapons","_handle","_gangPoints"];
...
[[0,_message],"life_fnc_broadcast",true,false] spawn BIS_fnc_MP;
...
[[_unit,false],"life_fnc_setRestrained",false,false] spawn BIS_fnc_MP;
...
if(!isNull (_unit getVariable ["currentlyEscorting", objNull])) then {[] call life_fnc_stopEscorting;};
...
//[[player,"AinjPfalMstpSnonWnonDf_carried_fallwc","switch"],"life_fnc_animSync",true,false] spawn BIS_fnc_MP;
...
_unit spawn
{
	...
	waitUntil {_medic=""; if(!isNull life_medic_enroute) then { _medic = format["Paramedic %1 is en-route! ", name life_medic_enroute]; }; _Timer ctrlSetText format["%2Respawn available in: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString,_medic];
	...
	waitUntil {_medic=""; if(!isNull life_medic_enroute) then { _medic = format["Paramedic %1 is en-route! ", name life_medic_enroute]; }; _Timer ctrlSetText format["%2You will bleed to death in: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString,_medic];
...
if(side _source != west && alive _source) then
{
		...
		else
	{
		if ((_source getVariable ["gang","0"]) in life_gang_wars) then
		{
			...
			[[187, _source, format["Gang killed %1 (%2)", profileName, getPlayerUID player]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
			...
			if (_giveStat) then
			{
				...
				[[0,1],"life_fnc_addStatistic",_source,false] spawn BIS_fnc_MP;
		...
		{
			[[200],"life_fnc_addExperience",_source,false] spawn BIS_fnc_MP;
...
{
	_extra = 0;
	_diff = (_source getVariable ["season",0]) - _myPoints;
	if (_diff > 50) then { _extra = floor (_diff / 50) };
	_gangPoints = floor(_gangPoints + _extra);
	[[2,_gangPoints],"life_fnc_addStatistic",_source,false] spawn BIS_fnc_MP;
};

if(side _source == west && playerSide == civilian && life_wanted > 0) then
{
	[[life_wanted / 2,life_wanted],"life_fnc_bountyReceive",_source,false] call BIS_fnc_MP;
	serv_wanted_remove = [player];
	publicVariableServer "serv_wanted_remove";
	//[[player,_source,true],"life_fnc_wantedBounty",false,false] spawn BIS_fnc_MP;
	//[[player],"life_fnc_wantedRemove",false,false] spawn BIS_fnc_MP;
};
...
//[[58, player, format["%1 was killed by %2", name _unit, name _source]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
_source addRating 999999;

// Sort out the body
//deleteVehicle _unit;
[[2, player, format["Killed by %1 (%2)",name _source,getPlayerUID _source]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
