...
else
{
	if (_unit isKindOf "House") then
	{
		...
		if ((getMarkerPos _marker) select 0 == 0) then
		{
			...
			_dir = if (typeOf cursorObject in ["Land_HouseA","Land_HouseB","Land_HouseB1","Land_HouseC_R","Land_HouseA1_L","Land_HouseB1_L","Land_HouseC1_L","Land_HouseA1","Land_HouseDoubleAL","Land_HouseDoubleAL2"]) then
			{ (direction _unit) - 180 } 
			else
			{ (direction _unit) - 90 };
...
[[getPlayerUID player,playerSide,life_garage_type,player],"ASY_fnc_getVehicles",false,false] spawn BIS_fnc_MP;
