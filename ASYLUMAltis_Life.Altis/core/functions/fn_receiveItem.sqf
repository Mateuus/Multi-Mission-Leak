...
if(_diff != (parseNumber _val)) then
{
	if(([true,_item,_diff] call life_fnc_handleInv)) then
	{
		...
		[[_from,_item,str((parseNumber _val) - _diff),_unit],"life_fnc_giveDiff",_from,false] spawn BIS_fnc_MP;
		//[[61, player, format["Given %1 from %2 but only had room for %3", _val, name _from, _diff]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn BIS_fnc_MP;
	};
}
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		//[[60, player, format["Given %1 %2 from %3", _val, _type, name _from]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn BIS_fnc_MP;
