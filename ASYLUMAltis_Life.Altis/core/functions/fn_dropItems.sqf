...
if (!_death) then
{
	...
	if ((time - life_last_transfer) < 3) exitWith{ _exit = true; hint "You can only drop items once per 3 seconds. Try dropping more at a time.";};
...

{
	...
	if (_item == "life_inv_dirty_money" && life_inv_dirty_money > 0) then { [[223, _unit, format["Dropped Dirty Money: $%1 ", _value ]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP; };
	
	if ((_value > 0) && (playerSide == civilian) && !(_item in ["life_inv_debitcard","life_money"])) then
	{
		_var = [_item,1] call life_fnc_varHandle;
		//_pos = _unit modelToWorld [0,3,0]; //ASLToAGL (getPosASL _unit);
		_obj = createVehicle [_model, getPosATL vehicle _unit, [], 2, "NONE"];
		...
		_obj spawn { sleep 3; [[_this],"life_fnc_simDisable",false,false] spawn BIS_fnc_MP; _this enableSimulation false; };
	};
...
{
	[[222, player, format["Dropped Money: $%1 ", life_money ]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	_obj = createVehicle ["Land_Money_F", getPosATL vehicle _unit, [], 2, "NONE"];
