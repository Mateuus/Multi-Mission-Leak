...
if (!life_isdowned) then {
	//[[48, player, format["%1 was downed", name player]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	life_isdowned = true;
	...
	if (vehicle player == player) then {
		...
		[[player,"AcinPercMstpSnonWnonDnon_agony"],"life_fnc_animSync",true,false] spawn BIS_fnc_MP;
	...
	while {_downed} do {
		if (!(player getVariable ["restrained",false])) then{
			if (!(_inVehicle)) then{
				if (_time == 5) then {[[player,"AinjPpneMstpSnonWrflDnon"],"life_fnc_animSync",true,false] spawn BIS_fnc_MP;};
				if (_time == 5) then {
