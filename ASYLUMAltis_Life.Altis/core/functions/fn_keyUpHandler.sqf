...
switch (_code) do
{
	...
	case 9:
	{
		if (!isNil "last_medic_call") then
		{
			...
			{
				if (!isNull last_medic_call && !alive last_medic_call) then
				{
					hint format["%1 has been informed that you are en-route.  Make haste!", last_medic_call getVariable ["name", "An unknown civilian"]];
					[[player],"life_fnc_medicEnroute",last_medic_call,false] spawn BIS_fnc_MP;
				}
				else
				{
			...
			switch (playerSide) do
			{
				...
				case (independent): {  // medic siren for medics
					if(isNull (missionNamespace getVariable["life_sirenLogic",ObjNull])) then {
						...
						[[_veh,life_sirenLogic],"life_fnc_medicSiren",nil,true] spawn BIS_fnc_MP;
					} else {
		...
		if(!_alt && !_ctrlKey) then
		{
			...
			if(((_index > -1 )||(_veh in life_vehicles)) && player distance _veh < 10 && !(playerSide == civilian && typeOf _veh == "B_Heli_Light_01_F" && _veh getVariable["Life_VEH_color",0] == 14)) then
			{
				if(_locked == 2) then
				{
					...
					[[_veh, "unlock",10],"life_fnc_playSound",true,false] spawn BIS_fnc_MP;
					systemChat "You have unlocked your vehicle.";
					//[[106, player, format["Unlocked vehicle %1", typeOf _veh]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
				}
				else
				{
					_veh lock 2;
					[[_veh,2], "life_fnc_lockVehicle",_veh,false] spawn BIS_fnc_MP;
					[[_veh, "car_lock",15],"life_fnc_playSound",true,false] spawn BIS_fnc_MP;
					systemChat "You have locked your vehicle.";
					//[[107, player, format["Locked vehicle %1", typeOf _veh]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
				};
			};
		};
	};
