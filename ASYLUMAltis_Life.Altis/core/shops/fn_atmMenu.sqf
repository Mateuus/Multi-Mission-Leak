...
if (life_gang_atm) then
{
	...
	if (!life_gov_atm) then {
		[[player, life_gang],"ASY_fnc_queryGangBank",false,false] spawn BIS_fnc_MP; 
		group player setVariable["gangBankInUse",getPlayerUID player,true];
	}
	else { [[player, "999999"],"ASY_fnc_queryGangBank",false,false] spawn BIS_fnc_MP; };
}
else
{
...
if (life_wanted > 0 && (time - life_last_wanted) > 240) then
{
	life_last_wanted = time;
	[[Str(life_wanted),name player,5],"life_fnc_clientMessage",true,false] spawn BIS_fnc_MP;
};