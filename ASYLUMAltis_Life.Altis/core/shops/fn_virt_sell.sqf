...
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
	// Wong item sale
	_extra = ".";
	if (_type in ((["wongs"] call life_fnc_virt_shops) select 2)) then
	{
		_cut = 0;
	...
	//[[93, player, format["Sold %1 %2 for %3", _amount, _name, _price]],"ASY_fnc_logIt",false,false] spawn BIS_fnc_MP;
	if (_qty > -1) then
	{
