...
switch (playerSide) do
{
	case west:
	{
		...
		life_actions pushBack (player addAction["Stop Escorting",life_fnc_stopEscorting,"",0,false,false,"",' !isNull (player getVariable["currentlyEscorting",objNull]) && ((player getVariable["currentlyEscorting",objNull]) getVariable ["Escorting",false])']);
		...
		if("B_UavTerminal" in (assignedItems player)) then {
			life_actions pushBack (player addAction["<t color='#0000FF'>Connect to UAV</t>",{createVehicleCrew cursorTarget; player connectTerminalToUAV cursorTarget; [cursorTarget,"ASY_fnc_setIdleTime",false,false] spawn BIS_fnc_MP;},false,90,false,false,"",'!isNull cursorTarget && alive cursorTarget && (player distance cursorTarget) < 6 && (typeOf cursorTarget == "B_UAV_01_F") && (!isUAVConnected cursorTarget)' ]);
		...
		// Splint
		life_actions pushBack (player addAction["<t color='#FF0000'>Apply Splint to Self</t>",life_fnc_splint,player,-1,false,false,"",'life_brokenLeg && (93 in life_talents) && (life_inv_splint > 0) && (life_inv_splint > 0)']);
		life_actions pushBack (player addAction["<t color='#FF0000'>Apply Splint</t>",life_fnc_splint,cursorTarget,-1,false,false,"",'!isNull cursorTarget && isPlayer cursorTarget && alive cursorTarget && (cursorTarget getVariable ["broken",false]) && (93 in life_talents) && (life_inv_splint > 0)']);
	...
	case independent:
	{
		...
		//Lights?
		life_actions pushBack (player addAction["Emergency Lights ON",{[[vehicle player,0.22],"life_fnc_medicLights",true,false] spawn BIS_fnc_MP; vehicle player setVariable["lights",true,true];},"",0,false,false,"",' vehicle player != player && ((typeOf vehicle player) == "C_Offroad_01_F" || (typeOf vehicle player) == "C_SUV_01_F") && !isNil {vehicle player getVariable "lights"} && ((driver vehicle player) == player) && !(vehicle player getVariable "lights")']);
	...
	case civilian:
	{
		...
		!isNull cursorTarget && cursorTarget getVariable ["playerSurrender",false] && !(currentWeapon player in life_disallowedThreatWeapons) && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 5 && !(cursorTarget getVariable ["Escorting",false]) && speed cursorTarget < 1 ']);
...
//Car controls
life_actions pushBack (player addAction ["<t color='#BBBB00'>Put Seatbelt On</t>", {life_seatbelt=true}, "", 3, false, true, "", 'vehicle player isKindOf "Car" && !life_seatbelt']);
...
//Unflip action
life_actions pushBack (player addAction["Unflip Vehicle",life_fnc_flip,"",-1,false,false,"",'(cursorTarget isKindOf "Car" || cursorTarget isKindOf "Air") && (vehicle player == player) && (player distance cursorTarget < 8) && (local cursorTarget || cursorTarget in life_vehicles) && (speed cursorTarget < 1)']);
...
//Custom Repair
life_actions pushBack (player addAction["<t color='#FF0000'>Repair Vehicle</t>",life_fnc_repairTruck,"",99,false,false,"", ' vehicle player == player && !isNull cursorTarget && ((cursorTarget isKindOf "Car") OR (cursorTarget isKindOf "Air") OR (cursorTarget isKindOf "Ship")) && (player distance cursorTarget < 4.5) ']);
//Service Truck Stuff
...
life_actions pushBack (player addAction["<t color='#FF0000'>Blood Transfusion Self</t>",life_fnc_bloodbag,player,-1,false,false,"",'((playerSide == independent && damage player > 0) || ((10 in life_talents || 90 in life_talents) && damage player > 0.2) || ((11 in life_talents || 91 in life_talents) && damage player > 0.1) || ((12 in life_talents || 92 in life_talents) && damage player > 0)) && (life_inv_bloodbag > 0)']);
life_actions pushBack (player addAction["<t color='#FF0000'>Give Blood Transfusion</t>",life_fnc_bloodbag,cursorTarget,-1,false,false,"",'!isNull cursorTarget && isPlayer cursorTarget && alive cursorTarget && ((playerSide == independent && damage cursorTarget > 0) || ((10 in life_talents || 90 in life_talents) && damage cursorTarget > 0.2) || ((11 in life_talents || 91 in life_talents) && damage cursorTarget > 0.1) || ((12 in life_talents || 92 in life_talents) && damage cursorTarget > 0)) && (life_inv_bloodbag > 0)']);
