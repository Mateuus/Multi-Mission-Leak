...
{
	_unit = objNull;
	{ if (getPlayerUID _x == (_task select 6)) then { _unit = _x } } forEach playableUnits;
	if (!isNull _unit) then { [[_unit, -1, true], "ASY_fnc_manageRadio", false, false] spawn BIS_fnc_MP; };
};

[[_task select 0, _success],"life_fnc_finishDispatch",west,false] spawn BIS_fnc_MP;
