...
if (!life_swatSpawn) then
{
	if(count life_spawn_point == 0) then
	{
		...
		if(playerSide == civilian) then
		{
			...
			switch (true) do
			{
				case ( (_sp select 0) in ["turf_label_1","turf_label_2","turf_label_3"] || worldName == "Australia") : 
				{
		...
		else
	{
		if(playerSide == civilian) then
		{
			switch (true) do
			{
				case ( (life_spawn_point select 0) in ["turf_label_1","turf_label_2","turf_label_3"] || worldName == "Australia" ) : 
				{
