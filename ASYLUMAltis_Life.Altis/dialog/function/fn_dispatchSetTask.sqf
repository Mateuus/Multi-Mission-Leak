...
if (ctrlText 39512 == "Unassign") exitWith
{
	if (!isNil "life_dispatchTasks") then
	{
		...
		if (_i > -1) then
		{
			...
			[[[0,1,2],format["Dispatch has relieved you of %1.", _row select 1]],"life_fnc_broadcast",_cop,false] spawn BIS_fnc_MP;
		};
	};
};
...
[[_task select 0,_task select 1,_task select 2], "life_fnc_newDispatch", _cop, false] spawn BIS_fnc_MP;