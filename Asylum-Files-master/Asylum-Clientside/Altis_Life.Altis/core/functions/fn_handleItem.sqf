...
_ammo = 0;
if(typeName _item == "ARRAY") then {_ammo = _item select 1; _item = _item select 0};
_left = [_item,1] call KRON_StrLeft;
if (_left == "[") then
{
	_item = call compile _item;
	if (!isNil {_item select 3}) then { _customTexture = _item select 3; };
...
if(_bool) then
{
	switch((_details select 6)) do
	{
		...
		case "CfgVehicles":
		{
			...
			{
				(backpackContainer player) setObjectTexture [0, _customTexture];
				player setVariable ["customTextureBP", [_item, _customTexture], true];
			};
		...
		case "CfgMagazines":
		{
			...
			{
				if(_ammo > 0) then {
					player addMagazine[_item,_ammo];
				} else {
							...
							else
						{
								...
								else
							{
									...
									else
								{
									...
									if (_customTexture != "") then
									{
										player setObjectTexture [0, _customTexture];
										player setVariable ["customTexture", [_item, _customTexture], true];
										diag_log format["[handleItem] CT = %1", player getVariable ["customTexture", "Nadda"]];
									};
							...
							else
						{
								...
								else
							{
									...
									else
								{
									...
									if(vas_prompt_choice && worldName != "Australia") then
									{
										switch (_type) do
										{
											case 1: { player addPrimaryWeaponItem _item; };
