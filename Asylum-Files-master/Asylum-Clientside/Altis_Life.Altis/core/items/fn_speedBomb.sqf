...
[_unit] spawn
{
	...
	{ [[2,"A speed bomb has been activated on this vehicle and will detonate when your speed drops below 50km/h!"],"life_fnc_broadcast",_x,false] spawn life_fnc_MP; } foreach (crew _veh);
	waitUntil {(speed _veh) < 50};
	serv_killed = [player,"1090"];
	publicVariableServer "serv_killed";
	_test = "Bo_Mk82" createVehicle [0,0,9999];
//Begin Edit: Change position to spawn bomb on player vehicle for towers, docks, and buildings. ~Gnash
	_test setPosATL (getPosATL _veh);
//End Edit
	_test setVelocity [100,0,0];
	hint "A speed bomb you planted on a vehicle has DETONATED!";
	[[0,format["A speed bomb planted by %1 has detonated!", profileName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};