...
if(!isNull _killer && {_killer != _unit} && {side _killer != west} && !(_killer getVariable["undercoverOfficer",false]) && {alive _killer}) then
{
	if(vehicle _killer isKindOf "LandVehicle") then
	{
		//Did you kill a cop or not?
		if( playerSide == west OR player getVariable["undercoverOfficer",false]) then
		{
			[[getPlayerUID _killer,_killer getVariable["realname",name _killer],"18"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
		}
		else
		{
	...
	{
		//Make sure bounty hunters dont get wanted if they need to kill
		if(!(_killer getVariable["bountyHunter",false]) && playerSide != west) then
		{
			//Did you kill a cop or not?
			if( playerSide == west OR player getVariable["undercoverOfficer",false]) then
			{
...
//Killed by cop stuff...
if(((_killer getVariable ["bountyHunter",false]) || (_killer getVariable["undercoverOfficer",false]) || (side _killer == west)) && (playerSide != west)) then {
