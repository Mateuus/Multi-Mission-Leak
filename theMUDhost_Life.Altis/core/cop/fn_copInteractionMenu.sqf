...
private["_civRestrain","_civEscorted"];

_civRestrain = _curTarget getVariable ["restrained",false];
_civEscorted = _curTarget getVariable ["Escorting",false];
...
if(player distance (getMarkerPos "police_hq_1") < 20) then
{
	_Btn6 ctrlSetText localize "STR_pInAct_ArrestCell";
	_Btn6 buttonSetAction "[life_pInact_curTarget] call life_fnc_jailTimeCell;";
}
else
{
...
if(!((player distance (getMarkerPos "jail_marker") < 50 ) OR (player distance (getMarkerPos "police_hq_1") < 20) OR  (player distance (getMarkerPos "police_hq_2") < 30) OR (player distance (getMarkerPos "cop_spawn_3") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30) && _civRestrain)) then 
{
	_Btn6 ctrlEnable false;
};

if(_civEscorted) then
{
...
_Btn10 ctrlSetText localize "STR_pInAct_RemoveGear";
_Btn10 buttonSetAction "[life_pInact_curTarget] call life_fnc_removeGear;";
