...
_msg = "!!!ALERT!!! A security camera has identified this person robbing a gas station. Check your map for the location.";
...
if (currentWeapon _robber == "") exitWith { hint "You can't rob a shop without a gun!" };
...
if(_chance >= 60) then
{
	...
	[[1,format["ALARM! - Gas Station: %1 is being robbed!", _shop]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
if(_cops < 1) exitWith{[[_vault,-1],"disableSerialization;",false,false] spawn life_fnc_MP; hint "You cannot rob a store unless there is a police officer online!";};
...
if(_rip) then
{
	...
	if(_robber distance _shop > 10.5) exitWith { deleteMarker "Marker200"; _shop switchMove ""; hint "You need to stay within 10m to Rob registry! - Now the register is locked."; 5 cutText ["","PLAIN"]; _rip = false; };
	5 cutText ["","PLAIN"];
