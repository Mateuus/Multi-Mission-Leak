...
while {true} do
{
	if((round(_time - time)) > 0) then {
		...
		hintSilent parseText format[(localize "STR_Jail_time")+ "<br/> <t size='2'><t color='#FF0000'>%1</t></t>",_countDown];
	};
	
	if(player distance (getMarkerPos "cell_1") > 60) exitWith {
		_esc = true;
	};
...
switch (true) do
{
	case (_bail) :
	{
		...
		player setPos (getMarkerPos "cell_release");
