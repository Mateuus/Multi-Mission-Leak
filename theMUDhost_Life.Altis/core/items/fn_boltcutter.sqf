...
_msg = "!!!ALERT!!! Someone has seen this person attempting to break into someones home! A marker has been placed on your map where the alarm was triggered. Please respond immediately.";
...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	...
	[] spawn {
		...
		[[getPlayerUID player,profileName,"461"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
		{
			if(side _x == west) then {
				[[player],"life_fnc_breakinMarker",_x,false] spawn life_fnc_MP;
