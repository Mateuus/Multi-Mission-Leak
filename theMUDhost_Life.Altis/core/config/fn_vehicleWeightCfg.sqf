/*
	File: fn_vehicleWeightCfg.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Master configuration for vehicle weight.
*/
private["_className"];
_className = [_this,0,"",[""]] call BIS_fnc_param;

switch (_className) do
{
	case "C_Offroad_01_F": {75};
	case "B_G_Offroad_01_F": {75};
	case "B_Quadbike_01_F": {25};
	case "I_Truck_02_covered_F": {200};
	case "I_Truck_02_transport_F": {200};
	case "C_Hatchback_01_F": {40};
	case "C_Hatchback_01_sport_F": {40};
	case "C_SUV_01_F": {50};
	case "C_Van_01_box_F": {150};
	case "C_Boat_Civil_01_F": {100};
	case "C_Boat_Civil_01_police_F": {100};
	case "C_Boat_Civil_01_rescue_F": {100};
	case "B_Truck_01_box_F": {450};
	case "B_MRAP_01_F": {65};
	case "O_MRAP_02_F": {65};
	case "I_MRAP_03_F": {65};
	case "B_Heli_Light_01_F": {90};
	case "O_Heli_Light_02_unarmed_F": {200};
	case "C_Rubberboat": {50};
	case "B_G_Boat_Transport_01_F": {45};
	case "B_Boat_Transport_01_F": {45};
	case "O_Truck_03_covered_F": {300};
	case "O_Truck_03_device_F": {350};
	case "Land_CargoBox_V1_F": {1000};
	case "O_Heli_Transport_04_F": {125};
	case "B_Heli_Transport_03_F": {200};
	case "O_Heli_Transport_04_box_F": {400};
	case "B_Heli_Transport_03_unarmed_F": {200};
	case "B_SDV_01_F": {50};
	case "Box_IND_Grenades_F": {50};              //Small Supply Crate for Houses
	case "B_supplyCrate_F": {200};                //Large Supply Crate for Houses
	case "Box_NATO_Grenades_F": {50};             //Small Cargo Crate for igiLoad
	case "IG_supplyCrate_F": {200};               //Large Cargo Crate for igiLoad
	case "Land_Cargo40_cyan_F": {100000};
	case "Land_Cargo40_red_F": {100000};
	default {-1};
};