...
if(playerSide == west || player getVariable["undercoverOfficer",false]) then {
	...
	if(typeOf (_curTarget) in ["O_MRAP_02_F","O_MRAP_02_hmg_F","B_MRAP_01_F","B_MRAP_01_hmg_F","I_MRAP_03_hmg_F","B_Heli_Transport_01_F","I_Heli_Transport_02_F"]) then
	{
		if( locked _curTarget == 2) then
		{
			_Btn8 ctrlSetText localize "STR_vInAct_Doors";
			_Btn8 ctrlEnable false;
		}
		else
		{
			_Btn8 ctrlSetText localize "STR_vInAct_Doors";
			_Btn8 buttonSetAction "[life_vInact_curTarget] call life_fnc_vehicleDoors;";
		};
	}
	else
	{
	...
	if(typeOf _curTarget == "O_Truck_03_device_F") then {
		...
		if((locked _curTarget == 2 ) OR (count crew _curTarget == 0 )) then {_Btn3 ctrlEnable false;};
	};
	...
	if(typeOf (_curTarget) in ["O_MRAP_02_F","O_MRAP_02_hmg_F","B_MRAP_01_F","B_MRAP_01_hmg_F","I_MRAP_03_hmg_F","B_Heli_Transport_01_F","I_Heli_Transport_02_F"]) then
	{
		if( locked _curTarget == 2) then
		{
			_Btn4 ctrlSetText localize "STR_vInAct_Doors";
			_Btn4 ctrlEnable false;
		}
		else
		{
			_Btn4 ctrlSetText localize "STR_vInAct_Doors";
			_Btn4 buttonSetAction "[life_vInact_curTarget] call life_fnc_vehicleDoors;";
		};
	}
	else
	{
	...
	if((_curTarget isKindOf "Car" OR _curTarget isKindOf "Ship") && !life_istazed && life_inv_demolitioncharge > 0) then
	{
		_Btn5 ctrlSetText localize "STR_vInAct_Plantcharge";
		_Btn5 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_demolitionCharge;";
	}
	else
	{
