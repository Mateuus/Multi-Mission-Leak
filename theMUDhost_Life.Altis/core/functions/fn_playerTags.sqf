...
_goggles = ["G_Balaclava_blk","G_Bandanna_blk"];
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				case ((headgear _x) in _headgear): {format["<t color='#FF0000'>Masked Player</t>"];};
				case ((goggles _x) in _goggles): {format["<t color='#FF0000'>Masked Player</t>"];};
				...
				case (!isNil {(_x getVariable "rank")}): {format["<img image='%1' size='1'></img> %2",switch ((_x getVariable "rank")) do {
					case 1: {"icons\ranks\constable.paa"}; 
					case 2: {"icons\ranks\sergeant.paa"};
					case 3: {"icons\ranks\lieutenant.paa"};
					case 4: {"icons\ranks\detective.paa"};
					case 5: {"icons\ranks\swat.paa"};
					case 6: {"icons\ranks\captain.paa"};
					case 7: {"icons\ranks\chief.paa"};
			...
			if(_x getVariable ["DirectVON",false]) then {_text = "<t color='#00FF88'>[Speaking]</t> " + _text};
