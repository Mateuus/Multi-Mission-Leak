...
if(typeOf _curTarget in["IG_supplyCrate_F","Box_NATO_Grenades_F"]) exitWith {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
    ...
    if(playerSide == west && side _curTarget != west && {(_curTarget distance player < 5)}) then {
        [_curTarget] call life_fnc_copInteractionMenu;
    };
	
	if(playerSide == civilian && player getVariable["undercoverOfficer",false] && side _curTarget != west && {(_curTarget distance player < 5)}) then {
		[_curTarget] call life_fnc_copInteractionMenu;
	};

    if(playerSide == civilian && !(player getVariable["undercoverOfficer",false]) && {(_curTarget distance player < 5)}) then {
