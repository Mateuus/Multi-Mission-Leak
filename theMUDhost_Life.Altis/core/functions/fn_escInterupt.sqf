...
_canUseControls = {
	...
	if((player getVariable["restrained",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (life_is_arrested) OR (life_is_arrestedCell) OR (life_istazed)) then {false} else {true};
};
