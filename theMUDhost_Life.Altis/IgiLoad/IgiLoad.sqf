...
//	Author: Igi_PL & Modified by Kevin Webb																						//
...
if (isnil "IL_Variables") then
{
	...
	// Supported vehicles
	IL_Supported_Vehicles_OFFROAD = ["C_Offroad_01_F", "B_G_Offroad_01_F", "I_G_Offroad_01_F"];
	IL_Supported_Vehicles_VAN = ["B_G_Van_01_transport_F", "C_Van_01_transport_F"];
	...
	//Supported cargo
	IL_Supported_HEMTT = ["B_Truck_01_transport_F", "B_Truck_01_box_F", "B_Truck_01_mover_F", "B_Truck_01_ammo_F", "B_Truck_01_fuel_F", "B_Truck_01_medical_F", "B_Truck_01_Repair_F"];
	IL_Supported_KAMAZ = ["I_Truck_02_transport_F", "O_Truck_02_transport_F"];
	...
	IL_Supported_OFFROAD = ["C_Offroad_01_F", "B_G_Offroad_01_F", "B_G_Offroad_01_armed_F", "I_G_Offroad_01_armed_F", "I_G_Offroad_01_F"];
	IL_Supported_SUV = ["C_SUV_01_F"];
	IL_Supported_Hatchback = ["C_Hatchback_01_F", "C_Hatchback_01_sport_F"];
	IL_Supported_Quadbike = ["I_Quadbike_01_F", "C_Quadbike_01_F", "O_Quadbike_01_F", "B_G_Quadbike_01_F", "B_Quadbike_01_F", "C_Kart_01_F"];
	IL_Supported_Gokart = ["C_Kart_01_Blu_F", "C_Kart_01_Fuel_F", "C_Kart_01_Red_F", "C_Kart_01_Vrana_F"];
	...
	IL_Supported_Cargo_Veh_Offroad = IL_Supported_Quadbike + IL_Supported_Gokart;
	...
	IL_Supported_Cargo_Veh_Kamaz = IL_Supported_Quadbike + IL_Supported_Rubberboat + IL_Supported_SDV + IL_Supported_Hatchback + IL_Supported_UGV + IL_Supported_OFFROAD + IL_Supported_SUV;
	...
	IL_Supported_Cargo_Veh_HEMTT = IL_Supported_Cargo_Veh_Kamaz + IL_Supported_VAN;
	...
	IL_Supported_Cargo_Veh_Mohawk = IL_Supported_Quadbike;
	IL_Supported_Cargo_NonVeh_Mohawk = IL_Supported_Supply_Crate + IL_Supported_Veh_Ammo + IL_Supported_Barrel + IL_Supported_Tank + IL_Supported_Box_H2;
...
//	END IL_Init_Veh
	...
	IL_Init_Box =
	{
		...
		if (_obj_type in IL_Supported_Gokart) then
		{
			_obj setVariable["slots", 2, true];
			_obj setVariable["cargo_offset", 0.5, true];
		};
...
//	END IL_Cargo_Para
	...
	IL_Do_Load =
	{
		...
		{
			{
				if (locked _x < 2 || playerSide != civilian) then
				{
					if (IL_DevMod) then
					{
				...
				} else { hint "You could not load the vehicle, as it was locked."; };
				if (_done) exitWith {};
			} forEach (_obj_lst);
		}
		else
		...
		} else { hint "You cannot do this while your vehicle is locked."; _v setVariable["can_load", true, true]; };
	};
//	END IL_Do_Load
		...
		{
			{
				...
				if (_x getVariable "doors" == _doors) then
				{
					...
					{
						_x setVariable["loaded",false,true];
...
if (_obj_main_type in IL_Supported_Vehicles_MOHAWK) then
{
	...
	"((driver _target == _this) || (('Turret' in (assignedVehicleRole _this)) && (vehicle _this == _target) && (_target getVariable 'can_copilot')) || ((_this in (nearestObjects[ _target modelToWorld [0,-6,-3], [], IL_SDistL + IL_SDistL_Heli_offset])) && (_target getVariable 'can_outside'))) && (_target animationPhase 'CargoRamp_Open' == 1) && (_target getVariable 'can_load') && ((locked _target) < 2)"
	];
};
